...
1. **Run Migration**:
```bash
mysql -h <mas_hrms DB host — see backend/.env> -u shivam_user -p'<set DB_PASSWORD in backend/.env>' mas_hrms < backend/sql/050_customization.sql
```

2. **Verify Tables**:
```bash
mysql -h <mas_hrms DB host — see backend/.env> -u shivam_user -p'<set DB_PASSWORD in backend/.env>' mas_hrms -e "SHOW TABLES LIKE 'customization%';"
...
- Database: `mysql -h <mas_hrms DB host — see backend/.env> -u shivam_user -p'<set DB_PASSWORD in backend/.env>' mas_hrms`
