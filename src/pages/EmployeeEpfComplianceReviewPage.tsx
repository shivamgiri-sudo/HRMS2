...
export default function EmployeeEpfComplianceReviewPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(err?.message || "Unable to load the EPF review link.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const act = async (action: "confirm" | "request_correction" | "esign") => {
    if ((action === "confirm" || action === "esign") && Object.values(declarations).some((value) => !value)) {
      setError("Please confirm all EPF declaration statements before continuing.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(140deg,#071427_0%,#11284a_45%,#0b1524_100%)] px-4 py-8 text-slate-100">
      <div className="mx-auto max-w-5xl space-y-5">
        <div className="rounded-[30px] border border-white/10 bg-white/5 p-6 backdrop-blur">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-cyan-300">EPF Employee Review</p>
          <h1 className="mt-2 text-3xl font-black">{payload?.session.document_name || "EPF Declaration"}</h1>
          <p className="mt-2 text-sm text-slate-300">These details have been prepared for your joining/statutory documents. Please review carefully. Some values may have been entered by HR where system data was unavailable.</p>
        </div>
          ...
          <div className="grid gap-5 xl:grid-cols-[1.1fr,0.9fr]">
            ...
            <div className="rounded-[30px] border border-white/10 bg-white/5 p-5">
              ...
              <div className="mt-4 space-y-2 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-slate-200">
                {[
                  ["detailsCorrect", "I confirm that the details are correct."],
                  ["consentGiven", "I consent to use these details for EPF/statutory compliance."],
                  ["delayRiskAccepted", "I understand incorrect details may delay PF benefits."],
                  ["declarationAccepted", "I understand I am responsible for the declaration submitted."],
                ].map(([key, label]) => (
                  <label key={key} className="flex items-start gap-3">
                    <input
                      type="checkbox"
                      checked={Boolean(declarations[key as keyof typeof declarations])}
