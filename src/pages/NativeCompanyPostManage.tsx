...
export default function NativeCompanyPostManage() {
  ...
  const manageQuery = useManageCompanyPosts();
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast({ title: "Post removed", description: "The moderation management list has been refreshed." });
            ...
            <div className="max-w-3xl space-y-4">
              ...
              <div className="space-y-3">
                ...
                <p className="max-w-2xl text-sm leading-7 text-blue-50/92 sm:text-[15px]">
                  Filter posts by lifecycle state, inspect moderation metadata, and remove anything that should no longer remain visible or operationally active.
                </p>
              </div>
            </div>
            <Button asChild className="h-auto justify-between rounded-[1.15rem] bg-white px-4 py-3 text-left text-[color:var(--brand-700)] hover:bg-blue-50">
              <Link to="/engagement/company-feed/approvals">
                <span>
                  ...
                  <span className="mt-1 block text-xs text-slate-500">Jump to the posts still waiting for action.</span>
                </span>
          ...
          <CardContent className="space-y-6 p-5 sm:p-6">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              ...
              <Input
                ...
                placeholder="Search copy, employee ID, notes, or status"
              ...
              <div className="rounded-[1.2rem] border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1">{manageQuery.error?.message ?? "Unable to load moderation history."}</p>
                  </div>
                </div>
                <Button variant="outline" className="mt-4 rounded-xl" onClick={() => void manageQuery.refetch()}>
            ...
            {!manageQuery.isLoading && !manageQuery.isError ? (
              <div className="space-y-4">
                {filteredPosts.map((post) => (
                  <div key={post.id} className="rounded-[1.5rem] border border-slate-200 bg-slate-50/80 p-5">
                      ...
                      <div className="space-y-3">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-slate-500">
                            Employee {post.author_employee_id}
                          </span>
                        </div>
                            ...
                            {post.media.slice(0, 4).map((media) => (
                              ...
                              >
                                <img
                                  ...
                                  alt={`Managed post image ${media.sort_order}`}
                        ...
                        <div className="grid gap-2 text-sm text-slate-500 md:grid-cols-2">
                          <p>Created: <span className="font-semibold text-slate-700">{formatDateTime(post.created_at)}</span></p>
                          <p>Updated: <span className="font-semibold text-slate-700">{formatDateTime(post.updated_at)}</span></p>
                          {post.approved_at ? <p>Approved: <span className="font-semibold text-slate-700">{formatDateTime(post.approved_at)}</span></p> : null}
                          {post.rejected_at ? <p>Rejected: <span className="font-semibold text-slate-700">{formatDateTime(post.rejected_at)}</span></p> : null}
                        </div>
                        {post.review_notes ? (
                          <div className="rounded-[1rem] border border-slate-200 bg-white p-3 text-sm text-slate-600">
                            <span className="font-semibold text-slate-900">Review notes:</span> {post.review_notes}
                          </div>
                        ) : null}
                        {post.rejection_reason ? (
                          <div className="rounded-[1rem] border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">
                            <span className="font-semibold">Rejection reason:</span> {post.rejection_reason}
                          </div>
                        ) : null}
                      </div>
