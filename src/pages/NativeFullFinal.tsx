...
import {
  ...
  Loader, Lock, RefreshCcw, ShieldCheck, UserMinus,
} from "lucide-react";
...
type ExitRequest = {
  ...
  /** Joined from ff_calculations — present when backend includes it */
...
export function netFromForm(form: FFFormState) {
  return toNum(form.earned_leave_encashment) + toNum(form.gratuity_amount)
    - toNum(form.notice_recovery) - toNum(form.salary_hold) - toNum(form.advances_recovery);
}
function netFromFF(ff: FFCalculation) {
  return ff.earned_leave_encashment + ff.gratuity_amount
    - ff.notice_recovery - ff.salary_hold - ff.advances_recovery;
}

/** Left-border accent for exit request list items. */
    ...
    <div className="space-y-5">
      ...
      {previewLoading && (
        <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-500">
          <Loader className="h-4 w-4 animate-spin" /> Computing real figures from attendance, leave and loan records…
        </div>
      )}
            ...
            <div key={key}>
              ...
              {isDeviating && (
                <p className="mt-1 text-xs font-bold text-amber-700">
                  Differs from the computed value ({INR(computedFields[key]!.value)}) — explain below.
                </p>
              )}
            </div>
          );
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          />
          {netMismatch && (
            <p className="mt-1 text-xs font-bold text-amber-700">Warning: manual net payable differs from calculated value.</p>
          )}
        </div>
      </div>
      <button
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 py-3.5 text-sm font-bold text-white transition-colors hover:bg-slate-800 disabled:opacity-50"
      >
...
function FFBreakdown({ ff }: { ff: FFCalculation }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Credits & Deductions tables */}
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          <h4 className="mb-2 text-xs font-black uppercase tracking-wide text-slate-500">Credits</h4>
          <table className="w-full text-sm">
            <tbody>
              {([["Earned Leave Encashment", ff.earned_leave_encashment], ["Gratuity Amount", ff.gratuity_amount]] as [string, number][]).map(([label, amount]) => (
      ...
      <div className="grid gap-3 text-sm md:grid-cols-3">
        ...
        <div className="rounded-xl bg-slate-50 p-3">
          ...
          <p className="mt-1 font-mono font-bold text-slate-950">{ff.calculation_date?.slice(0, 10)}</p>
        </div>
      </div>
    </div>
  );
...
export default function NativeFullFinal() {
  ...
  /** Count of provisional settlements currently visible (from loaded ffCalc). */
  const provisionalCount = useMemo(
    () => (ffCalc && Number(ffCalc.is_ff_provisional ?? 0) === 1 ? 1 : 0),
    [ffCalc],
  );
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition-colors hover:bg-slate-50 disabled:opacity-50 lg:self-start"
          >
        ...
        {/* ── KPI strip — colored gradient tiles ── */}
        <div className="grid gap-4 md:grid-cols-4">
          {/* Total Eligible Exits — slate */}
          <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-slate-100 p-4">
            ...
            <p className="mt-2 text-2xl font-black text-slate-700">{exitRequests.length}</p>
          </div>
          {/* Clearance Progress — blue */}
          <div className="rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
            <p className="text-xs font-black uppercase text-blue-600">Clearance Progress</p>
            <p className="mt-2 text-2xl font-black text-blue-700">
              {selectedRequest ? `${selectedClearancePct}%` : "—"}
            </p>
          </div>
          {/* Settlement Status — emerald */}
          <div className="rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-green-100 p-4">
            <p className="text-xs font-black uppercase text-emerald-600">Settlement Status</p>
            <p className="mt-2 text-2xl font-black text-emerald-700 capitalize">{ffCalc?.status ?? "—"}</p>
          </div>
          {/* Provisional (Blocked) — red */}
          <div className="rounded-2xl border border-red-200 bg-gradient-to-br from-red-50 to-rose-100 p-4">
            ...
            <p className="mt-2 text-2xl font-black text-red-700">{provisionalCount}</p>
          </div>
        </div>
          ...
          <div className="w-80 flex-shrink-0 overflow-hidden rounded-3xl border bg-white shadow-sm">
            ...
            {loadingRequests ? (
              ...
              <div className="px-4 py-12 text-center text-slate-400">
                ...
                <p className="text-sm font-semibold">No eligible exit requests found.</p>
              </div>
            ) : (
                ...
                {exitRequests.map((req) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full p-4 text-left transition-colors ${rowBorderClass(req)} ${
            ...
            {!selectedRequest ? (
              ...
              <div className="space-y-5">
                ...
                {!ffCalc ? (
                  ...
                  <>
                    {/* F&F Breakdown panel — rose gradient header */}
                    <div className="overflow-hidden rounded-3xl border bg-white shadow-sm">
                      <div className="bg-gradient-to-br from-rose-600 via-pink-600 to-rose-700 px-5 py-4">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                        ...
                        {/* Provisional lock strip inside dialog header */}
                    ...
                    {/* Approve button — disabled + tooltip when provisional */}
                    {ffCalc.status === "draft" && (
                      <button
                        onClick={approveFF}
                        ...
                        className="flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-600 py-3.5 text-sm font-bold text-white transition-colors hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {approving
