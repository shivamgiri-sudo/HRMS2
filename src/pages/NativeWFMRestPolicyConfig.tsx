...
export default function NativeWFMRestPolicyConfig() {
  ...
  // even though they can write a branch-scoped policy just fine at the API level; the
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            ...
            {form.scope_type === "branch" && (
              <label className="text-xs font-bold uppercase text-slate-500">Branch
                {branchesQ.isError ? (
                  <>
                    <input value={form.scope_id} onChange={(e) => setForm({ ...form, scope_id: e.target.value })} placeholder="branch UUID" className="mt-1 block w-full rounded-xl border p-3 text-sm" />
