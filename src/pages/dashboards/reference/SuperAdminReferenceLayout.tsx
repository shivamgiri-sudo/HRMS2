...
export function SuperAdminReferenceLayout({ data }: { data: ReferenceDashboardData }) {
  ...
  const onLeave = asNumber(data.workforce.on_leave ?? numberAt(data.workforce, "summary", "on_leave"));
  ...
  const movement = arrayAt(data.workforce, "movement").slice(-12).map((row) => ({
  ...
  return (
    <div className="reference-dashboard-page">
      <ReferenceHeader title="Super Admin Dashboard 👋" subtitle="Monitor your entire HR ecosystem and system health" />
      ...
      <div className="grid gap-4 xl:grid-cols-[0.82fr_0.78fr_0.78fr_1.22fr]">
        ...
        <ReferencePanel title="Employee Trend (This Year)" action={<span className="text-[10px] text-[#61708a]">This Year</span>}>
          <ReferenceLineChart data={movement} height={190} />
        </ReferencePanel>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.05fr_0.9fr_1.05fr]">
        <ReferencePanel title="Branch / Process Snapshot" action={<a className="text-[10px] font-semibold text-[#0b63e5]" href="/reports">View all</a>} bodyClassName="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-[9px]"><thead className="bg-[#f8fafc] text-[#61708a]"><tr><th className="px-4 py-2">Branch / Process</th><th>Employees</th><th>Present %</th><th>Status</th></tr></thead><tbody className="divide-y divide-[#edf1f6]">
              {branchRows.length ? branchRows.slice(0, 8).map((row, index) => {
                const pct = asNumber(row.present_pct ?? row.attendance_pct);
                return <tr key={String(row.id ?? index)}><td className="px-4 py-2.5 font-medium text-[#1d2b45]">{String(row.branch_name ?? row.process_name ?? row.name ?? `Scope ${index + 1}`)}</td><td>{formatValue(row.employee_count ?? row.employees)}</td><td>{formatValue(pct, "%")}</td><td className={pct !== null && pct >= 70 ? "font-semibold text-[#16a34a]" : "font-semibold text-[#f97316]"}>{pct !== null && pct >= 70 ? "Healthy" : "Warning"}</td></tr>;
        ...
        <ReferencePanel title="Approval Queue" action={<span className="rounded-full bg-[#edf4ff] px-2 py-0.5 text-[9px] font-bold text-[#0b63e5]">{formatValue(pending)} Pending</span>} bodyClassName="p-0">
          <div className="divide-y divide-[#edf1f6]">
            ...
            <ReferenceListRow icon={Network} title="Job Requisitions" value={asNumber(data.ats.pending_requisitions)} tone="violet" href="/jobs" />
          </div>
        </ReferencePanel>
        ...
        <ReferencePanel title="Quick Actions">
          <div className="grid grid-cols-4 gap-3">
            ...
            <ReferenceQuickLink icon={Megaphone} title="Create Announcement" href="/communication" tone="red" />
            ...
            <ReferenceQuickLink icon={Users} title="User Management" href="/users" tone="blue" />
      ...
      <div className="grid gap-4 xl:grid-cols-[0.9fr_0.9fr_1.2fr]">
        <ReferencePanel title="System Alerts" action={<a className="text-[10px] font-semibold text-[#0b63e5]" href="/system-logs">View all</a>} bodyClassName="p-0">
          <div className="divide-y divide-[#edf1f6]">
            ...
            <ReferenceListRow icon={TriangleAlert} title="Payroll pending" subtitle="Review current payroll cycle" value={asNumber(data.payroll.pendingQueues as unknown)} tone="amber" href="/payroll" />
        ...
        <ReferencePanel title="Compliance Alerts" action={<a className="text-[10px] font-semibold text-[#0b63e5]" href="/compliance">View all</a>} bodyClassName="p-0">
          <div className="divide-y divide-[#edf1f6]">
            <ReferenceListRow icon={TriangleAlert} title="Documents expired" value={asNumber(data.workforce.expired_documents)} tone="red" href="/compliance" />
            <ReferenceListRow icon={TriangleAlert} title="Policies pending acknowledgement" value={asNumber(data.workforce.pending_policy_acknowledgements)} tone="amber" href="/compliance" />
