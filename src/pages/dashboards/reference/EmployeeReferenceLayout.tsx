...
import { asNumber, formatValue, numberAt, stringAt } from "../reference-dashboard-model";
    ...
    <div className="reference-dashboard-page">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.02fr_0.98fr]">
            ...
            {[
              ...
              return (
                <div key={item.label} className="flex items-center gap-3 px-4 py-5">
                  <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${item.tone === "violet" ? "bg-[#f3efff] text-[#7c3aed]" : item.tone === "green" ? "bg-[#eaf8ef] text-[#16a34a]" : item.tone === "amber" ? "bg-[#fff4e8] text-[#f97316]" : "bg-[#edf4ff] text-[#0b63e5]"}`}>
                    <Icon className="h-5 w-5" />
                  </span>
                  <div className="min-w-0">
                    <p className="truncate text-[10px] font-semibold text-[#1d2b45]">{item.label}</p>
                    <p className="mt-1 text-[23px] font-extrabold leading-none text-[#0b1f44]">{formatValue(item.value, item.suffix)}</p>
                    <p className="mt-2 truncate text-[9px] text-[#71809a]">{item.helper}</p>
                  </div>
                </div>
              );
            })}
        ...
        <AIInsightPanel
          ...
          role="employee"
          title="Attendance & Leave AI Brief"
          enabled={!data.loading}
          data={{ present_days: present, absent_days: absent, late_days: late, attendance_pct: attendancePct, leave_balances: leaveRows }}
        />
      </div>
      ...
      <ReferencePanel title="My Onboarding Status" bodyClassName="px-5 py-4">
        <div className="grid items-center gap-4 sm:grid-cols-[52px_1fr_90px]">
          <span className="flex h-12 w-12 items-center justify-center rounded-full bg-[#f3efff] text-[#7c3aed]">
            <UserCheck className="h-5 w-5" />
          </span>
          <div>
            <div className="flex items-center justify-between gap-3 text-[11px]">
              <span className="font-semibold text-[#1d2b45]">Stage: {stage}</span>
              <span className="font-medium text-[#61708a]">Completed Steps: {formatValue(completedSteps)} / {formatValue(totalSteps)}</span>
            </div>
            <div className="mt-3 h-2 overflow-hidden rounded-full bg-[#edf1f6]">
              <div className="h-full rounded-full bg-[#8b5cf6]" style={{ width: `${Math.min(100, Math.max(0, onboardingPct ?? 0))}%` }} />
            </div>
          </div>
          <div className="text-right">
            <p className="text-[26px] font-extrabold leading-none text-[#0b1f44]">{formatValue(onboardingPct, "%")}</p>
            <p className="mt-1 text-[9px] text-[#71809a]">Complete</p>
          </div>
        </div>
      </ReferencePanel>
        ...
        <ReferencePanel title="My Leave Balance" action={<a href="/leaves" className="text-[10px] font-semibold text-[#0b63e5]">View Leave Policy</a>} bodyClassName="p-0">
          <div className="divide-y divide-[#edf1f6]">
            {leaveRows.length ? leaveRows.map((row) => {
              ...
              return (
                <div key={row.label} className="grid grid-cols-[1fr_90px_1fr_90px] items-center gap-3 px-5 py-3 text-[10px]">
                  <span className="font-medium text-[#1d2b45]">{row.label}</span>
                  <span className="font-bold text-[#16a34a]">{formatValue(row.remaining)} Days</span>
                  <ReferenceProgress label="" value={used} max={total || 1} tone="green" />
                  <span className="text-right font-medium text-[#61708a]">{formatValue(used)} / {formatValue(total)}</span>
                </div>
              );
