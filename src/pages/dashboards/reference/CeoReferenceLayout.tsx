...
export function CeoReferenceLayout({
  data,
  filters,
}: {
  ...
  const payrollReadiness = totalPayroll && ready !== null ? Math.round((ready / totalPayroll) * 1000) / 10 : metricValue(m, "payroll");
  ...
  const movement = arrayAt(data.workforce, "movement").slice(-10).map((row) => ({
    label: String(row.period ?? row.label ?? ""),
    value: Number(row.headcount ?? row.value ?? 0),
  }));
    ...
    <div className="reference-dashboard-page">
      ...
      <ReferenceActionStrip
        ...
        items={[
          ...
          { label: "BGV Pending", value: bgv, detail: "Approvals pending", tone: "red", href: "/ats/bgv-verification" },
      ...
      <div className="grid gap-4 xl:grid-cols-[1.45fr_0.55fr]">
        <div className="grid grid-cols-2 gap-0 overflow-hidden rounded-xl border border-[#e3e9f2] bg-white sm:grid-cols-4">
          {[
            ...
            return (
              <div key={String(label)} className={`flex min-h-[100px] items-start gap-3 border-[#edf1f6] p-4 ${index % 4 !== 3 ? "sm:border-r" : ""} ${index < 4 ? "border-b" : ""}`}>
                <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${tone === "green" ? "bg-[#eaf8ef] text-[#16a34a]" : tone === "red" ? "bg-[#fff0f1] text-[#ef4444]" : tone === "amber" ? "bg-[#fff4e8] text-[#f97316]" : tone === "violet" ? "bg-[#f3efff] text-[#7c3aed]" : "bg-[#edf4ff] text-[#0b63e5]"}`}><IconComponent className="h-4 w-4" /></span>
                <div><p className="text-[10px] font-semibold text-[#1d2b45]">{label}</p><p className="mt-2 text-[21px] font-extrabold leading-none text-[#0b1f44]">{formatValue(value)}</p><p className="mt-2 text-[8px] text-[#71809a]">Live organisation value</p></div>
              </div>
            );
          })}
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[0.62fr_1.05fr]">
        <AIInsightPanel
          contextType="executive_dashboard"
          role="ceo"
          title="Executive AI Briefing"
          enabled={!data.loading}
          data={{ attendance_pct: attendance, shrinkage_pct: shrinkage, revenue_gap_mtd: revenueGap, active_headcount: active, onboarding_pending: onboarding, bgv_pending: bgv, tat_breached: tat, payroll_readiness_pct: payrollReadiness }}
        />

        <ReferencePanel title="Good / Bad Insights">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="border-r border-[#edf1f6] pr-4">
              <p className="text-[10px] font-bold text-[#16a34a]">Good Insights</p>
              <div className="mt-3 space-y-2">
                <ReferenceListRow icon={UserCheck} title="Login adherence" subtitle="Organisation attendance performance" value={attendance === null ? null : `${attendance}%`} tone={attendance !== null && attendance >= 90 ? "green" : "amber"} />
                <ReferenceListRow icon={BadgeCheck} title="Certified learners" subtitle="Training readiness" value={certified} tone="green" />
                <ReferenceListRow icon={Target} title="Payroll readiness" subtitle="Employee data completeness" value={payrollReadiness === null ? null : `${payrollReadiness}%`} tone={payrollReadiness !== null && payrollReadiness >= 90 ? "green" : "amber"} />
              </div>
            </div>
            <div>
              <p className="text-[10px] font-bold text-[#ef4444]">Bad Insights</p>
              <div className="mt-3 space-y-2">
                <ReferenceListRow icon={TriangleAlert} title="TAT breaches" subtitle="Items beyond SLA" value={tat} tone="red" />
                <ReferenceListRow icon={CircleAlert} title="Name mismatch" subtitle="Blocking records" value={mismatch} tone="red" />
                <ReferenceListRow icon={IndianRupee} title="Revenue gap" subtitle="MTD revenue at risk" value={formatCurrency(revenueGap)} tone="red" />
              </div>
            </div>
          </div>
        </ReferencePanel>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
        <ReferencePanel title="Quality Overview (Last 30 Days)">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-lg border border-[#e3e9f2] p-4"><p className="text-[9px] text-[#71809a]">Org Quality Score</p><p className="mt-2 text-[23px] font-extrabold text-[#0b1f44]">{formatValue(qualityScore)}</p></div>
            <div className="rounded-lg border border-[#e3e9f2] p-4"><p className="text-[9px] text-[#71809a]">Quality vs Target</p><p className="mt-2 text-[23px] font-extrabold text-[#0b1f44]">{formatValue(qualityScore, "%")}</p><ReferenceProgress label={`Target ${formatValue(qualityTarget, "%")}`} value={qualityScore} max={qualityTarget || 100} tone={qualityScore !== null && qualityTarget !== null && qualityScore >= qualityTarget ? "green" : "red"} /></div>
            <div className="rounded-lg border border-[#e3e9f2] p-4"><p className="text-[9px] text-[#71809a]">Risk Agents</p><p className="mt-2 text-[23px] font-extrabold text-[#0b1f44]">{formatValue(riskAgents)}</p></div>
          </div>
          <div className="mt-4 overflow-x-auto rounded-lg border border-[#e3e9f2]">
            <table className="w-full text-left text-[9px]"><thead className="bg-[#f8fafc] text-[#61708a]"><tr><th className="px-3 py-2">Process</th><th>Avg Score</th><th>Agents</th><th>Calls</th><th>Status</th></tr></thead><tbody className="divide-y divide-[#edf1f6]">
              {processRows.length ? processRows.slice(0, 6).map((row, index) => <tr key={String(row.id ?? index)}><td className="px-3 py-2 font-medium text-[#1d2b45]">{String(row.process_name ?? row.process ?? `Process ${index + 1}`)}</td><td>{formatValue(row.avg_score ?? row.score)}</td><td>{formatValue(row.agents ?? row.agent_count)}</td><td>{formatValue(row.calls ?? row.audit_count)}</td><td className="font-semibold text-[#16a34a]">{String(row.status ?? "—")}</td></tr>) : <tr><td colSpan={5} className="px-3 py-8 text-center text-[#94a3b8]">Quality scorecard is unavailable</td></tr>}
            </tbody></table>
          </div>
        </ReferencePanel>
        ...
        <ReferencePanel title="KPI Performance">
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-lg border border-[#e3e9f2] p-4"><p className="text-[9px] text-[#71809a]">Org Avg KPI Score</p><p className="mt-4 text-[23px] font-extrabold text-[#0b1f44]">{formatValue(orgScore)}<span className="text-[10px] font-medium text-[#71809a]"> /100</span></p></div>
            <div className="rounded-lg border border-[#d7f0df] bg-[#f2fbf5] p-4"><p className="text-[9px] text-[#71809a]">Best Process</p><p className="mt-4 text-[15px] font-bold text-[#16a34a]">{String(bestProcess?.name ?? bestProcess?.process_name ?? "—")}</p><p className="mt-3 text-[20px] font-extrabold text-[#0b1f44]">{formatValue(bestProcess?.score)}</p></div>
            <div className="rounded-lg border border-[#fee3c5] bg-[#fff9f2] p-4"><p className="text-[9px] text-[#71809a]">Needs Attention</p><p className="mt-4 text-[15px] font-bold text-[#f97316]">{String(needsAttention?.name ?? needsAttention?.process_name ?? "—")}</p><p className="mt-3 text-[20px] font-extrabold text-[#0b1f44]">{formatValue(needsAttention?.score)}</p></div>
          </div>
