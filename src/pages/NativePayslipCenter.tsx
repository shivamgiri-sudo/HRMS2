...
type Payslip = { id: string; employee_id: string; employee_name: string; employee_code?: string; designation?: string; department?: string; month: number; year: number; basic: number; hra: number; other_allowances: number; gross_pay: number; ctc?: number; ctc_annual?: number; pf_employee: number; esic_employee: number; lwp_deduction?: number; lwp_days?: number; advance_recovery?: number; tds_amount?: number; total_deductions: number; net_pay: number; working_days?: number; present_days?: number; epf_number?: string; uan_number?: string; pan_number?: string; bank_account_masked?: string; esi_number?: string; branch_name?: string; location_name?: string; payslip_ref?: string; cheque_no?: string | null; payment_mode?: string | null; payment_date?: string | null; earnings?: PayslipComponent[]; deductions?: PayslipComponent[]; employer_costs?: PayslipComponent[]; acknowledged_at?: string | null; status?: string; };
...
/**
 * `statutory` is resolved server-side from the financial year this certificate
 * covers. The Income-tax Act, 2025 renamed the salary TDS certificate from
 * Form 16 to Form 130 on 1 April 2026, so the name depends on the year covered,
 * not on today's date — a reissued FY 2025-26 certificate is still a Form 16.
 * Optional so an older backend that omits it still renders.
 */
type Form16Statutory = { act: string; certificate_form: string; certificate_label: string; salary_tds_section: string; quarterly_return_form: string; rebate_section: string; period_term: string };
/**
 * `basis` states how much of the year the figures rest on. A mid-year joiner
 * legitimately has fewer than 12 months; a year with unfinalized runs does not,
 * and the totals alone cannot tell those apart. Optional so an older backend
 * still renders.
 */
type Form16Basis = { months_paid: number; first_month: string | null; last_month: string | null; is_partial_year: boolean; financial_year_range: string };
type Form16Data = { financial_year: string; period: string; statutory?: Form16Statutory; basis?: Form16Basis; employee: { name: string; pan: string | null; designation: string | null; period: string }; gross_salary: number; standard_deduction: number; tds_deducted: number; net_taxable_income: number; declaration: { hra: number; "80c": number; "80d": number; regime: string; } | null; };
...
async function downloadPayslipPdf(payslip: Payslip): Promise<void> {
  ...
  const employerCost = (code: string) => Number(payslip.employer_costs?.find((c) => c.component_code.toUpperCase() === code)?.amount ?? 0);
  ...
  // PORTFOLIO was missing here entirely — a Personal Allowance component never
  // matched "PA"/"PERSONAL_ALLOWANCE" and silently fell into the Other Allowance
  // residual below, mislabeled on every PDF this function produced.
  const pa = earning("PA") || earning("PERSONAL_ALLOWANCE") || earning("PORTFOLIO");
  ...
  // Sum by name whatever component has no slot above, rather than inferring it as
  // gross-minus-known — that silently absorbed both new heads (SHSH, PLI, ...) and
  // any arithmetic error into one unexplained "Other Allowance" figure.
  const SLOTTED = new Set(["BASIC", "HRA", "BONUS", "CONV", "CONVEYANCE", "PA",
    "PERSONAL_ALLOWANCE", "PORTFOLIO", "MA", "MEDICAL_ALLOWANCE", "SPECIAL",
    "SPECIAL_ALLOWANCE", "ARREAR", "INCENTIVE"]);
  const unslottedEarnings = (payslip.earnings ?? [])
    .filter((e) => !SLOTTED.has(e.component_code.toUpperCase()))
    .reduce((t, e) => t + Number(e.amount || 0), 0);
  const knownEarnings = basic + hra + bonus + conv + pa + ma + sa + arrear + incentive;
  const oa = Math.max(Number(payslip.gross_pay ?? 0) - knownEarnings - unslottedEarnings, 0) + unslottedEarnings;
  ...
  const adDed = deduction("ADVANCE") || deduction("ADVANCE_RECOVERY") || deduction("ADV") || Number(payslip.advance_recovery ?? 0);
  const SLOTTED_DED = new Set(["PF_EMPLOYEE", "PF_EMP", "ESIC_EMPLOYEE", "ESIC_EMP",
    "TDS", "LWP_DEDUCTION", "LWP", "LOAN", "LOAN_RECOVERY",
    "ADVANCE", "ADVANCE_RECOVERY", "ADV"]);
  const unslottedDeductions = (payslip.deductions ?? [])
    .filter((d) => !SLOTTED_DED.has(d.component_code.toUpperCase()))
    .reduce((t, d) => t + Number(d.amount || 0), 0);
  const knownDed = pf + esic + tds + lwpDed + loan + adDed;
  const otherDed = Math.max(unslottedDeductions, Number(payslip.total_deductions ?? 0) - knownDed, 0);
  const employerPf = employerCost("PF_EMP_CO") || employerCost("PF_EMPLOYER");
  const employerEsic = employerCost("ESIC_EMP_CO") || employerCost("ESIC_EMPLOYER");
...
function Form16Modal({ data, runId, employeeId, onClose }: { data: Form16Data; runId: string; employeeId: string; onClose: () => void }) {
  // Falls back to "Form 16" only when the backend predates the statutory block.
  const certificateLabel = data.statutory?.certificate_label ?? "Form 16";
  const [downloading, setDownloading] = useState(false);
  const [downloadError, setDownloadError] = useState("");
  // The certificate PDF is a GET behind bearer auth, so it cannot be a plain
  // window.open link (the token lives in localStorage, not a cookie). Fetch it
  // as a blob through hrmsApi — which attaches the Authorization header — then
  // open the object URL, the same pattern the NEFT CSV download already uses.
  const downloadCertificate = async () => {
    setDownloading(true);
    setDownloadError("");
    try {
      const blob = await hrmsApi.getBlob(`/api/payroll/form16-certificate/${runId}/${employeeId}/pdf`);
      const url = URL.createObjectURL(blob);
      window.open(url, "_blank", "noopener,noreferrer");
      setTimeout(() => URL.revokeObjectURL(url), 60_000);
    } catch (err) {
      setDownloadError((err as Error).message || "Could not generate the certificate PDF.");
    } finally {
      setDownloading(false);
    }
  };
  const rows: [string, string][] = [["Financial Year", data.financial_year], ["Employee", data.employee.name], ["PAN", data.employee.pan ?? "N/A"], ["Designation", data.employee.designation ?? "N/A"], ["Period", data.employee.period], ["Gross Salary (annual, actual)", INR(data.gross_salary)], ["Standard Deduction", INR(data.standard_deduction)], ["TDS Deducted (annual, actual)", INR(data.tds_deducted)], ["Net Taxable Income (annual)", INR(data.net_taxable_income)]];
  if (data.basis) rows.push(["Months Paid", `${data.basis.months_paid} of 12${data.basis.first_month ? ` (${data.basis.first_month} – ${data.basis.last_month})` : ""}`]);
  if (data.statutory) rows.push(["Deducted under", `Section ${data.statutory.salary_tds_section}`], ["Governing Act", data.statutory.act]);
  ...
  return <div className="fixed inset-0 z-60 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm"><div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-3xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b p-6"><div><h2 className="text-lg font-black text-slate-950">{certificateLabel} Part B Data</h2><p className="text-sm text-slate-500">{data.period}{data.statutory ? ` · ${data.statutory.act}` : ""}</p></div><button onClick={onClose} className="text-slate-400 transition-colors hover:text-slate-700"><X className="h-5 w-5" /></button></div><div className="p-6">{data.basis?.is_partial_year && <div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-800">Covers {data.basis.months_paid} of 12 months. Correct for a mid-year joiner or leaver — otherwise a payroll run for this year may still be unfinalized.</div>}<table className="w-full text-sm"><tbody>{rows.map(([label, value]) => <tr key={label} className="border-b last:border-0"><td className="w-1/2 py-2.5 font-semibold text-slate-500">{label}</td><td className="py-2.5 text-right font-mono font-semibold text-slate-900">{value}</td></tr>)}</tbody></table></div><div className="border-t p-6">{downloadError && <div className="mb-3 rounded-2xl border border-red-200 bg-red-50 p-3 text-sm font-bold text-red-700">{downloadError}</div>}<div className="grid gap-3 sm:grid-cols-2"><button onClick={() => void downloadCertificate()} disabled={downloading} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-4 py-3 text-sm font-bold text-white transition-colors hover:bg-slate-800 disabled:opacity-50">{downloading ? <Loader className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}{downloading ? "Preparing..." : `Download ${certificateLabel} (PDF)`}</button><button onClick={onClose} className="rounded-2xl border border-slate-200 py-3 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-50">Close</button></div><p className="mt-3 text-center text-xs text-slate-400">Part B (employer-computed) certificate. Part A is issued by TRACES and provided separately.</p></div></div></div>;
}

function PayslipModal({ payslip, runId, onClose }: { payslip: Payslip; runId: string; onClose: () => void; }) {
...
export default function NativePayslipCenter() {
  ...
  const uploadDisbursalCsv = async () => {
    ...
    try {
      ...
      const raw = await fetch(`/api/payroll/runs/${selectedRunId}/disbursal-upload`, {
        method: "POST",
        headers: { "Content-Type": "text/csv", Authorization: `Bearer ${token}` },
        body: text,
      });
      if (!raw.ok) throw new Error(`Upload failed: ${raw.status}`);
      const res = await raw.json() as { success: boolean; inserted: number; updated: number; unmatched: string[]; message: string };
  ...
  const canGenerate = selectedRun ? ["approved", "locked", "disbursed"].includes(selectedRun.status) : false;
