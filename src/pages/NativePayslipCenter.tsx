...
export default function NativePayslipCenter() {
  ...
  useEffect(() => { if (selectedRunId) { const run = runs.find((r) => r.id === selectedRunId) ?? null; setSelectedRun(run); void loadLines(selectedRunId); if (run && ["locked", "disbursed"].includes(run.status)) void loadNeftSummary(selectedRunId); else setNeftSummary(null); } }, [selectedRunId]);
  const generatePayslip = async (employeeId: string) => { if (!selectedRunId) return; setGeneratingFor(employeeId); setMessage(""); try { await hrmsApi.post(`/api/payroll/payslip/${selectedRunId}/generate`, { employeeId }); setMessage("Payslip generated successfully."); await loadLines(selectedRunId); } catch (err: unknown) { setMessage((err as Error).message || "Failed to generate payslip."); } finally { setGeneratingFor(null); } };
  ...
  const riskCount = lines.filter((l) => lineRisks(l).length > 0).length;
  const netMismatchCount = lines.filter((l) => Math.abs(expectedNet(l) - Number(l.net_pay ?? 0)) > 1).length;
  ...
  return <DashboardLayout><div className="space-y-6"><div className="rounded-lg border bg-card p-4 space-y-4"><div><h3 className="font-semibold text-base">My Salary Structure</h3><p className="text-xs text-muted-foreground mt-1">Annual CTC breakdown</p></div><div><h4 className="font-medium text-sm mb-2">Employee Earnings</h4><table className="w-full text-xs"><tbody><tr className="border-b"><td className="py-1.5 text-left">Basic Salary</td><td className="py-1.5 text-right">₹{fmt(30000)}</td><td className="py-1.5 text-right text-muted-foreground">40%</td></tr><tr className="border-b"><td className="py-1.5 text-left">House Rent Allowance</td><td className="py-1.5 text-right">₹{fmt(15000)}</td><td className="py-1.5 text-right text-muted-foreground">20%</td></tr><tr className="border-b"><td className="py-1.5 text-left">Conveyance</td><td className="py-1.5 text-right">₹{fmt(3750)}</td><td className="py-1.5 text-right text-muted-foreground">5%</td></tr><tr className="border-b"><td className="py-1.5 text-left">Medical</td><td className="py-1.5 text-right">₹{fmt(1500)}</td><td className="py-1.5 text-right text-muted-foreground">2%</td></tr><tr className="font-medium bg-muted"><td className="py-2 pl-2">Total Earnings</td><td className="py-2 text-right">₹{fmt(50250)}</td><td className="py-2 text-right">100%</td></tr></tbody></table></div><div><h4 className="font-medium text-sm mb-2">Employer Contribution</h4><table className="w-full text-xs"><tbody><tr className="border-b"><td className="py-1.5 text-left">PF Employer</td><td className="py-1.5 text-right">₹{fmt(6030)}</td><td className="py-1.5 text-right text-muted-foreground">12%</td></tr><tr className="border-b"><td className="py-1.5 text-left">ESI Employer</td><td className="py-1.5 text-right">₹{fmt(1639)}</td><td className="py-1.5 text-right text-muted-foreground">3.25%</td></tr><tr className="font-medium bg-muted"><td className="py-2 pl-2">Total Employer</td><td className="py-2 text-right">₹{fmt(7669)}</td><td className="py-2 text-right">15.25%</td></tr></tbody></table></div><div className="rounded-lg bg-blue-50 border border-blue-200 p-3 space-y-1.5"><div className="flex justify-between items-center"><span className="font-semibold text-sm">Total Annual CTC</span><span className="font-bold text-lg">₹{fmt(603180)}</span></div><div className="flex justify-between items-center text-sm text-muted-foreground"><span>Monthly Equivalent</span><span className="font-medium">₹{fmt(50265)}</span></div></div></div><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm font-black uppercase tracking-[0.2em] text-blue-600">Payroll</p><h1 className="mt-2 text-3xl font-black text-slate-950">Payslip Center</h1><p className="mt-2 max-w-4xl text-slate-600">Generate, validate, view and distribute payslips with payroll risk checks.</p></div><button onClick={() => { void loadRuns(); if (selectedRunId) void loadLines(selectedRunId); }} disabled={loadingRuns} className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition-colors hover:bg-slate-50 disabled:opacity-50"><RefreshCcw className="h-4 w-4" />Refresh</button></div><RoleInsightsPanel roles={roleKeys} title="Payslip control insights" />{message && <div className={`flex items-center justify-between gap-3 rounded-2xl border p-4 text-sm font-bold ${message.includes("Failed") || message.includes("Error") ? "border-red-200 bg-red-50 text-red-800" : "border-blue-200 bg-blue-50 text-blue-800"}`}><div className="flex items-center gap-3"><AlertTriangle className="h-4 w-4 flex-shrink-0" />{message}</div></div>}
