...
const metricIcons: Record<string, React.ReactNode> = {
  ...
  DIALS: <PhoneCall className="h-5 w-5" />,
  AHT: <Clock3 className="h-5 w-5" />,
  TALK_TIME: <Clock3 className="h-5 w-5" />,
  ACW: <Clock3 className="h-5 w-5" />,
...
export function PerformanceMetricGrid({
  ...
  const items: Array<PerformanceMetric | { metricCode: string; label: string }> = loading
    ? Array.from({ length: 8 }, (_, index) => ({ metricCode: `loading-${index}`, label: "Loading metric" }))
    : metrics;
    ...
    <section aria-label="Performance scorecard" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {items.map((item) => {
        ...
        const target = !loading && metric.target !== null
          ...
          : "No approved target";
        const weight = !loading && Number(metric.weightage) > 0 ? ` · Weight ${metric.weightage}%` : "";
        ...
        return (
          <KpiCard
            ...
            loading={loading}
            icon={metricIcons[item.metricCode] ?? <Activity className="h-5 w-5" />}
            tone={loading ? "default" : tone(metric)}
            description={`${target}${weight} · ${verification}`}
