...
/** The roles that open TPZ Process today. Unchanged: they keep the role list AND the OPERATIONS_DASHBOARD page permission. */
const TPZ_ROLES = ["super_admin", "admin", "ceo", "coo", "manager", "process_manager", "operations_manager", "branch_head", "qa", "quality_analyst", "tq_head"] as const;
...
export function TpzRoute({ children }: { children: ReactNode }) {
  ...
  return (
    <ProtectedRoute roles={TPZ_ROLES}>
      <WorkforcePageGate pageCode="OPERATIONS_DASHBOARD">{children}</WorkforcePageGate>
    </ProtectedRoute>
  );
}
