...
import { CloviaInboundOverview } from "./CloviaInboundOverview";
...
/**
 * Clovia INBOUND slide. Five views, nothing removed:
 *   Overview          branded landing dashboard -- real KPIs/trend/highlights from the two views below, with click-to-expand section tiles
...
export type InboundView = "overview" | "calls" | "addons" | "mis" | "classic";
const VIEWS: Array<{ key: InboundView; label: string }> = [
  { key: "overview", label: "Overview" }, { key: "calls", label: "Call performance (live dialer)" }, { key: "addons", label: "CSAT, Quality, Rechurn & Tickets" },
...
export function CloviaInboundSlide({ from, to, onRangeChange, classic }: { from: string; to: string; onRangeChange: (from: string, to: string) => void; classic: ReactNode }) {
  const [view, setView] = useState<InboundView>("overview");
  return (
    <div className="space-y-5">
      ...
      {view === "overview" && <CloviaInboundOverview from={from} to={to} />}
