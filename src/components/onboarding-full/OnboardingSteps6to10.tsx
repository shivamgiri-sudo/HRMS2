  ...
  return (
    <div>
      ...
      {opts
        ...
        : <Input type={type} inputMode={mode as any} value={value || ""} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} />
      }
    </div>
  );
}
...
export function Step8Education({
  ...
  return (
    <Card>
      <CardHeader><CardTitle>Education / Qualification</CardTitle></CardHeader>
      <CardContent>
        ...
        <div className="mt-4 flex gap-3">
          <Button onClick={onAdd} disabled={saving || !qual.qualification} className="gap-2">
            {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />} Add Qualification
          </Button>
          <Button variant="ghost" onClick={() => setQual(EMPTY_QUAL)} disabled={saving}>Clear</Button>
        </div>

        <SectionHead>Added Qualifications</SectionHead>
        <div className="space-y-2">
          {!status?.qualifications.length && <p className="text-sm text-slate-500">No qualifications added yet.</p>}
          {status?.qualifications.map((q) => (
            <div key={q.id} className="flex items-center gap-3 rounded-xl border bg-slate-50 px-4 py-3 text-sm">
              <div className="flex-1">
                <span className="font-bold">{q.qualification}</span>
                {q.specialization_course_name && <span className="text-slate-500"> · {q.specialization_course_name}</span>}
                {q.institution_name && <span className="text-slate-500"> · {q.institution_name}</span>}
                {q.passed_out_year && <span className="text-slate-500"> · {q.passed_out_year}</span>}
                {q.passed_out_percentage && <span className="text-slate-500"> · {q.passed_out_percentage}%</span>}
              </div>
            </div>
          ))}
        </div>
...
export function Step9ExperienceLang({
  ...
  return (
    <Card>
      <CardHeader><CardTitle>Experience, Family & Language</CardTitle></CardHeader>
      <CardContent>
        ...
        <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          ...
          {!isFresher && <>
            ...
            <div className="md:col-span-3">
              ...
              <Input value={experience.reasonForLeaving} onChange={(e) => updExp("reasonForLeaving", e.target.value)} />
            </div>
          </>}
        </div>
        ...
        <div className="mb-3 flex flex-wrap gap-2">
          {LANGUAGES_COMMON.map((l) => (
            <button key={l} type="button"
              ...
              className={`rounded-full border px-3 py-1 text-xs font-medium transition-colors ${newLang.language_name === l ? "bg-slate-900 text-white border-slate-900" : "border-slate-300 hover:border-slate-600"}`}>
              {l}
            </button>
          ))}
        </div>
        <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-5 items-end">
          ...
          <div>
            ...
            <div className="flex gap-3 mt-1">
              {(["can_read", "can_write", "can_speak"] as const).map((sk) => (
                <label key={sk} className="flex items-center gap-1 text-xs cursor-pointer select-none">
                  <input type="checkbox" checked={newLang[sk]} onChange={() => setNewLang((p) => ({ ...p, [sk]: !p[sk] }))} />
                  {sk.replace("can_", "")}
                </label>
              ))}
            </div>
          ...
          <div className="flex items-end">
            <Button size="sm" onClick={addLanguage} disabled={!newLang.language_name.trim()} className="gap-1">
          ...
          <div className="mt-3 overflow-x-auto rounded-xl border">
            <table className="w-full text-sm">
              <thead className="bg-slate-50">
                <tr>{["Language", "Read", "Write", "Speak", "Level", ""].map((h) => <th key={h} className="p-2 text-left text-[11px] font-semibold uppercase">{h}</th>)}</tr>
              </thead>
              <tbody>
                {languages.map((l) => (
                  <tr key={l.id} className="border-t">
                    <td className="p-2 font-medium">{l.language_name}</td>
                    {(["can_read", "can_write", "can_speak"] as const).map((f) => (
                      <td key={f} className="p-2">
                        <input type="checkbox" checked={l[f]} onChange={() => toggleLangBool(l.id, f)} />
                      </td>
                    ))}
                    <td className="p-2 capitalize">{l.proficiency}</td>
                    <td className="p-2">
                      <button type="button" onClick={() => removeLanguage(l.id)}><Trash2 className="h-3.5 w-3.5 text-red-400" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="mt-5 flex justify-end">
          <Button onClick={onSave} disabled={saving}>
            {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : "Save Experience & Language"}
          </Button>
        </div>
      </CardContent>
    </Card>
...
export function Step10Statutory({
  ...
  return (
    <Card>
      <CardHeader><CardTitle>Statutory Declaration & Submit</CardTitle></CardHeader>
      <CardContent className="space-y-6">
        ...
        {otpVerified ? (
          <p className="rounded-xl bg-emerald-50 border border-emerald-200 px-4 py-3 text-sm font-bold text-emerald-700">
            ✓ Mobile {employee.mobileNumber} verified
          </p>
        ) : (
          <div className="flex flex-wrap gap-3 items-end">
            <div>
              <p className="text-sm text-slate-600 mb-2">An OTP will be sent to <strong>{employee.mobileNumber || "your registered mobile"}</strong></p>
              <Button variant="outline" onClick={onSendOtp} disabled={saving || !employee.mobileNumber}>
            ...
            {otpSent && (
              <div className="flex gap-2 items-end">
                <div>
                  ...
                  <Input value={otpCode} onChange={(e) => setOtpCode(e.target.value)} maxLength={6} inputMode="numeric" className="w-32" placeholder="6-digit OTP" />
                </div>
                <Button onClick={onVerifyOtp} disabled={saving || otpCode.length !== 6}>Verify</Button>
              </div>
            )}
          </div>
        )}
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          <input type="checkbox" checked={statutory.declarationAccepted} onChange={(e) => updS("declarationAccepted", e.target.checked)} className="mt-1 h-4 w-4 flex-shrink-0" />
          <span className="text-sm text-slate-700">
            ...
            I understand that any misrepresentation or omission will result in rejection or termination of employment.
          </span>
        </label>
        ...
        <div className="flex flex-wrap gap-3 pt-2">
          <Button variant="outline" onClick={onSave} disabled={saving}>Save Statutory</Button>
          <Button
            onClick={onSubmit}
