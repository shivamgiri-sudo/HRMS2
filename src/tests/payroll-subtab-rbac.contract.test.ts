/**
 * Guards the two ways payroll sub-tab RBAC silently stopped working.
 *
 ...
 * five payroll screens were merged into tabbed pages, N page codes collapsed into 1 and the
 * folded screens' codes kept their grants while nothing consulted them any more — a single
 * PAYROLL_BANK_READINESS grant opened disbursal, the payment-file export, CSV upload and
 * manual payment entry.
 *
 * Two ratchets here, both of which the merges would have tripped:
 *
 ...
 *
 *   2. A payroll route must not carry a roles={[...]} list that ProtectedRoute will ignore.
...
const MERGED_PAGE_TABS: Record<string, Record<string, string>> = {
  "src/pages/payroll/PaymentDisbursalCenter.tsx": {
    bank: "PAYROLL_BANK_READINESS",
    disbursal: "PAYROLL_DISBURSAL",
  },
