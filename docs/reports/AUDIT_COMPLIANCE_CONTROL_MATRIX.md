...
| 22 | EXIT CLEARANCE INCOMPLETE | `exit_clearance_checklist` (011_exit_management.sql) | YES — table exists with `status` column | Rows where any department entry in `exit_clearance_checklist` has `status != 'cleared'` for an employee whose exit is confirmed. Control result: FAIL when clearance incomplete at exit date. | Status enum values must be confirmed; migration shows `status VARCHAR(50) NOT NULL DEFAULT 'pending'`. |
