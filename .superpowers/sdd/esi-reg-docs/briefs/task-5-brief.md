# Task 5 Brief: Frontend — EsiRegDocsTab Component

## Context
Task 5 of 6. Create a single new file: `src/pages/payroll/EsiRegDocsTab.tsx`. This is the full ESI Registration Documents tab UI — KPI strip, employee table with readiness chips, bulk-select, drill-down drawer.

## What to build

Create `src/pages/payroll/EsiRegDocsTab.tsx` with exactly the following content:
...
cd c:/Users/ADMIN/Desktop/HRMS2-latest && npx tsc --noEmit 2>&1 | grep EsiReg
...
## Global constraints
- Export name: `export default function EsiRegDocsTab()`
- No props on the default export
- All 4 API calls use `hrmsApi` (not fetch)
- Drawer: `max-w-2xl`, slide from right, purple gradient header
- Design tokens: purple gradient header, GlassCard table, tone system for KPI
- Stage ONLY `src/pages/payroll/EsiRegDocsTab.tsx`

## Report file
Write to: `c:/Users/ADMIN/Desktop/HRMS2-latest/.superpowers/sdd/esi-reg-docs/briefs/task-5-report.md`
Return only: DONE/BLOCKED, commit SHA, tsc result, concerns.
