...
## Context
Task 3 of 7. The AI Command Bar. Tasks 1-2 are complete:
- `src/hooks/useAmbientInsights.ts` — exports `useAmbientInsights`, `AmbientChip`
- `src/components/ai/AmbientStrip.tsx` — named export `AmbientStrip`

## Your job
Create ONE new file: `src/components/ai/CommandPalette.tsx`

This is the full-screen command palette opened by ⌘K. It uses `cmdk` (already installed as `"cmdk": "^1.1.1"` in package.json).

## Three input modes
- **No prefix** → AI mode: calls `POST /api/ai/ask`, renders answer inline
- **`/` prefix** → nav mode: filters `navGroups` from navConfig
- **`@` prefix** → employee search: calls `GET /api/employees?search=&limit=8`
...
## Global Constraints
- Named export `CommandPalette` only (no default export)
- No `any` — all types explicit
- `cmdk` import: `import { Command } from "cmdk"` — this is correct for cmdk v1.x
- `navGroups` imported from `@/components/layout/navConfig` — this is a named export already in the file
- Employee navigate: `/employees/${emp.id}` (not `/profile`)
- AI ask on Enter key only (not on every keystroke)
- Clicking backdrop (`aria-hidden` div) calls `onClose`
- `z-50` for dialog (above `z-40` ambient strip)
- `max-w-[640px]` panel, `pt-[10vh]` from top

## Important: cmdk v1 API
In cmdk v1.x the import is `import { Command } from "cmdk"` and the sub-components are:
- `Command` — root container
- `Command.Input` — text input
- `Command.List` — scrollable list
- `Command.Group` — group with heading
- `Command.Item` — individual item with `onSelect`
- `Command.Empty` — shown when group has no results
- `shouldFilter={false}` prop on `Command` disables built-in filtering (we filter manually)

## Steps
1. Write the file exactly as specified
2. Run `cd c:/Users/ADMIN/Desktop/HRMS2-latest && npx tsc --noEmit` — fix TypeScript errors
3. Commit: `git add src/components/ai/CommandPalette.tsx && git commit -m "feat(ai): CommandPalette — cmdk-powered AI + nav + employee search overlay"`

## Report file
`c:\Users\ADMIN\Desktop\HRMS2-latest\.superpowers\sdd\ai-cmd-task-3-report.md`

Return only: status, commit hash, TypeScript result, concerns.
