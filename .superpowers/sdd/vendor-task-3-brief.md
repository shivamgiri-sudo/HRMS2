# Task Brief: Vendor Task 3 — Create VendorSheet Component

## Context
MAS PeopleOS HRMS compact UI redesign. Task 3 of 4 in Vendor Payment module.
Working directory: `C:\Users\ADMIN\Desktop\HRMS2-latest`

## Goal
Create a new file: `src/components/finance/vendor/VendorSheet.tsx`

This is a self-contained unified Sheet for create/edit/detail vendor operations.
...
Note: The `Select` import is included but not used in the implementation above — the vendor_type and payment_terms fields use plain `Input`. That's fine — remove the unused `Select` import to keep tsc clean.

## Steps
1. Check if `src/components/finance/vendor/` directory exists (it does — Task 1 created it)
2. Write the file exactly as above, removing unused `Select` imports
3. Run `npx tsc --noEmit 2>&1 | grep -i "VendorSheet"` — fix any errors
4. Commit: `git add src/components/finance/vendor/VendorSheet.tsx && git commit -m "feat(vendor): unified VendorSheet replaces 3 separate overlays"`
5. Write report to `.superpowers/sdd/vendor-task-3-report.md`
