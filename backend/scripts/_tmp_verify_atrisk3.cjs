...
async function main() {
  ...
  const periodStartExpr = "DATE(MIN(DATE_FORMAT(e.date_of_exit, '%Y-%m-01')))";
  const periodEndExpr = "LAST_DAY(MIN(DATE_FORMAT(e.date_of_exit, '%Y-%m-01')))";
    ...
    return `(
      SELECT COUNT(*) FROM at_risk ar
       WHERE ar.join_date <= ${asOfExpr}
         ...
         AND (ar.branch_id <=> e.branch_id)
         AND (ar.process_id <=> e.process_id)
         AND (ar.cost_centre_id <=> e.cost_centre_id)
         AND ${atRiskBucketSql(asOfExpr, "ar.join_date")} = ${bucket}
    )`;
  }
  ...
  const sql = `
    ...
    SELECT DATE_FORMAT(e.date_of_exit, '%Y-%m') AS month,
           ...
           ROUND(COUNT(*) * 100.0 / NULLIF(${atRiskAvgExpr}, 0), 2) AS aon_attrition_rate_pct
      FROM employees e
     ...
     GROUP BY DATE_FORMAT(e.date_of_exit, '%Y-%m'), b.branch_name, e.branch_id, e.process_id, e.cost_centre_id, ${bucket}, ${bucketOrder}
     ORDER BY month DESC, b.branch_name, ${bucketOrder}
  `;

  const [rows] = await conn.execute(sql);
  console.log('Full query result (per bucket, branch NOIDA-2, June 2026):');
  console.table(rows);
