/**
 * Wires clovia_outbound_cdr_raw (sql/1720) into KPI Studio.
 *
 * Run with: npx tsx scripts/add-clovia-outbound-cdr-kpi-sources.ts
 */
...
async function main() {
  ...
  const source = await saveDataSource({
    source_code: "CLOVIA_OUTBOUND_CDR",
    source_name: "Clovia — Outbound CDR",
    source_type: "local_query",
    source_object: "clovia_outbound_cdr_raw",
    date_column: "report_date",
    description: "Clovia's own Outbound Report sheet (sql/1720) -- found while auditing the same workbook used for Chat Performance/CRM Disposition; no DB backing exists for this data anywhere.",
  ...
  await saveSourceField({
    ...
    filter_json: [{ column: "status", op: "eq", value: "Connected" }],
  } as never);
  await saveSourceField({ data_source_id: sourceId, field_name: "talk_seconds", source_column: "length_seconds", aggregate_fn: "SUM" });

  const totalId = await ensureMetric("CLOVIA_OUTBOUND_CDR_TOTAL_CALLS", "Clovia Outbound CDR total calls", "count", "higher_is_better");
  const connectedPctId = await ensureMetric("CLOVIA_OUTBOUND_CDR_CONNECTED_PCT", "Clovia Outbound CDR connected %", "pct", "higher_is_better");
  const achtId = await ensureMetric("CLOVIA_OUTBOUND_CDR_ACHT", "Clovia Outbound CDR average call handling time", "seconds", "lower_is_better");
  ...
  console.log("[SEED] CLOVIA_OUTBOUND_CDR wired: total calls, connected%, ACHT");
  process.exit(0);
}
