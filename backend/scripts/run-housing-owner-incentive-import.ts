/**
 * One-off: loads the real Housing Owner "Incentive" sheet (exported to
 * JSON by _export_incentive_json.py) into an upload_batch and runs the
 * real import function against mas_hrms. See
 * housing-owner-incentive-bulk.service.ts / sql/1737 for what this is.
 *
 * Run with: npx tsx scripts/run-housing-owner-incentive-import.ts
 */
...
import { importHousingOwnerIncentiveBatch } from "../src/modules/bulk-upload/housing-owner-incentive-bulk.service.js";
...
async function main() {
  ...
  const result = await importHousingOwnerIncentiveBatch(batchId, USER_ID);
