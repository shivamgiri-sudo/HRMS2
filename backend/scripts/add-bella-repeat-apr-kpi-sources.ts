/**
 * Wires bella_repeat_apr_daily_actual (sql/1724) into KPI Studio.
 *
 * Run with: npx tsx scripts/add-bella-repeat-apr-kpi-sources.ts
 */
...
async function main() {
  ...
  const source = await saveDataSource({
    source_code: "BELLA_REPEAT_APR_DAILY",
    source_name: "Bella Vita Repeat LOB — APR (Daily)",
    source_type: "local_query",
    source_object: "bella_repeat_apr_daily_actual",
    date_column: "call_date",
    description: "Bella Vita Repeat LOB's own APR sheet (sql/1724) -- found while auditing the same dashboard used for the Repeat LOB sales fields; no DB backing exists for this data anywhere.",
  ...
  const callsId = await ensureMetric("BELLA_REPEAT_APR_TOTAL_CALLS", "Bella Repeat LOB APR total calls", "count", "higher_is_better");
  const achtId = await ensureMetric("BELLA_REPEAT_APR_ACHT", "Bella Repeat LOB APR average call handling time", "seconds", "lower_is_better");
  const utilId = await ensureMetric("BELLA_REPEAT_APR_UTILIZATION_PCT", "Bella Repeat LOB APR login utilization", "pct", "higher_is_better");
  ...
  console.log("[SEED] BELLA_REPEAT_APR_DAILY wired: total calls, ACHT, login utilization%");
  process.exit(0);
}
