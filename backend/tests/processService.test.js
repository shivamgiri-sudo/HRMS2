...
const mockGetRepo = getProcessRepository;
const fakeProcess = {
    id: "proc-1",
    process_code: "IB",
...
describe("processService.create", () => {
    ...
    it("creates and returns new process when code is unique", async () => {
        ...
        const result = await processService.create({ processCode: "IB", processName: "Inbound" }, "user-1");
        expect(result).toEqual(fakeProcess);
    ...
    it("throws when process code already exists (case-insensitive)", async () => {
        ...
        await expect(processService.create({ processCode: "ib", processName: "Inbound 2" }, "user-1")).rejects.toThrow("Process code already exists");
...
describe("processService.update", () => {
    ...
    it("throws when process does not exist", async () => {
        mockRepo.getById.mockResolvedValueOnce(null);
        await expect(processService.update("missing", { processName: "X" }, "user-1")).rejects.toThrow("Process not found");
...
describe("processService.updateStatus", () => {
    ...
    it("throws when process does not exist", async () => {
        mockRepo.getById.mockResolvedValueOnce(null);
        await expect(processService.updateStatus("missing", true, "user-1")).rejects.toThrow("Process not found");
    });
});
