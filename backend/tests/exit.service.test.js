...
describe("exitService.createExitRequest", () => {
    ...
    it("inserts exit request and returns it", async () => {
        ...
        const result = await exitService.createExitRequest({ employeeId: "emp-1", exitDate: "2026-06-30", exitType: "voluntary", reason: "Better opportunity" }, "user-1");
    ...
    it("passes null reason when not provided", async () => {
        ...
        const result = await exitService.createExitRequest({ employeeId: "emp-2", exitDate: "2026-07-31", exitType: "voluntary" }, "user-2");
...
describe("exitService.updateExitStatus", () => {
    ...
    it("throws when exit request not found", async () => {
        ...
        await expect(exitService.updateExitStatus("missing", "submitted", "Remarks", "user-1")).rejects.toThrow("Exit request not found");
    });
});
// ─── getExitStats ─────────────────────────────────────────────────────────────
