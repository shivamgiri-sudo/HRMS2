...
describe("integrationService.getByKey", () => {
    ...
    it("throws when not found", async () => {
        ...
        await expect(integrationService.getByKey("no_such_key")).rejects.toThrow("Integration not found");
    });
});
describe("integrationService.create", () => {
    ...
    it("throws when integration_key already exists", async () => {
        ...
        await expect(integrationService.create({ integrationKey: "dialer_1", integrationName: "Dup", integrationType: "rest_pull" }, "user-1")).rejects.toThrow("Integration key already exists");
    });
    it("creates and returns new integration", async () => {
        ...
        const result = await integrationService.create({ integrationKey: "dialer_3", integrationName: "Dialer 3", integrationType: "rest_pull" }, "user-1");
...
describe("integrationService.update", () => {
    ...
    it("throws when integration not found", async () => {
        mockExecute.mockResolvedValueOnce([[], []]);
        await expect(integrationService.update("missing_key", { integrationName: "X" }, "user-1")).rejects.toThrow("Integration not found");
    });
