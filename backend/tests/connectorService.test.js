...
const mockGetByKey = integrationService.getByKey;
const mockListFieldMaps = integrationService.listFieldMaps;
const mockCreateRun = integrationService.createRun;
const mockAnalyze = analyzeSchema;
const mockPromote = promoteRows;
const fakeConfig = {
    id: "cfg-1",
...
describe("runConnector", () => {
    ...
    it("stores raw payload in integration_raw_payload", async () => {
        ...
        expect(calls.some((s) => /integration_raw_payload/i.test(s))).toBe(true);
    });
    it("stores schema snapshot", async () => {
        ...
        await runConnector("dialer_1", rawPayload, "user-1");
        const calls = mockExecute.mock.calls.map(([sql]) => sql);
        expect(calls.some((s) => /integration_schema_snapshot/i.test(s))).toBe(true);
    });
    it("generates suggestions for unmapped fields", async () => {
        ...
        await runConnector("dialer_1", rawPayload, "user-1");
        const calls = mockExecute.mock.calls.map(([sql]) => sql);
        expect(calls.some((s) => /integration_field_map_suggestion/i.test(s))).toBe(true);
    });
    ...
    it("updates run to complete with row counts", async () => {
        ...
        await runConnector("dialer_1", rawPayload, "user-1");
        const calls = mockExecute.mock.calls.map(([sql]) => sql);
        const updateCall = calls.find((s) => /UPDATE.*integration_connector_run/i.test(s));
    ...
    it("marks run as failed when promoteRows throws", async () => {
        ...
        await runConnector("dialer_1", rawPayload, "user-1");
        const calls = mockExecute.mock.calls.map(([sql]) => sql);
        const failUpdate = calls.find((s) => /UPDATE.*integration_connector_run/i.test(s));
