...
import { createProcessSchema, processFiltersSchema, updateProcessSchema, updateProcessStatusSchema, } from "../src/modules/process/process.validation.js";
...
describe("createProcessSchema", () => {
    ...
    it("rejects processCode longer than 80 characters", () => {
        expect(() => createProcessSchema.parse({ ...valid, processCode: "A".repeat(81) })).toThrow();
    });
    it("rejects non-UUID departmentId", () => {
        expect(() => createProcessSchema.parse({ ...valid, departmentId: "bad-id" })).toThrow();
    });
