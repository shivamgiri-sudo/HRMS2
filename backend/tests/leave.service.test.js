...
describe("leaveService.createLeaveType", () => {
    it("throws when code already exists", async () => {
        exec.mockResolvedValueOnce([[fakeType], []]);
        await expect(leaveService.createLeaveType({ leaveCode: "CL", leaveName: "Casual", maxDaysPerYear: 12, carryForward: false, requiresApproval: true, paidLeave: true })).rejects.toThrow("Leave code already exists");
    });
...
describe("leaveService.reviewRequest", () => {
    it("throws when request not found", async () => {
        exec.mockResolvedValueOnce([[], []]);
        await expect(leaveService.reviewRequest("nope", { status: "approved" }, "mgr-1")).rejects.toThrow("Leave request not found");
    });
