...
describe("wfmService.createShift", () => {
    it("throws when shift_code already exists", async () => {
        exec.mockResolvedValueOnce([[fakeShift], []]);
        await expect(wfmService.createShift({ shiftCode: "GEN", shiftName: "General", startTime: "09:00", endTime: "18:00", requiredMinutes: 540 }, "user-1")).rejects.toThrow("Shift code already exists");
    });
    it("creates shift", async () => {
        ...
        const r = await wfmService.createShift({ shiftCode: "GEN", shiftName: "General", startTime: "09:00", endTime: "18:00", requiredMinutes: 540 }, "user-1");
        expect(r.shift_code).toBe("GEN");
    });
});
...
describe("wfmService.clockIn", () => {
    it("throws when session already exists for that date", async () => {
        ...
        await expect(wfmService.clockIn({ employeeId: "emp-1", sessionDate: "2026-05-21", punchSource: "MANUAL" }, "user-1")).rejects.toThrow("Session already exists");
    });
    it("creates session and returns it", async () => {
        ...
        const r = await wfmService.clockIn({ employeeId: "emp-1", sessionDate: "2026-05-21", punchSource: "MANUAL" }, "user-1");
...
describe("wfmService.submitRegularization", () => {
    it("creates regularization request", async () => {
        ...
        const r = await wfmService.submitRegularization({ employeeId: "emp-1", sessionDate: "2026-05-20", reason: "Was present" }, "emp-1");
...
describe("wfmService.reviewRegularization", () => {
    it("throws when not found", async () => {
        exec.mockResolvedValueOnce([[], []]);
        await expect(wfmService.reviewRegularization("nope", { status: "approved" }, "mgr-1")).rejects.toThrow("Regularization not found");
    });
