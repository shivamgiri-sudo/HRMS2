...
function nestedValue(source: Record<string, any>, sourcePath?: string | null) {
  const normalized = safeTrim(sourcePath);
  if (!normalized) return null;
  return normalized.split(".").reduce<any>((acc, key) => (acc == null ? null : acc[key]), source);
}
...
function deriveFieldValue(map: RowDataPacket, sourceContext: Record<string, any>) {
  ...
  for (const fieldMap of fieldMaps) {
    ...
    try {
      if ("setText" in field && typeof field.setText === "function") {
        (field as any).setText(textValue);
      }
