...
export const REPORT_CATALOG: ReportDefinition[] = [
...

  {
    ...
    columns: [
      ...
      { key: "date_of_joining", label: "DOJ", format: "date", width: 100 },
      { key: "joining_month", label: "Joining Month", format: "text", width: 110 },
      ...
      { key: "salary_effective_date", label: "Salary Start Date", format: "date", width: 130, sensitive: true },
  ...
  {
    code: "regularization-audit",
    name: "Regularization Audit Report",
    category: "Attendance",
    subcategory: "Exceptions",
    description: "Full audit trail for every attendance regularization request — plain regularizations and formal disputes together: the attendance status actually recorded, what it was changed to, who submitted the request and when, who approved or rejected it, when, and their remarks",
    rowGrain: "One row per regularization request",
    primaryKey: ["employee_code", "attendance_date", "requested_at"],
    columns: [
      { key: "employee_code", label: "Emp Code", format: "text", width: 100 },
      { key: "cost_centre_code", label: "Cost Centre Code", format: "text" },
      { key: "cost_centre_name", label: "Cost Centre", format: "text" },
      ...
      { key: "attendance_date", label: "Attendance Date", format: "date", width: 100 },
      { key: "dispute_type", label: "Dispute Type", format: "status", width: 140 },
      { key: "actual_attendance_status", label: "Actual Attendance (Before)", format: "status", width: 140 },
      { key: "requested_attendance_status", label: "Changed To", format: "status", width: 120 },
      ...
      { key: "payroll_impact", label: "Payroll Impact", format: "boolean", width: 100 },
      { key: "approval_status", label: "Approval Status", format: "status", width: 120 },
      { key: "requested_at", label: "Requested At", format: "datetime", width: 140 },
      { key: "approver_name", label: "Approver Name", format: "text", width: 140 },
      { key: "approved_at", label: "Approved At", format: "datetime", width: 140 },
    ...
    exportRoles: ["super_admin", "admin", "hr", "hr_head", "wfm"],
    sourceTables: ["attendance_regularization", "attendance_reason_master", "employees"],
    calculationNotes: "Covers both plain regularizations and formal disputes (no dispute_type restriction), unlike attendance-dispute-summary. Approver name/time/remarks reflect the latest review stage only — attendance_regularization stores a single reviewed_by/reviewed_at/reviewer_note that is overwritten at each approval stage, not a full per-stage history. When a manager submits on an employee's behalf (requested_by_type = 'manager'), the exact submitter is not captured on this table and is not shown here.",
    branchScoped: true,
    processScoped: true,
  },

  {
    code: "attendance-direct-edit-log",
    name: "Attendance Direct Edit Log",
    category: "Attendance",
    subcategory: "Exceptions",
    description: "All direct attendance status changes made via the Attendance Lookup 'Change Status' dialog by Payroll Head or Super Admin — these are admin-level direct edits that bypass the regularization approval workflow and write to attendance_manual_override, not attendance_regularization. Use the Regularization Audit Report for employee-submitted regularization requests.",
    rowGrain: "One row per manual override",
    primaryKey: ["employee_code", "attendance_date", "submitted_at"],
    columns: [
      { key: "employee_code",        label: "Emp Code",          format: "text",     width: 100 },
      { key: "employee_name",        label: "Employee Name",     format: "text",     width: 180 },
      ...
      { key: "department_name",      label: "Department",        format: "text",     width: 120 },
      { key: "designation_name",     label: "Designation",       format: "text",     width: 140 },
      { key: "attendance_date",      label: "Attendance Date",   format: "date",     width: 110 },
      { key: "status_before",        label: "Status Before",     format: "status",   width: 130 },
      { key: "status_after",         label: "Changed To",        format: "status",   width: 130 },
      { key: "lwp_before",           label: "LWP Before",        format: "number",   width: 90  },
      { key: "lwp_after",            label: "LWP After",         format: "number",   width: 90  },
      { key: "reason",               label: "Reason",            format: "text",     width: 220 },
      { key: "approval_status",      label: "Status",            format: "status",   width: 110 },
      { key: "payroll_month",        label: "Payroll Month",     format: "text",     width: 100 },
      { key: "payroll_month_locked", label: "Month Locked",      format: "boolean",  width: 100 },
      { key: "submitted_at",         label: "Submitted At",      format: "datetime", width: 140 },
      { key: "submitted_by",         label: "Submitted By",      format: "text",     width: 160 },
      { key: "approved_at",          label: "Approved At",       format: "datetime", width: 140 },
      { key: "approved_by_name",     label: "Approved By",       format: "text",     width: 160 },
      { key: "rejected_at",          label: "Rejected At",       format: "datetime", width: 140 },
    ...
    viewRoles: ["super_admin", "admin", "hr", "hr_head", "payroll_head", "payroll_admin", "wfm"],
    exportRoles: ["super_admin", "admin", "hr", "hr_head", "payroll_head"],
    sourceTables: ["attendance_manual_override", "employees", "auth_user"],
    calculationNotes: "Filtered by submitted date (created_at), not attendance date. 'Submitted By' joins auth_user on created_by (the user_id of whoever clicked Change Status in Attendance Lookup). All manual overrides — pending, approved, rejected — are included.",
    branchScoped: true,
    processScoped: true,
  },
...

  {
    ...
    columns: [
      ...
      { key: "email_id", label: "Email ID", format: "text", width: 180, sensitive: true },
      { key: "gender", label: "Gender", format: "text", width: 80 },
      { key: "pf_eligibility", label: "PF Eligibility", format: "text", width: 100 },
      { key: "esi_eligibility", label: "ESI Eligibility", format: "text", width: 100 },
      { key: "reporting_manager", label: "Reporting Manager", format: "text", width: 160 },
      ...
      { key: "net_in_hand", label: "Net In Hand (Inhand)", format: "currency", width: 140, align: "right", sensitive: true },
      { key: "offered_ctc", label: "CTC", format: "currency", width: 120, align: "right", sensitive: true },
    ],
    filters: [F_DATE_FROM, F_DATE_TO, F_BRANCH],
    viewRoles: ROLES_HR_MANAGER,
    exportRoles: ROLES_HR_ADMIN,
  ...
  {
    code: "attendance-source-sheet",
    name: "Attendance Source Sheet (Cosec vs APR)",
    category: "Attendance",
    subcategory: "Register",
    description: "Per employee for the month: payroll status of every day next to BOTH the COSEC (biometric) and APR (dialler) duration, with the payroll source flagged. Download gives the colour-coded Status / Cosec / APR sheet",
    rowGrain: "One row per employee per month",
    primaryKey: ["employee_code"],
    columns: [
      { key: "employee_code", label: "Emp Code", format: "text", width: 100 },
      ...
      { key: "present_days", label: "Present Days", format: "number", width: 90, align: "right" },
      { key: "half_days", label: "Half Days", format: "number", width: 90, align: "right" },
      { key: "absent_days", label: "Absent Days", format: "number", width: 90, align: "right" },
      { key: "missing_punch_days", label: "Missing Punch Days", format: "number", width: 110, align: "right" },
      { key: "cosec_hours", label: "Cosec Hours", format: "number", width: 100, align: "right" },
      { key: "apr_hours", label: "APR Hours", format: "number", width: 100, align: "right" },
    ],
    filters: [F_MONTH, F_BRANCH, F_PROCESS],
    viewRoles: ROLES_WFM,
    exportRoles: ["super_admin", "admin", "hr", "hr_head", "wfm", "branch_wfm", "payroll", "payroll_head"],
    sourceTables: ["attendance_daily_record", "employees"],
    calculationNotes: "Status, COSEC (biometric_minutes) and APR (dialler_minutes) come straight from attendance_daily_record. The payroll source is attendance_daily_record.attendance_source: the duration that decided that day's status.",
    branchScoped: true,
    processScoped: true,
    sensitivityLevel: "confidential",
    containsPII: true,
