...
async function payroll(code:string,filters:VerifiedAdapterFilters,scope:BranchScope){
  ...
  // PROFESSIONAL_TAX below still maps to salary_prep_line.professional_tax, which is
  // hardcoded to 0 by payrollCalculate.service.ts (resolveProfessionalTax) as of
  // 2026-09-11 — PT removed from active payroll company-wide by explicit stakeholder
  // decision. Left mapped rather than deleted: this is a generic field resolver shared
  // by every report code, and the column stays valid (always 0) for historical rows.
