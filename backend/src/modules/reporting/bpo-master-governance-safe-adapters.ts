...
async function journeyEvents(): Promise<JourneyEvent[]> {
  ...
  const clearance = await sourceTableSql("exit_clearance_checklist");
  const clearanceColumns = await sourceColumns("exit_clearance_checklist");
  if (clearance && exitRequest && has(clearanceColumns,"id") && has(clearanceColumns,"exit_request_id") && has(clearanceColumns,"created_at")) {
    ...
    output.push({ source: "exit_clearance_checklist", sql: `${journeySelect({ source_schema:"'mas_hrms'",source_table:"'exit_clearance_checklist'",source_record_id:`xc.${quote("id")}`,activity_ts:ts,employee_id:`xr.${quote("employee_id")}`,approver_id:has(clearanceColumns,"assigned_to")?`xc.${quote("assigned_to")}`:"NULL",journey_phase:"'EXIT CLEARANCE'",journey_stage:has(clearanceColumns,"department")?`xc.${quote("department")}`:"'CLEARANCE'",activity_type:"'CLEARANCE_ACTIVITY'",activity_status:has(clearanceColumns,"status")?`xc.${quote("status")}`:"NULL",activity_description:has(clearanceColumns,"remarks")?`xc.${quote("remarks")}`:"'Exit clearance activity'",remarks:has(clearanceColumns,"remarks")?`xc.${quote("remarks")}`:"NULL",exit_request_id:`xc.${quote("exit_request_id")}`,exit_stage:"'CLEARANCE'",clearance_department:has(clearanceColumns,"department")?`xc.${quote("department")}`:"NULL",clearance_status:has(clearanceColumns,"status")?`xc.${quote("status")}`:"NULL",evidence_reference:`xc.${quote("id")}` })} FROM ${clearance} xc JOIN ${exitRequest} xr ON xr.${quote("id")}=xc.${quote("exit_request_id")}` });
  }
  return output;
}
...
const SOURCE_CONTRACTS: Record<string, string[]> = {
  ...
  "bpo-interview-to-exit-journey-ledger": ["ats_candidate","ats_candidate_stage_log","ats_offer_letters","ats_onboarding_bridge","candidate_bgv_check","employees","employee_lifecycle_event","leave_request","attendance_regularization","coaching_session","pip_record","salary_prep_line","exit_request","exit_approval_log","exit_clearance_checklist"],
};
