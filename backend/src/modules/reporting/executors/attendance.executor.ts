...
export async function attendanceRegisterMonthly(
  ...
  //
  // The exit-date guard distinguishes two populations that both have active_status=0
  // and no date_of_exit:
  //   - Employees incorrectly marked inactive (bulk-import issue): they have attendance
  //     records generated by the biometric/import pipeline in the current month.
  //   - Genuinely gone ex-employees whose exit date was never backfilled: they do not
  //     have current-month attendance records being generated for them.
  //
  // The positive-status guard was removed because it excluded employees whose only
  // records were absent/missing_punch — the most common case for employees whose
  // attendance data has not yet synced or who were absent all month. The
  // date_of_exit guard (date_of_exit IS NULL OR date_of_exit >= firstDay) is now
  // the primary defence against ex-employees re-appearing.
  //
  // Employees with a date_of_exit that falls within the period are still included so
  // their partial-month attendance (days 1..exit_date) is preserved; post-exit cells
  // are blanked in the pivot pass below.
  //
