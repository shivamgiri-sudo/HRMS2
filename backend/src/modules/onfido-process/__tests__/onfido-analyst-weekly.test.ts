...
describe("splitIntoWeeks", () => {
  it("splits a month into Monday-start weeks clipped to the range", () => {
    ...
    expect(weeks[0]).toEqual({ start: "2026-08-01", end: "2026-08-02", label: "WC 27 Jul" });
    expect(weeks[1]).toEqual({ start: "2026-08-03", end: "2026-08-09", label: "WC 03 Aug" });
    expect(weeks[weeks.length - 1].end).toBe("2026-08-31");
  });

  it("returns a single clipped week for a one-day range", () => {
    expect(splitIntoWeeks("2026-08-05", "2026-08-05")).toEqual([{ start: "2026-08-05", end: "2026-08-05", label: "WC 03 Aug" }]);
  });
