...
router.post("/heartbeat", h(async (req: AuthenticatedRequest, res: Response) => {
  ...
  // Resolve employee_id from auth user
  const [empRows] = await db.execute<RowDataPacket[]>(
    `SELECT e.id, e.full_name, bm.branch_name
     FROM employees e
  ...
  if (!empRows.length) {
    // User has no employee record — still accept silently (non-employee users)
    return res.json({ success: true });
  }

  const emp = empRows[0] as { id: string; full_name: string; branch_name: string | null };

  await db.execute(
    `INSERT INTO employee_live_location (employee_id, latitude, longitude, accuracy, captured_at, full_name, branch_name)
     VALUES (?, ?, ?, ?, NOW(), ?, ?)
    ...
    [emp.id, latitude, longitude, accuracy ?? null, emp.full_name, emp.branch_name ?? null],
  );
