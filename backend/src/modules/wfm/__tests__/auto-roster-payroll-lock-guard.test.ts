 ...
 * check (roster-lock-guard.ts), already wired into manual assignment, the
 * PM post-publish correction endpoint, manager realignment, and shift-swap
 * apply. Source-level assertions, matching this suite's established style
 * for a function embedded in a large enclosing service rather than exported
 * standalone (see auto-roster-locked-assignment.test.ts).
 */
...
describe("generateDraft's per-employee attendance/payroll-lock guard (Area 3)", () => {
  ...
  it("checks the lock inside the per-employee assignment loop, before the row is added to assignedToday", () => {
    ...
    const loopBody = SOURCE.slice(loopStart, loopStart + 3300);
