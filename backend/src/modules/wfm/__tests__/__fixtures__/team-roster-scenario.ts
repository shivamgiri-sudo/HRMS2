...
export const lineRow = (l: LineRow) => ({
  ...
  old_shift_start_time: null, old_shift_end_time: null, new_shift_template_id: null, reason: null, ...l,
});
...
export function installBase(fake: FakeDb, s: Scenario = {}) {
  ...
  fake.on(/FROM wfm_shift_template WHERE id IN/, () => rows(s.templates ?? [TEMPLATE]));
