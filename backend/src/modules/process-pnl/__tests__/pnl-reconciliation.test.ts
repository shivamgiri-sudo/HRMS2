...
function mockDb(options: { payrollRows?: number } = {}) {
  ...
  execute.mockImplementation(async (sql: string) => {
    const q = String(sql);
    if (
      q.includes("FROM cost_centre_master ccm") &&
      q.includes("LEFT JOIN branch_master")
    ) {
      return [
        [
          {
    ...
    // Owner ruling 2026-09-26: no legacy bill is added to HRMS figures. This db_bill mirror row set
    // is a TRAP: the shared GRN reader must never query it, so none of it may reach any figure.
    ...
    if (q.includes("FROM finance_budget_header h")) {
      return [
        [{ id: "fbh-1", branch_id: "branch-noida", branch_name: "NOIDA" }],
        [],
      ];
    }
        ...
        [
          {
            bill_source_id: 1,
            budget_source_id: 9,
            expense_type_name: "BSS/IB/Noida/534",
            amount: L(999),
            branch_name: "Noida",
            branch_id: "branch-noida",
            cost_centre_id: "cc-noida-1",
    ...
    if (q.includes("COUNT(l.id) AS `rows`")) {
      return [
        [{ rows: payrollRows, latest_synced_at: "2026-08-31 10:00:00" }],
        [],
      ];
    }
    ...
    if (
      q.includes("FROM salary_prep_line l") &&
      q.includes("GROUP BY") &&
      q.includes("cost_centre_id") &&
      !q.includes("branch_id")
    ) {
    ...
    if (
      q.includes("COUNT(*) AS `rows`") &&
      q.includes("FROM pnl_running_salary_snapshot")
    ) {
    ...
    if (q.includes("cost_centre_id") && q.includes("IS NULL"))
      return [[{ count: 0, amount: 0 }], []];
    if (q.includes("COUNT(*) AS `rows`"))
      return [[{ rows: 3, latest_synced_at: "2026-08-19 09:00:00" }], []];
    return [[], []];
  });
}
...
describe("P&L reconciliation — seat-rate estimate", () => {
  function mockWithSeatLines() {
    ...
    execute.mockImplementation(async (sql: string, params?: unknown[]) => {
      ...
      if (
        q.includes("FROM billing_invoice_particular_snapshot p") &&
        q.includes("p.period_code >= ?")
      ) {
        return [
          [
            {
  ...
  it("shows margin as NA when the month has estimated revenue but no people cost at all", async () => {
    ...
    const { getPnlReconciliation } =
      await import("../pnl-reconciliation.service.js");
    const out = await getPnlReconciliation("2026-09", {
      asOfDate: "2026-09-15",
    });
...
describe("P&L reconciliation", () => {
  it("builds active cost-centre P&L from recognised revenue, GRN, budget and payroll", async () => {
    ...
    // 120 revenue - 60 payroll; the db_bill mirror's Rs 20 L GRN is NOT counted (no HRMS GRN here).
    expect(row?.operatingProfit).toBe(L(60));
    expect(out.totals.revenue).toBe(L(120));
    expect(out.branches[0].branchName).toBe("NOIDA");
    expect(out.branches[0].operatingProfit).toBe(L(60));
    // Mirror GRN spend contributes nothing anywhere.
  ...
  it("labels an open month as live when payroll is not posted", async () => {
    ...
    expect(out.blockers.join(" ")).toContain(
      "Live P&L uses accrued running salary",
    );
...
describe("P&L reconciliation — OP% scope rules (2026-09-15 OP% check)", () => {
  ...
  function withOverrides(
    overrides: (q: string) => unknown[] | undefined,
    options?: { payrollRows?: number },
  ) {
    mockDb(options);
    const base = execute.getMockImplementation()!;
    execute.mockImplementation(async (sql: string, params?: unknown[]) => {
  ...
  /** HRMS-raised consumed GRN (Rs 25 L). Without any HRMS GRN the "no IDC anywhere" rule NAs the
   *  margin, so tests that assert a margin feed the app-side reader explicitly. */
  const hrmsConsumedGrn = (q: string) =>
    q.includes("lifecycle_status = 'consumed'")
      ? [
          { cost_centre_id: "cc-noida-1", amount: L(20) },
          { cost_centre_id: "cc-noida-2", amount: L(5) },
        ]
  ...
  it("counts pay of staff with no cost centre in branch and company cost, but in no row", async () => {
    ...
    // Baseline: revenue 120, payroll 60 (rows), HRMS-raised GRN 25 (hrmsConsumedGrn) → OP 35. Plus Rs 6 L of unmapped staff.
    ...
    expect(out.branches[0]).toMatchObject({
      unallocatedPayroll: L(6),
      payrollCost: L(66),
      operatingProfit: L(29),
    });
    expect(
      out.rows.reduce((t, r) => t + r.payrollCost, 0),
      "rows keep only what is really theirs",
    ).toBe(L(60));
  ...
  it("while payroll is not posted, reads unmapped staff's ACCRUED pay from the running snapshot (audit item 13)", async () => {
    ...
    withOverrides(
      (q) => {
        if (
          q.includes("FROM pnl_running_salary_snapshot") &&
          q.includes("IS NULL") &&
          q.includes("GROUP BY s.branch_id")
        ) {
          return [
            {
              branch_id: "branch-noida",
    ...
    expect(
      out.totals.unallocatedPayroll,
      "accrued, from the snapshot — not the (absent) final run",
    ).toBe(L(4));
    ...
    const runningCall = execute.mock.calls.find(([sql]) =>
      String(sql).includes("GROUP BY s.branch_id"),
    );
    expect(
      runningCall?.[1],
      "branch filter applies to the snapshot's home branch",
    ).toEqual(["2026-08", "branch-noida"]);
  });
  ...
  it("narrows cost centres and unallocated payroll to the Client / Search processes (audit item 19)", async () => {
    ...
    const { getPnlReconciliation } =
      await import("../pnl-reconciliation.service.js");
    await getPnlReconciliation("2026-08", {
      branchIds: ["branch-noida"],
      processIds: ["p1", "p2"],
    });
    const calls = execute.mock.calls.map(([sql, params]) => ({
      sql: String(sql),
      params: (params ?? []) as unknown[],
    }));
    const ccCall = calls.find(
      (c) =>
        c.sql.includes("FROM cost_centre_master ccm") &&
        c.sql.includes("LEFT JOIN branch_master"),
    )!;
    expect(ccCall.sql).toContain("e.process_id IN (?,?)");
    expect(ccCall.params).toEqual(["branch-noida", "p1", "p2"]);
    const unallocatedCall = calls.find(
      (c) =>
        c.sql.includes("IS NULL") && c.sql.includes("GROUP BY e.branch_id"),
    )!;
    expect(unallocatedCall.sql).toContain("AND e.process_id IN (?,?)");
    expect(unallocatedCall.params).toEqual([
      "2026-08",
      "branch-noida",
      "p1",
      "p2",
  ...
  it("an explicitly empty process list matches nothing rather than everything", async () => {
    ...
    const ccCall = execute.mock.calls
      .map(([sql]) => String(sql))
      .find(
        (sql) =>
          sql.includes("FROM cost_centre_master ccm") &&
          sql.includes("LEFT JOIN branch_master"),
      )!;
  ...
  it("shows NA, not an inflated margin, when no GRN exists anywhere for the month", async () => {
    withOverrides((q) =>
      ...
      q.includes("FROM grn_entry_line_snapshot")
        ? []
        : undefined,
    );
    const { getPnlReconciliation } =
      await import("../pnl-reconciliation.service.js");
    const out = await getPnlReconciliation("2026-03", {
      branchIds: ["branch-noida"],
    });
  ...
  it("never reports a margin on negative revenue (credit notes above invoices)", async () => {
    withOverrides((q) =>
      q.includes("WITH invoice_actual AS")
        ? [
            {
              cost_centre_id: "cc-noida-1",
              cost_centre_code: "BSS/IB/Noida/534",
    ...
    const { getPnlReconciliation } =
      await import("../pnl-reconciliation.service.js");
    const out = await getPnlReconciliation("2026-06", {
      branchIds: ["branch-noida"],
    });
...
describe("P&L reconciliation — committed GRN estimate (reserved, not yet consumed)", () => {
  ...
  it("adds reserved GRN as a committed estimate inside the open window", async () => {
    ...
    // cc-noida-2 has no revenue/payroll of its own, plus the db_bill mirror's Rs 5L GRN
    // (fixture, ignored) and now Rs 8L reserved — the whole OP is both cost components together.
    expect(row.operatingProfit).toBe(-L(8));
    expect(out.totals.grnEstimated).toBe(L(8));
    expect(out.blockers.join(" ")).toMatch(
      /1 cost centre\(s\) also carry Rs 8\.00 L .* committed estimate/,
    );
  });
  ...
  it("owner rule fixture: consumed 100 + reserved 40 = indirect 140 for a closed month older than the window", async () => {
    withOverrides((q) => {
      if (q.includes("lifecycle_status = 'reserved'"))
        return [{ cost_centre_id: "cc-noida-1", amount: 40 }];
      if (q.includes("lifecycle_status = 'consumed'"))
        return [{ cost_centre_id: "cc-noida-1", amount: 100 }];
      return undefined;
    });
    const { getPnlReconciliation } =
      await import("../pnl-reconciliation.service.js");
    ...
    expect(row.operatingProfit).toBe(
      row.recognisedRevenue - row.payrollCost - 140,
    );
    // Draft is never read: the reserved reader asks for 'reserved' only.
    const reservedSql = execute.mock.calls
      .map((c) => String(c[0]))
      .find((q) => q.includes("lifecycle_status = 'reserved'"))!;
  ...
  it("reserved GRN alone (no consumed anywhere) satisfies the IDC-exists check and margin is not NA'd", async () => {
    withOverrides((q) => {
      if (q.includes("lifecycle_status = 'reserved'"))
        return [{ cost_centre_id: "cc-noida-1", amount: L(8) }];
...
describe("P&L reconciliation — below-the-line (depreciation, finance cost, tax)", () => {
  ...
  it("filters to company-wide rows only (process_id IS NULL AND branch_id IS NULL) — never the canonical engine's per-process rows", async () => {
    ...
    const call = execute.mock.calls.find(([sql]) =>
      String(sql).includes("FROM process_pnl_cost_component"),
    );
    expect(call, "must query process_pnl_cost_component").toBeTruthy();
    expect(String(call![0])).toContain(
      "process_id IS NULL AND branch_id IS NULL",
    );
  ...
  it("with nothing entered, truePat equals operatingProfit and a blocker explains why", async () => {
    ...
    expect(out.blockers.join(" ")).toMatch(
      /Depreciation, finance cost and tax have not been entered/,
    );
  });
  ...
  it("returns zero below-the-line figures when the table does not exist yet (pre-migration/older DB)", async () => {
    ...
    tableExists.mockImplementation(
      async (name: string) => name !== "process_pnl_cost_component",
    );
    const { getPnlReconciliation } =
      await import("../pnl-reconciliation.service.js");
    const out = await getPnlReconciliation("2026-08", {
  ...
  it("keeps a closed, otherwise idle cost centre's period budget in allocatedBudget", async () => {
    ...
    execute.mockImplementation(async (sql: string, params?: unknown[]) => {
      ...
      if (
        ...
        const [rows] = (await base(sql, params)) as [
        ...
        return [
          [
            ...
            {
              id: "cc-noida-closed",
              cost_centre_code: "BSS/IB/Noida/777",
              cost_centre_name: "Noida Closed",
              company_name: "Mas Callnet India Pvt Ltd",
              active_status: 0,
              branch_id: "branch-noida",
              branch_name: "NOIDA",
        ...
        return [
          [
            ...rows,
            {
              ...
              sub_head: null,
              item_name: "Rent",
              cost_centre_id: "cc-noida-closed",
              cost_centre_code: "BSS/IB/Noida/777",
              amount: 105000,
            },
          ],
    ...
    const closed = out.rows.find(
      (row) => row.costCentreId === "cc-noida-closed",
    );
    expect(
      closed,
      "closed cost centre with only budget still has a row for the month",
    ).toBeDefined();
