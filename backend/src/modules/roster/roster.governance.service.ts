...
export const rosterGovernanceService = {
  ...
  async advanceCycleStatus(id: string, newStatus: string, userId: string, req?: Request): Promise<RosterCycle> {
    ...
    // Email notification per rostered employee (fire-and-forget), alongside the SMS blast
    // below rather than instead of it. Ships in shadow — the gateway resolves recipients
    // and records a claim without delivering until roster_published is switched live.
    if (newStatus === 'published') {
      setImmediate(async () => {
        try {
          ...
          console.log(`[roster-notify] cycle ${id}: ${stats.employees} employees — sent=${stats.sent} shadow=${stats.shadow} skipped=${stats.skipped}`);
        } catch (err) {
