...
function statusBadge(status: string): string {
  const map: Record<string, { bg: string; text: string; label: string }> = {
    ...
    branch_head_approved:    { bg: "#fef9c3", text: "#854d0e", label: "Br. Approved" },
    accounts_head_approved:  { bg: "#fef9c3", text: "#854d0e", label: "Accts. Approved" },
    approved:                { bg: "#dcfce7", text: "#166534", label: "Approved" },
    finance_head_approved:   { bg: "#dcfce7", text: "#166534", label: "Fin. Approved" },
...
function kpiCell(label: string, value: string, color?: string, subtext?: string): string {
...
function kpiStrip(cells: { label: string; value: string; color?: string; subtext?: string }[]): string {
...
function dataTable(headers: string[], rows: (string | { raw: string })[][], compact = false): string {
  ...
  const body = rows.map((row, ri) =>
    `<tr style="background:${ri % 2 === 0 ? C.card : "#f8fafc"};">${row.map((cell) => {
...
export function renderEmail(report: BranchHealthReport, opts: { generatedAt: string; dashboardUrl?: string }): string {
  ...
  const budgetBody = hasBudget
    ? kpiStrip([
        ...
        { label: "Utilization", value: `${raw.budget.utilizationPct}%`, color: raw.budget.utilizationPct >= 95 ? C.danger : raw.budget.utilizationPct >= 80 ? C.warn : C.success },
      ])
  ...
  const grnStatsBody = kpiStrip([
    { label: "Raised (MTD)", value: String(raw.grnStats.raised), color: C.primary },
    { label: "Approved", value: String(raw.grnStats.approved), color: C.success },
    { label: "Pending Approval", value: String(raw.grnStats.pending), color: raw.grnStats.pending >= 5 ? C.warn : raw.grnStats.pending > 0 ? C.accent : C.primary },
    { label: "Total Value (MTD)", value: inr(raw.grnStats.totalRaisedAmount), color: C.primary },
  ]);
  ...
  const lateProcessRows = raw.lateStats.processWise.map((p) => [p.process, String(p.count)]);
  const lateBody = `
  ...
  const shrinkageBody = kpiStrip([
    { label: "Scheduled", value: String(raw.shrinkage.scheduled), color: C.primary },
    { label: "Present", value: String(raw.shrinkage.present), color: C.success },
    { label: "Absent", value: String(raw.shrinkage.absent), color: raw.shrinkage.absent > 0 ? C.warn : C.success },
    { label: "On Leave", value: String(raw.shrinkage.onLeave), color: C.muted },
  ...
  const rows = [
    ...
    // Performance
    sectionHeader("8. Process-wise Performance (Last 7 Days)"),
    `<tr><td>${card(perfBody)}</td></tr>`,
    // Pending actions
    sectionHeader("9. Pending Actions"),
    `<tr><td>${card(actionsBody)}</td></tr>`,
    // Signals
    sectionHeader("10. Critical Intervention Points"),
    ...
    sectionHeader("11. Positive Achievements"),
...
export function subjectLine(report: BranchHealthReport): string {
  const statusTag = { healthy: "✅ Healthy", watch: "⚠️ Watch", critical: "🔴 Critical" }[report.overallStatus];
