...
export async function importBbAprMasmisBatch(
  ...
  if (batchRows.length === 0) {
    // "Nothing left to do" is ambiguous: it's the normal shape of a re-run after every row
    // already finished, but also the shape of a batch whose row-staging step never persisted
    // anything -- see BATCH-1788948395588-R6909 in bulk-upload.routes.ts. Only the second case
    // is a failure; telling them apart needs a second query, at ANY row_status.
