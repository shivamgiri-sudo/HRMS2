 ...
 * live db_masmis.bvo_order_export table (3,050,861 real rows, the largest
 * table in this batch, most recent upload 2026-08-11) the separate My
 * Dashboards tool already uses. Live schema matches the doc exactly --
 * note real dates here are stored as raw text/Excel-serial strings, not
 * converted (order_date/created_at_raw are varchar, matching this
 * table's own existing real data), same "store raw, don't guess a
 * conversion" discipline as bb_apr's fhd column.
 */
...
export async function importBvoOrderExportMasmisBatch(
  batchId: string,
