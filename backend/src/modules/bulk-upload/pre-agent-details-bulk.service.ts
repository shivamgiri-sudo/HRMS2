 ...
 * db_masmis.pre_agent_details (sql/1766). Columns confirmed directly
 * against the real file the user supplied
 * (C:\Users\MAS60358\Desktop\Housing Premium\Premum_Agent Details.xlsx --
 * note the "Premum" typo is in the real filename only, not touched here):
 * Emp ID, Agent Name(As per CRM), TL Name, Center, DOJ, Tenure, Tenure
 * Bucket, Target, Achievement, Ach%, Status -- an achievement-tracking
 * sheet, not a plain roster. Deliberately a different, company-specific
 * shape from owner-agent-details-bulk.service.ts -- the two real files are
 * not parallel (Owner's real identity column is "MAS", Premium's is
 * "Emp ID").
 */
...
export async function importPreAgentDetailsBatch(
  batchId: string,
