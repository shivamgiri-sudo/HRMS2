...
export async function importEmployeeMasterBatch(
   ...
   *
   * designation is required for the same reason, added 2026-08-27 in f6a2e1fd and restored
   * here after merge 12433693 dropped this half of it while keeping its test. Leaving it
   * optional produced 157 active employees with no designation at all, every one of them
   * created in July and August 2026 through this path. A person with no designation has no
   * grade band, no APR eligibility rule and no seat rate, shows on the org chart as an
   * unlabelled card, and cannot be recovered after the fact — none of the 157 exist in
   * db_bill under their code, name+DOJ, PAN or mobile, and only one has an ATS candidate
   * record, so there is nothing to back-fill from. The value has to be captured at import
   * or it is gone.
   *
