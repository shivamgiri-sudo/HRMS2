...
describe("manual performance upload inspection", () => {
  it("accepts a valid CSV and reports deterministic evidence", () => {
    const inspection = inspectManualUploadFile(
      dataset(),
      csv("employee_code,score_date,audit_id,quality_score\nMAS001,2026-07-01,A-1,98.5\n"),
      "quality.csv",
    );
  ...
  it("rejects a file when a mapped metric column is missing", () => {
    expect(() => inspectManualUploadFile(
      dataset(),
      csv("employee_code,score_date,audit_id\nMAS001,2026-07-01,A-1\n"),
      "quality.csv",
  ...
  it("rejects duplicate headers case-insensitively", () => {
    expect(() => inspectManualUploadFile(
      dataset(),
      csv("employee_code,EMPLOYEE_CODE,score_date,audit_id,quality_score\nMAS001,MAS001,2026-07-01,A-1,99\n"),
      "quality.csv",
  ...
  it("rejects silent row truncation when the configured maximum is exceeded", () => {
    ...
    expect(() => inspectManualUploadFile(
      limited,
      csv("employee_code,score_date,audit_id,quality_score\nMAS001,2026-07-01,A-1,98\nMAS002,2026-07-01,A-2,97\n"),
      "quality.csv",
  ...
  it("rejects Excel content for a CSV dataset", () => {
    const buffer = excelBuffer([
      { employee_code: "MAS001", score_date: "2026-07-01", audit_id: "A-1", quality_score: 99 },
    ]);
  ...
  it("builds the governed template in mapping order", () => {
    ...
    expect(required).toEqual(["employee_code", "score_date", "audit_id", "quality_score"]);
    expect(buildManualUploadTemplate(dataset()))
      .toBe("employee_code,score_date,audit_id,quality_score\r\n");
  });
});
