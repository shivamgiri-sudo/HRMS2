...
// The half-day rules live in shared/halfDayLeave.ts because the attendance engine needs the
// SAME rules: it re-grades days from punches and would otherwise restore a half day to
// 'leave_approved' (which payroll pays at 1.0) on its next run. Re-exported here so existing
// importers of leave.service keep working.
export {
  HALF_DAY_LEAVE_CODES,
