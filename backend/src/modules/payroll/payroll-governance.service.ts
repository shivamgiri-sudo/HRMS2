...
export const payrollGovernanceService = {
  async readiness(runId: string) {
    ...
    const checks: Array<Promise<PayrollReadinessIssue | null>> = [
        ...
        `SELECT e.id, e.employee_code,
           ...
           JOIN (
             SELECT UserID, ReportDate AS report_date, ROUND(SUM(TIME_TO_SEC(Net_Login)) / 60) AS apr_minutes
               FROM (
                 SELECT DISTINCT UserID, ReportDate, Net_Login FROM apr
      ...
      // The apr table (upstream, read-only) carries literal duplicate rows for some
      // employee/dates -- same UserID+ReportDate+Net_Login inserted more than once. Confirmed
      // live 2026-09-07: 5,240 employee/date combos in August alone. A plain SUM(Net_Login) over
      // those doubles apr_minutes and manufactures a false mismatch against a correctly-recorded
      // attendance_daily_record row. The apr_dedup subquery below (SELECT DISTINCT before
      // summing) collapses an exact repeat while still correctly adding together two genuinely
      // different login-session rows that happen to share the same UserID+ReportDate (they
      // differ on Net_Login, so DISTINCT does not merge them). This reads around the upstream
      // defect in our own query only -- it does not touch db_bill's schema or data, which stays
