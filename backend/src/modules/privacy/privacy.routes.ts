...
/**
 * dpdp_config seeds these keys with placeholders — "To be configured",
 * "privacy@yourcompany.com" — so that a fresh install has rows to edit rather than
 * missing ones. They are not contact details, and this endpoint feeds the site footer on
 * every page, so returning them verbatim would publish a fake privacy contact to every
 * visitor. Anyone raising a DPDP grievance would write to an address nobody reads, and
 * the statutory obligation would look discharged when it is not.
 *
 * A placeholder is therefore reported as unconfigured. The footer only renders when a
 * name comes back, so it stays absent until someone sets real values, and appears on its
 * own once they do.
 */
...
// GET /grievance-officer — public: the site footer renders this on every page.
privacyRouter.get(
  "/grievance-officer",
