...
export async function getLiveKpiPerformance(employeeId: string, period: Period, anchorDate?: string) {
  ...
  try {
    ...
    //
    // But it is NOT swallowed silently. This whole block was deleted by a
    // whole-tree snapshot commit (e74f1ec2, titled only "fix(manifest): ...")
    // while MyPerformanceTile.tsx kept rendering its percentile chip, so the
    // comparison simply stopped appearing and nothing anywhere said so. A bare
    // `catch {}` is the same failure mode a second time: the chip is guarded on
    // `typeof percentile === "number"`, so a broken query here is invisible in
    // the UI by design. Log it, so the next disappearance is diagnosable.
    process.stderr.write(
      JSON.stringify({
        level: "warn",
        module: "kpi-master",
        event: "PEER_COMPARISON_FAILED",
        employeeId,
