...
describe('calculateTrackerSummary', () => {
  it('should calculate summary stats from employee rows', () => {
    ...
    expect(summary).toEqual({
      ...
      id_creation_overdue_count: 0, // none of these fixtures set id_creation_sla_breached
    });
    expect(
...
// ─── getJoiningDocumentsTracker: exclusion filter + employee-ID SLA field ─────

import { getJoiningDocumentsTracker } from '../ats.joiningDocumentsTracker.service';
...
describe('getJoiningDocumentsTracker', () => {
  ...
  it('excludes every NON_REACTIVATABLE_STATUSES value, not just resigned/terminated', async () => {
    mocks.mockBuildScopeWhereClause.mockResolvedValueOnce({ sql: '1=1', params: [] });
    // db.query() and db.execute() are the same mock in this file (see the header
    // comment on the db/mysql.js mock above) — one queued response per call the
    // service makes: the row query, then queryTrackerSummary's aggregate.
  ...
  it('sums id_creation_overdue_count in the summary aggregate', async () => {
    mocks.mockBuildScopeWhereClause.mockResolvedValueOnce({ sql: '1=1', params: [] });
    vi.mocked(db.query).mockResolvedValueOnce([[], []]);
    vi.mocked(db.execute).mockResolvedValueOnce([[{
      total_employees: 1, completed_count: 0, in_progress_count: 0, pending_count: 1,
