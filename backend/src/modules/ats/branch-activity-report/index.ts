...
async function loadFacts(reportDate: string) {
  const from = [
    monthStartOf(reportDate),
    addDays(reportDate, -OPEN_TOKEN_LOOKBACK_DAYS),
  ].sort()[0];
...
export async function buildBranchActivityReports(
  ...
  return branches.map((branch) => {
    ...
    const html = renderEmail(data, {
      generatedAt,
      dashboardUrl,
      dateLabel: dateLabel(reportDate),
      branchLabel: branch,
    });
    return {
      branch,
...
export async function sendBranchActivityReports(
  opts: SendOptions = {},
): Promise<BranchSendResult[]> {
  const dryRun = opts.dryRun !== false;
  const wanted = opts.branches?.map((b) => b.toLowerCase());
  const reports = (
    await buildBranchActivityReports(opts.reportDate, opts.dashboardUrl)
  ).filter((r) => !wanted || wanted.includes(r.branch.toLowerCase()));
  if (!dryRun && (!env.SMTP_USER || !env.SMTP_PASS))
    throw new Error("sendBranchActivityReports: SMTP is not configured");
    ...
    try {
      if (r.branch === UNMAILABLE_BRANCH) {
        results.push({
          ...
          status: "skipped",
          reason: "activity with no branch recorded",
        });
        continue;
      }
      ...
      if (
        !dryRun &&
        opts.shouldSend &&
        !(await opts.shouldSend(r.branch, r.reportDate))
      ) {
        results.push({
          ...base,
          to,
