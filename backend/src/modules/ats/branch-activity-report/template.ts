 ...
 */
import { SLA, fmtMin, type BranchBlock, type Escalation, type RecruiterRow, type ReportData, type SlaStat, type Summary } from "./metrics.js";

/** Fluid: fills the reading pane up to this ceiling. Outlook desktop ignores max-width and simply uses the pane width. */
const MAX_W = 960;
const C = {
  navy: "#0B1F3A", navy2: "#16335C", gold: "#C9A24B", ink: "#1E293B", muted: "#64748B", line: "#E2E8F0",
  soft: "#F8FAFC", page: "#EEF2F7", green: "#15803D", greenBg: "#DCFCE7", amber: "#B45309", amberBg: "#FEF3C7",
  red: "#B91C1C", redBg: "#FEE2E2", orange: "#EA580C", blue: "#1D4ED8", teal: "#0F766E",
} as const;
...
const muted = (t: string) => `<span style="color:${C.muted};font-size:10.5px;font-weight:400">${esc(t)}</span>`;
...
const slaPill = (s: SlaStat) => (s.reliable ? pill(pct(s.pct), slaColor(s.pct).fg, slaColor(s.pct).bg) : pill("n/a", C.muted, C.soft));
...
const td = (t: string, o: { align?: Align; bold?: boolean; color?: string; raw?: boolean; bg?: string; size?: number } = {}) =>
...
const section = (title: string, sub: string, body: string, accent: string = C.gold) => `
...
function kpiCard(label: string, ftd: string, mtd: string, tone: string): string {
...
function kpiStrip(d: ReportData): string {
  ...
  const cards = [
    ...
    kpiCard("Closure pending", n(pending), n(mtd.open), pending > 0 ? C.red : C.green),
    kpiCard("SLA-2 met", ftd.sla2.reliable ? pct(ftd.sla2.pct) : "n/a", mtd.sla2.reliable ? pct(mtd.sla2.pct) : "n/a", slaTone(ftd.sla2)),
  ];
...
function escalationSection(d: ReportData, showBranch: boolean): string {
  const count = (l: number) => d.escalations.filter((e) => e.level === l).length;
  const chips = [
    pill(`L3 HR Head · ${count(3)}`, "#fff", C.red), pill(`L2 Branch Head · ${count(2)}`, "#fff", C.orange),
    pill(`L1 Recruiter · ${count(1)}`, C.amber, C.amberBg), pill(`Open, inside SLA · ${d.onTrackOpen}`, C.green, C.greenBg),
  ].join(" &nbsp;");
  ...
  const body = d.escalations.length === 0
  ...
  return section("Escalations — token closure pending", `Recruiter has not closed the token · SLA: call ≤ ${SLA.waitToCallMin}m, closure ≤ ${fmtMin(SLA.callToClosureMin)}`, body, C.red);
}

// ── Performance table (Today / Week / Month) ─────────────────────────────────────────────

interface MetricLine { label: string; get: (s: Summary) => string; strong?: boolean; tone?: (s: Summary) => string }
...
const METRIC_LINES: MetricLine[] = [
  ...
  { label: "Interview updates filed (any token day)", get: (s) => `${n(s.formsSubmitted)}${s.formsFromEarlier ? ` (${n(s.formsFromEarlier)} old)` : ""}` },
  { label: "Interviewed", get: (s) => n(s.interviewed) },
  { label: "Selected", get: (s) => n(s.selected), strong: true, tone: () => C.green },
  ...
  { label: "Closure pending (open)", get: (s) => n(s.open), strong: true, tone: (s) => (s.open > 0 ? C.red : C.green) },
...
const BRANCH_LABELS = ["Walk-ins", "Tokens generated", "Token closed", "Interview updates filed (any token day)", "Selected", "Rejected", "No-show", "Closure pending (open)", "Selection % (of interviewed)"];
const BRANCH_LINES: MetricLine[] = [
  ...METRIC_LINES.filter((m) => BRANCH_LABELS.includes(m.label)),
  { label: "SLA-1 met (token → call)", get: (s) => (s.sla1.reliable ? pct(s.sla1.pct) : "n/a") },
  { label: "SLA-2 met (call → closure)", get: (s) => (s.sla2.reliable ? pct(s.sla2.pct) : "n/a") },
];

function periodTable(ftd: Summary, wtd: Summary, mtd: Summary, d: ReportData, lines: MetricLine[] = METRIC_LINES): string {
  const rows = lines.map((m, i) => `<tr>
    ${td(m.label, { align: "left", bold: m.strong, bg: zebra(i) })}
    ${[ftd, wtd, mtd].map((s) => td(m.get(s), { bold: m.strong, color: m.tone?.(s), bg: zebra(i) })).join("")}</tr>`).join("");
  return dataTable(`<tr>${th("Metric", "left")}${th("Today")}${th(`Week · from ${d.weekStart.slice(5)}`)}${th(`Month · from ${d.monthStart.slice(5)}`)}</tr>${rows}`);
}
...
function funnelStages(s: Summary): Array<[string, number, number]> {
  return [["Walk-in", s.walkins, -1], ["Token generated", s.tokens, 0], ["Interview called", s.called, 1], ["Token closed", s.closed, 1],
    ["Interviewed", s.interviewed, 3], ["Selected", s.selected, 4], ["Joined", s.joined, 5]];
}

function funnelRow([label, v, parent]: [string, number, number], i: number, stages: Array<[string, number, number]>): string {
...
function slaBlock(title: string, target: string, get: (s: Summary) => SlaStat, d: ReportData): string {
  ...
  return dataTable(head
    + row("Met", (x) => slaPill(x), 0)
    + row("Average", (x) => fmtMin(x.avgMin), 1)
    + row("Slowest 10% (P90)", (x) => fmtMin(x.p90Min), 2)
    + row("Breaches", (x) => `<b style="color:${x.breached ? C.red : C.green}">${n(x.breached)}</b>`, 3)
    + row("Data coverage", (x) => `<span style="color:${x.reliable ? C.muted : C.amber}">${x.measured}/${x.population} · ${x.coveragePct}%</span>`, 4));
}
...
function slaSection(d: ReportData): string {
  ...
  return section("SLA scorecard", "Two service levels across the walk-in journey", body);
}
...
function recruiterRow(r: RecruiterRow, i: number): string {
  ...
  const c = (t: string, o: { bold?: boolean; color?: string; raw?: boolean } = {}) => td(t, { ...o, bg, size: 11.5 });
  return `<tr>
...
function recruiterTable(b: BranchBlock): string {
  ...
  const active = b.recruiters.filter((r) => r.ftd.tokens > 0 || r.openNow > 0 || r.ftd.formsSubmitted > 0);
  const idle = b.recruiters.length - active.length;
  const heads = ["Tkn", "Closed", "Filed", "Sel", "Rej", "NS", "Open", "Wait / Handle", "SLA-1", "SLA-2", "Mth Tkn", "Mth Sel%"].map((h) => th(h)).join("");
...
function branchPanel(b: BranchBlock, d: ReportData): string {
  ...
  const stat = quiet ? "No walk-ins today" : `Today · ${n(b.ftd.walkins)} walk-ins · ${n(b.ftd.selected)} selected · ${n(b.ftd.open)} open`;
...
function notesFooter(d: ReportData): string {
  ...
  const flags = [
    q.closedWithoutCallTime ? `${q.closedWithoutCallTime} MTD token(s) closed with no interview-call time recorded (excluded from SLA-1/2)` : "",
    q.negativeDurations ? `${q.negativeDurations} token(s) had a call/closure time earlier than arrival (excluded from SLA averages)` : "",
    q.measuredHandle >= 20 && q.instantClosures / q.measuredHandle > 0.5 ? `${q.instantClosures} of ${q.measuredHandle} MTD closures were stamped within 1 minute of the call — SLA-2 here likely reflects bulk closure, not interview time` : "",
    q.staleOpenTokens ? `${q.staleOpenTokens} open token(s) older than 7 days are excluded from escalations — close or mark them no-show` : "",
  ].filter(Boolean);
...
export interface RenderMeta { generatedAt: string; dashboardUrl?: string; dateLabel: string; /** Set for a single-branch email. */ branchLabel?: string }
...
function header(meta: RenderMeta): string {
  const title = meta.branchLabel ? `${esc(meta.branchLabel)} — Daily Recruitment Activity` : "Branch Activity — Daily Report";
...
function branchSections(d: ReportData, single: boolean): string {
  if (single) return d.branches.map((b) => section("Recruiter-wise activity", "Today, with month-to-date tokens and selection %", recruiterTable(b), C.navy2)).join("");
  return section("Branch-wise activity", "Each branch with its recruiter-wise breakdown", "", C.navy2) + d.branches.map((b) => branchPanel(b, d)).join("");
}
