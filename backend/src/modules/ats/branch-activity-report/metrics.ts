 ...
 *  Token closed      recruiter submitted the interview form (queue_status completed/no_show,
 *                    an ats_interview_submission row, or a final candidate decision)
...
export type Outcome =
  | "selected" | "rejected" | "no_show" | "walkout" | "hold" | "client_round" | "other_closed" | "open";
...
export interface RawTokenRow {
  ...
  decision_text: string | null;    // submission.final_decision ▸ candidate.final_decision ▸ status ▸ stage
...
export interface Escalation {
  ...
  process: string;
  recruiter: string;
  stage: "Waiting for call" | "Called — interview pending" | "In interview";
  arrivalHhmm: string;
  arrivalDate: string;
  runningMin: number;
  stageSlaMin: number;
...
export const addDays = (iso: string, n: number) => fromUtc(toUtc(iso) + n * dayMs);
...
const ratio = (n: number, d: number) => (d > 0 ? Math.round((n / d) * 1000) / 10 : 0);
 ...
 */
export function classifyOutcome(row: Pick<RawTokenRow, "decision_text" | "queue_status" | "sub_id">): Outcome {
  const text = String(row.decision_text ?? "").toLowerCase();
  const qs = String(row.queue_status ?? "").toLowerCase();
  if (includesAny(text, ["no show", "noshow", "no_show"]) || qs === "no_show") return "no_show";
  if (includesAny(text, ["walkout", "walk out", "walk-out", "dropout", "drop out"]) || qs === "walked_out") return "walkout";
  if (text.includes("reject") || text.includes("not_selected") || text.includes("not selected")) return "rejected";
  if (text.includes("selection discussion")) return row.sub_id ? "other_closed" : "open"; // a stage name, not a decision
  if (includesAny(text, ["select", "offer", "joined", "onboard", "converted", "payroll_validated", "bgv"])) return "selected";
  if (text.includes("client round")) return "client_round";
  if (text.includes("hold") || text.includes("callback") || text.includes("follow")) return "hold";
  if (row.sub_id || qs === "completed") return "other_closed";
  return "open";
}

export function toFact(row: RawTokenRow, canonicalBranch: (v: unknown) => string, recruiterKey: (id: unknown, name: unknown) => string): TokenFact {
  const outcome = classifyOutcome(row);
  const recruiter = String(row.recruiter_raw ?? "").replace(/\s*[·|]\s*[A-Z]{2,4}\d{3,}\s*$/i, "").trim() || "Unassigned";
  const qs = String(row.queue_status ?? "").toLowerCase();
  ...
  return {
    ...
    called: Number(row.has_call) === 1 || qs === "called" || qs === "in_interview",
    closed,
    outcome,
    joined: stage === "joined" || stage === "onboarded",
    inInterview: qs === "in_interview",
    waitMin: row.wait_min != null && row.wait_min >= 0 ? Number(row.wait_min) : null,
    handleMin: row.handle_min != null && row.handle_min >= 0 ? Number(row.handle_min) : null,
    negativeDuration: (row.wait_min != null && row.wait_min < 0) || (row.handle_min != null && row.handle_min < 0),
    sinceArrivalMin: Math.max(0, Number(row.since_arrival_min ?? 0)),
    sinceCallMin: row.since_call_min != null ? Math.max(0, Number(row.since_call_min)) : null,
  };
}

// ── aggregation ──────────────────────────────────────────────────────────────────────────

function slaStat(pairs: Array<{ value: number | null; openOver: boolean }>, target: number, population: number): SlaStat {
  const measured = pairs.filter((p) => p.value != null).map((p) => p.value as number);
  const met = measured.filter((v) => v <= target).length;
  const breached = measured.length - met + pairs.filter((p) => p.value == null && p.openOver).length;
...
export function summarize(facts: TokenFact[]): Summary {
  ...
  const sla1 = slaStat(
    ...
    facts.filter((f) => f.tokenGenerated && (f.called || (f.outcome !== "no_show" && f.outcome !== "walkout"))).length,
  );
  const sla2 = slaStat(
    facts.map((f) => ({
      ...
      openOver: f.called && !f.closed && (f.sinceCallMin ?? 0) > SLA.callToClosureMin,
    })),
    SLA.callToClosureMin,
    facts.filter((f) => f.closed && f.outcome !== "no_show" && f.outcome !== "walkout").length,
  );

  return {
    walkins, tokens, called, closed, interviewed, selected,
    rejected: c("rejected"), noShow, walkout, hold: c("hold"), clientRound: c("client_round"),
    open: c("open"),
...
export function escalationFor(f: TokenFact, reportDate: string): Escalation | null {
  if (f.closed) return null;
  const stage: Escalation["stage"] = f.inInterview ? "In interview" : f.called ? "Called — interview pending" : "Waiting for call";
  const running = f.called ? (f.sinceCallMin ?? f.sinceArrivalMin) : f.sinceArrivalMin;
  ...
  const level: EscalationLevel = f.arrivalDate < reportDate ? 3 : f.sinceArrivalMin >= SLA.sameDayEscalateMin ? 2 : 1;
  const escalateTo = level === 3 ? "HR Head" : level === 2 ? "Branch Head" : "Recruiter + Reporting Manager";
  return {
    level, escalateTo, branch: f.branch, tokenNumber: f.tokenNumber, candidateName: f.candidateName,
    process: f.process, recruiter: f.recruiter, stage, arrivalHhmm: f.arrivalHhmm, arrivalDate: f.arrivalDate,
    runningMin: running, stageSlaMin: stageSla, overSlaMin: running - stageSla, totalAgeMin: f.sinceArrivalMin,
  };
}

const byWorst = (a: Escalation, b: Escalation) => b.level - a.level || b.runningMin - a.runningMin;

interface Periods { ftd: TokenFact[]; wtd: TokenFact[]; mtd: TokenFact[] }

function splitPeriods(facts: TokenFact[], reportDate: string, weekStart: string, monthStart: string): Periods {
  const inRange = (f: TokenFact, from: string) => f.arrivalDate >= from && f.arrivalDate <= reportDate;
  return {
...
export function buildReport({ facts, reportDate }: BuildInput): ReportData {
  ...
  const withForms = (s: Summary, rows: TokenFact[], from: string, to: string): Summary => {
    const forms = rows.filter((f) => f.formDate && f.formDate >= from && f.formDate <= to);
    return { ...s, formsSubmitted: forms.length, formsFromEarlier: forms.filter((f) => f.arrivalDate < from).length };
  };
  ...
  const allEsc = live.filter((f) => !isStale(f)).map((f) => escalationFor(f, reportDate)).filter((e): e is Escalation => !!e);
  const openLive = live.filter((f) => !f.closed && !isStale(f));

  const branchNames = [...new Set(live.map((f) => f.branch))].sort();
  const branches: BranchBlock[] = branchNames.map((branch) => {
    const rows = live.filter((f) => f.branch === branch);
    const recNames = [...new Set(rows.map((f) => f.recruiterKey))];
    const recruiters: RecruiterRow[] = recNames.map((key) => {
  ...
  return {
    reportDate, weekStart, monthStart,
    overall: periodSummaries(live),
    branches,
    escalations: allEsc.sort(byWorst),
    onTrackOpen: openLive.length - allEsc.length,
    dataQuality: {
      closedWithoutCallTime: mtdFacts.filter((f) => f.closed && !f.called && f.outcome !== "no_show" && f.outcome !== "walkout").length,
      negativeDurations: mtdFacts.filter((f) => f.negativeDuration).length,
      instantClosures: mtdFacts.filter((f) => f.handleMin != null && f.handleMin <= 1 && f.outcome !== "no_show").length,
      measuredHandle: mtdFacts.filter((f) => f.handleMin != null && f.outcome !== "no_show").length,
