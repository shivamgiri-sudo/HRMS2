...
/** db_masmis.dalmia_apr_raw is created by sql/1781 -- absent until that migration is applied, which must not break the dashboard. */
...
export async function getDalmiaDashboard(monthInput?: string, fromInput?: string, toInput?: string): Promise<DalmiaDashboardData> {
  ...
  if (apr === null) notes.push("The dalmia_apr table does not exist yet (migration sql/1781 not applied), so Utilization is unavailable.");
