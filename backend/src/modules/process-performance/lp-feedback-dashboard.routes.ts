...
import { getLpFeedbackDashboard, getLpFeedbackDetail } from "./lp-feedback-dashboard.service.js";
...
router.get("/lp-feedback-dashboard", requireRole(...VIEWER_ROLES), h(async (req, res) => {
  const from = String(req.query.from ?? "");
  const to = String(req.query.to ?? "");
  const data = await getLpFeedbackDashboard(from, to);
  res.json({ success: true, data });
}));

router.get("/lp-feedback-dashboard/detail", requireRole(...VIEWER_ROLES), h(async (req, res) => {
  ...
  const data = await getLpFeedbackDetail(kind, key, String(req.query.from ?? ""), String(req.query.to ?? ""));
  if (!data) return res.status(404).json({ success: false, error: "No records for this selection in the chosen date range" });
  res.json({ success: true, data });
}));

export { router as lpFeedbackDashboardRouter };
