 ...
 * (sql/1870). Every GNC dashboard reads them through resolveGncTargets(), so a target is
...
export interface GncTargetsResolved {
  from: string;
  to: string;
  /** false until sql/1870 has been applied -- dashboards then show "no target configured". */
