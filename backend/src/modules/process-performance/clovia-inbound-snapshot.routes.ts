...
router.get("/clovia-inbound-snapshot", requireRole(...VIEWER_ROLES), h(async (_req, res) => {
  const data = await getCloviaInboundSnapshot();
  res.json({ success: true, data });
}));

export { router as cloviaInboundSnapshotRouter };
