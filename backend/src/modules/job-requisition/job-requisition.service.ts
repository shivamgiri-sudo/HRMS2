...
export const jobRequisitionService = {
   ...
   * Close a requisition (mark as filled or cancelled).
   *
   * Direct-close right is: super_admin (org-wide), branch_head (any branch —
   * the existing, unchanged right), or the requisition's own creator
   * (requested_by) — added 2026-09-08 per an explicit product decision that
   * whoever raised a requisition should be able to close their own without
   * needing branch_head/super_admin standing. The route's own requireRole is
   * deliberately wider than this (it has to admit every role that can BE a
   * creator, per the create route's own role list) — this in-service check
   * is the real authorization, per this codebase's standing rule that route
   * role lists are a coarse filter, not the security boundary.
   *
   * Everyone else can still reach this outcome via requestClose below.
   */
  async closeRequisition(
    ...
    const isCreator = existing.requested_by === actorId;
   ...
   *
   * Deliberately does not add a new column or requisition status: the
   * request is the job_requisition_approval_log row itself (mirrors
   * extendDeadline's own audit-only pattern), surfaced to the people who can
   * act on it via the same notifyApprovers this module already uses for
   * pending-approval alerts. Nothing about the requisition's own state
   * changes until one of them actually closes it.
   */
  async requestClose(
    id: string,
    actorId: string,
    ...
    const actionUrl = `/recruitment/job-requisition/${id}`;
    ...
    // above to happen to include them.
    if (existing.requested_by && existing.requested_by !== actorId) {
      await inboxService
        .createItem({
          user_id: existing.requested_by,
          type: "requisition_close_requested",
          title,
          description,
          entity_type: "job_requisition",
          entity_id: id,
