...
// ─── Close Requisition ───────────────────────────────────────────────────────
// requireRole here is deliberately as wide as the create route's own role list
// (super_admin, branch_head, plus everything that can BE a creator) — the real
// authorization (super_admin, branch_head, or the requisition's own creator)
// is enforced inside closeRequisition itself. Anyone in this list who is NOT
// the creator/a branch head/super_admin gets a 403 from the service, same as
// this codebase's standing rule that a route role list is a coarse filter,
...
// Open to the full REQUISITION_READ_ROLES set ("any HR designation") since
// requesting is advisory — it changes nothing about the requisition itself
// until someone with real close rights acts on it.
jobRequisitionRouter.post(
  "/:id/request-close",
  requireAuth,
  requireRole(...REQUISITION_READ_ROLES),
  ...
  h(async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!reason || typeof reason !== "string" || reason.trim().length < 5) {
      return res.status(400).json({
        success: false,
        message: "A close request reason of at least 5 characters is required",
      });
    }
