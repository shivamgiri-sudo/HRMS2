...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1097_page_code_alias_grant_reconciliation.sql", // 13 role/page grants pointed at "alias" page codes that no route gates on — active in the matrix but conferring nothing. Grants the real code where the role's job needs it (interviewer→ATS_RECRUITER_QUEUE, branch_head→ATS_DASHBOARD, payroll_hr→PAYROLL_EPF_COMPLIANCE, finance/accounts_head→ATS_JOINING_CONTROL_ROOM) and retires the dead EMPLOYEES alias rather than handing the employee directory to 22 more users as a side effect of a hygiene fix
