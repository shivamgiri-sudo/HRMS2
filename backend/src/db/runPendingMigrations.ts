...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1095_uat_feedback_intake.sql",                      // Phase 1 of the UAT governance platform: structured feedback intake, its PII-free audit spine, attachments, comments, the deterministic static-scan record, the effective-dated SLA matrix and approver delegation. Deliberately NOT an extension of helpdesk_ticket — its category/status are ENUMs that 39 endpoints read, and helpdesk-sla.service.ts recomputes breach flags across ALL tickets on every dashboard hit, so UAT rows would silently enter helpdesk SLA/aging/workload. status is VARCHAR, not ENUM, because Phases 4-6 add states and an ALTER on a populated table is the trap this table exists to avoid
  "1096_uat_release.sql",                              // Phase 1 part two: approvals, releases, structured retest evidence and rollback — the half of the lifecycle that runs after a fix exists. Nothing here depends on the AI pipeline, so it applies verbatim to hand-written fixes, which is every fix until Phase 4 is enabled. Retest is an evidence record rather than a Pass button: a retest storing only "passed" is indistinguishable from one nobody performed
  ...
  "1116_create_kpi_template_and_process_assignment.sql", // creates kpi_template and kpi_process_assignment, which the portal's KPI Assignments tab has always read and written and which have never existed. Three endpoints plus a live UI tab depend on them: /internal/kpi-templates selects from kpi_template, /internal/kpi-assignments joins kpi_process_assignment to kpi_template and process_master, and the POST/DELETE write to it — all four raised ER_NO_SUCH_TABLE, so the tab could not list, could not fill its template dropdown and could not save. Not reusing an existing table: kpi_process_template merges template and assignment into one row already bound to a single process, so it cannot express "assign this existing template to that process", and kpi_assignment assigns to designation/department/employee, not process; both are empty, and the live per-process data is kpi_process_config (291 rows, process_id + metric_id + target) at a different grain, untouched here. char(36) utf8mb4_unicode_ci matches process_master.id exactly — the server default is utf8mb4_0900_ai_ci and a foreign key whose collation differs from the referenced key is rejected with errno 3780, which is what half-applied 099. The UNIQUE KEY on (process_id, template_id, effective_from) is load-bearing: the POST uses ON DUPLICATE KEY UPDATE and without a unique constraint would insert a duplicate row on every re-assignment. assigned_by carries no FK because the handler falls back to the literal 'system'. CREATE TABLE IF NOT EXISTS, so it is idempotent; not executed by hand — it applies on the next backend start
  ];
