...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1105_revoke_unusable_alias_corrective_grants.sql", // self-correction to 1097: three of its five corrective grants opened a page whose APIs reject the role — interviewer on the recruiter workspace (every /recruiter/* endpoint is admin|hr|super_admin|recruiter) and finance_head/accounts_head on the joining control room (its router lists `finance`, not those two keys). Reverted; branch_head→ATS_DASHBOARD and payroll_hr→PAYROLL_EPF_COMPLIANCE were verified working and are kept
  ];
