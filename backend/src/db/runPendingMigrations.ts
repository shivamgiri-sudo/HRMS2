...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1214_repair_live_payroll_schema_gaps.sql", // repairs the four objects 398, 402 and 404 declared, that schema_migrations records applied with success = 1, and that MySQL never created: salary_prep_run.incentives_applied_at, salary_prep_line.payslip_emailed / payslip_emailed_at, and idx_spl_payslip_gen. All three were written ADD COLUMN IF NOT EXISTS (402 also CREATE INDEX IF NOT EXISTS) - MariaDB syntax MySQL 8.0.42 rejects with ER_PARSE_ERROR - and were recorded anyway because at the time a parse error was classified at file level rather than per statement. The four objects reached production BY HAND on 2026-08-14 with no schema_migrations row, the same way 1110's column did, so against production today this file is a guaranteed no-op: verified live, all four information_schema guards return non-zero, so every statement resolves to SELECT 1. It is registered for the REBUILD path - a fresh database gets none of these columns otherwise, because 398/402/404 still cannot execute, and payslip_emailed backs two mounted endpoints while incentives_applied_at is the guard against wiping applied incentives on recalculate. Placed LAST because it only ALTERs tables created early, so its position is immaterial and appending disturbs no released ordering. Every object guarded individually through information_schema + PREPARE, so an object that already exists cannot suppress the repairs after it - which is exactly how 509 lost its tail.
