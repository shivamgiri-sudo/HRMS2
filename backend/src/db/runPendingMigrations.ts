...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1753_bank_account_opening_balance_change_request.sql", // Registered 2026-09-11. Company Bank Account opening balance was editable in place by a single finance_head/accounts_head/super_admin with no before/after record -- raised directly by the user as a finance-integrity gap during live testing of the Company Bank Accounts drawer. New table backs a single-stage maker-checker (approver must not be the requester, same guard pattern as cost_centre_master's L1/L2 approval in cost-centre-management.service.ts): a change is proposed with a mandatory reason, then approved or rejected by a second qualifying user before opening_balance actually moves. company-bank-account.service.ts's update() no longer accepts openingBalance for an existing account -- see requestOpeningBalanceChange/decideOpeningBalanceChange there. See company-bank-account.routes.ts.
  ];
