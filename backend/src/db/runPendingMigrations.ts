...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1500_wfm_roster_import_engine.sql", // creates the WFM roster import preview tables that roster-import.routes.ts and roster-import.service.ts already write to; unlisted, those routes pass compile but fail on fresh databases with ER_NO_SUCH_TABLE
  ...
  "1248_grn_dbbill_migration_schema.sql", // schema prep for db_bill -> HRMS GRN/vendor/imprest migration; lock file already marked this released, so restoring it prevents a fresh database from silently skipping the traceability columns and branch map
  "1502_apr_source_column.sql", // adds apr.source/upload metadata with MySQL-8-safe guards so manual APR uploads can be protected from sync overwrites
