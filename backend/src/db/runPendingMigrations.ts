...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1653_bgv_page_access_hr_variants.sql", // Grants ATS_BGV / ATS_BGV_REPORT in role_page_access to branch_hr, hr_admin, ho_hr, process_hr, recruitment_hr and payroll_hr -- migration 1645's rbacPageMatrix.ts comments already declared this intent, but only granted role_report_permissions (a different table, for the Report Library), never the role_page_access row WorkforcePageGate's canViewPage() actually reads, so the BGV Verification Center page stayed invisible to these roles despite their backend API access already being granted. Additive, idempotent.
  ];
