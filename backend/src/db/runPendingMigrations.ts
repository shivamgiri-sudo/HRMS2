...
// Bounds verifySchemaVersion()'s whole operation (connect + every query) â€" see the call
 ...
 * schema problem â€" a missing file, a checksum mismatch, bad SQL â€" and badly wrong for a momentary
 ...
 * only â€" adding lock codes there would silently re-run statements inside other people's
 ...
 * â€" the metadata lock a DDL needs â€" defaults to 31,536,000 seconds, a full year: a
 ...
 * still fails the boot rather than hanging it â€" which is what the runner's own
 ...
 *
 * Every migration connection goes through here â€" the advisory-lock connection, the
...
// Non-b duplicates (020, 021, 022) are excluded â€" only b-variants are sourced.
// Duplicate numeric prefixes (010/010, 012/012, 198/198, 204/204, 271/271, 504/504) are intentional â€"
...
const MIGRATION_MANIFEST: string[] = [
  ...
  // â"€â"€ Additional migrations not yet in manifest â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
  ...
  // STOP_ON_FIRST_FAILURE then blocked every migration after it â€" same failure mode as 436.
  // The file is kept for history and superseded by 1068, which carries the corrected statements.
  "1065_billability_seat_cost.sql", // is_billable is 1 on all 58,626 rows and billable_status contradicts it on nearly every one; billability is really per (process x designation), and seat cost â€" what the client pays per person â€" was never modelled at all
  ...
  "1070_db_bill_expense_particulars.sql", // the budget and GRN LINE tables â€" expense_entry_particular carries CostCenterId, which is the only place cost is attributed below branch; expense_particular records each budget amount twice, so expense_type must be filtered when aggregating // db_bill is the live finance system and mas_hrms mirrored none of its budget (18,433 rows), GRN (85,463) or invoice line items (21,055) â€" the last of which carry the cost-centre-wise seat rate the P&L needs
  ...
  "1070_correct_auto_approved_bgv_and_bridge.sql", // never registered despite the 1070 number being reused by db-bill; applied by hand on 2026-08-03 (42 candidate_bgv_check + 6 candidate_bgv_report rows reset from fake system-verified/clear, ats_onboarding_bridge backfilled from real evidence) â€" listed here so a fresh environment picks it up; both halves are idempotent no-ops against data that is already corrected
  ...
  "1074_grn_invoice_gst_components.sql", // grn_invoice_component table + grn_cost_allocation.invoice_component_id â€" lets one vendor GRN's declared invoice total be broken into repeatable {amount without tax, GST slab} components (same invoice, multiple GST rates) instead of inheriting one GST rate from whichever budget line was picked
  "1075_bank_detail_sync_map_account_encrypt_false.sql", // corrects legacy_sync_map's bank_detail row: transform_rules_json declared 'account_encrypt':true but nothing has ever read that key (confirmed by grep) and live data is 100% unencrypted plaintext â€" the flag was asserting something the sync has never done
  "1076_mira_company_services_seed.sql", // 425's seed only inserted 6 of FALLBACK_FACTS's 7 rows (missing company-services); facts() does dbFacts.length ? dbFacts : FALLBACK_FACTS â€" all-or-nothing, not merged â€" so once any DB rows exist, a services-category question got zero facts, not even the fallback text, because 'services' was never among the seeded 6
  "1077_ai_prompt_audit_detected_intent.sql", // adds ai_prompt_audit_log.detected_intent for Mira Analytics top-intent breakdowns â€" instruments future requests only; question_hash/sanitized_context_hash are one-way SHA-256 hashes, nothing to backfill historical rows from
  "1078_ai_rate_limit_bucket.sql", // backs ai-rate-limiter.ts with a real table â€" the in-memory Map had no persistence and no cross-process sharing, so each backend process got its own independent 100/day bucket per user and a restart silently reset everyone's counter
  // 1079-1082 existed as files but were listed in neither the manifest nor the lock, so
  // they could never run anywhere â€" schema_migrations has no record of any of them and
  ...
  // are deliberately NOT listed here â€" see knownUnlisted in MIGRATION_MANIFEST.lock.json.
  ...
  // file is renamed â€" renaming a migration is how one silently re-runs. They are
  ...
  "1083_wfm_attendance_exceptions_page_code.sql", // gives /wfm/attendance-exceptions its own page code instead of borrowing WFM_LIVE_TRACKER (shared with 4 unrelated pages, and it locked out payroll â€" who own the 455 open salary_payable_days_mismatch blockers). Additive seed, already applied to live on 2026-08-07; idempotent, so a boot re-run is a no-op
  ...
  // has sat in backend/sql since it was written but was never added here, so it never ran â€"
  ...
  // â"€â"€ Finance / GRN / Imprest / Vendor enhancement, Phase A â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€
  ...
  // â€" "production runs with SKIP_MIGRATIONS=true, so a deploy applies nothing" â€" and that
  ...
  // 1213_wfm_shift_master_immutability_trigger.sql â€" REMOVED FROM MANIFEST (moved to knownUnlisted).
  // Requires MySQL SUPER privilege or log_bin_trust_function_creators=1 to create a trigger with
  // binary logging enabled â€" neither is available on the production host (maslms user). Running it
  ...
  "460_budget_line_tax_amendment.sql", // CREATE TABLE IF NOT EXISTS finance_budget_line_tax_amendment â€" maker-checker audit trail for governed tax-treatment corrections on active budget lines. Shipped in 737a0a42 with its code (requestTaxAmendment/reviewTaxAmendment in branch-budget.service.ts) but never scheduled; without it every tax-amendment API call fails with ER_NO_SUCH_TABLE. COLLATE=utf8mb4_unicode_ci explicit (3f15543d): FK to finance_budget_header(id) is utf8mb4_unicode_ci. Placed LAST: its FK requires finance_budget_header to exist, which is created later in a rebuilt database. CREATE TABLE IF NOT EXISTS â€" safe to re-run.
  ...
  "1236_payroll_hr_onboarding_requests_reactivate.sql", // Follow-up to 1235: payroll_hr already passed requireRole on the same two routes (since migration 1005) but its role_page_access row for ATS_ONBOARDING_REQUESTS was active_status=0 in production, with no deliberate deactivation found across every migration touching payroll_hr or this page code (1005, 1097, 1104, 1105, 1230, 271, 345) â€" treated as drift and reactivated. Single-row UPDATE ... SET active_status=1 WHERE active_status=0; no DROP, no DELETE.
  "1237_legacy_document_verified_backfill.sql", // One-time catch-up for migrateDocumentsFromLegacy.ts::insertBatch()'s verified=0 default, fixed to verified=1 the same session (these are historical documents verified offline before this system existed, same reasoning as the legacy joining-checklist placeholder's status='verified'). UPDATE employee_documents SET verified=1 WHERE legacy_source IN ('document_master','qual_docoments','esignature') AND verified=0 â€" scoped to the three genuine migration sources, deliberately excluding legacy_source='manual' (13 rows, provenance not established by any code comment found). Explicitly reviewed with the owner before registration: read-only breakdown run twice (2026-08-18, no drift between runs) â€" document_master 196,417 rows + esignature 62 rows = 196,479 rows flip from verified=0 to verified=1. Idempotent; no DROP, no DELETE.
  "1238_dispatch_log_skipped_status.sql", // Part of the SMS-dispatch DLT fix (2026-08-18): widens dispatch_log.status to add 'skipped', so dispatch.service.ts can distinguish an event it deliberately did not attempt over SMS (no registered DLT template mapped) from one that was attempted and genuinely failed. MODIFY COLUMN widening an ENUM â€" additive, preserves every existing value and row, naturally idempotent to re-run.
  "1240_emergency_contact_onboarding_backfill.sql", // One-time catch-up for the emergency-contact sync gap fixed the same session in employee-creation-orchestrator.service.ts (commit 6930066f) â€" candidate->employee conversion never copied candidate_onboarding_profile.emergency_contact_name/relation/mobile into employee_emergency_contact, the table the ID card and both HR/self-service emergency-contact editors read. Explicitly approved by the owner in the same session ("ok do it") after the exact scope was shown. Read-only scope check (2026-08-18): of 32,787 candidate_onboarding_profile rows only 100 have a non-empty emergency_contact_mobile; 98 have no matching employees row at all (never converted) and are correctly left alone; exactly 2 already-converted employees are eligible (MAS63085, MAS63086, both active), neither already has a contact_seq=1 row. INSERT ... SELECT ... WHERE NOT EXISTS, re-verified read-only against the live values before registration. Additive only â€" no DROP, no DELETE, no UPDATE of any existing row; UNIQUE KEY uq_emp_emergency_seq(employee_id, contact_seq) backstops a duplicate/concurrent run.
  "migrations/1300_client_billing_foundation.sql", // Foundation schema for the client-billing replica â€" creates three new tables (client_invoice_number_sequence, client_invoice, client_invoice_line) only. Does not touch existing cost_centre_master, branch_master, billing_invoice or billing_*_snapshot tables. Replaces the legacy db_bill/InitialInvoicesController.php proforma-invoice engine with a modern equivalent that fixes the race condition in invoice numbering (legacy used LOCK TABLES READ, incorrect mode and no real serialization). Uses MySQL's INSERT ... ON DUPLICATE KEY UPDATE last_value = LAST_INSERT_ID(last_value + 1) idiom for safe concurrent numbering. Pure CREATE TABLE â€" no ALTER of existing tables, no data migration. Additive and idempotent by construction; safe to re-run. Approval-stage fields (bill_no, rejected_*) included now to avoid future ALTER TABLE on a production table. cost_centre_id CHAR(36) COLLATE utf8mb4_unicode_ci matches cost_centre_master(id) collation.
  "migrations/1301_client_billing_approval_workflow.sql", // Approval-workflow schema for the client-billing replica â€" creates five new tables (client_provision, client_provision_deduction, client_po_number, client_po_particular, client_invoice_audit_log) with provision tracking, PO tracking, and append-only audit log for invoice state transitions. Does not touch client_invoice or any existing table. client_provision and client_po_number use atomic SQL balance mutations to fix legacy's PHP string-coercion balance bugs. client_invoice_audit_log replaces legacy's four inconsistent reject mechanisms with one auditable path. Pure CREATE TABLE â€" no ALTER of existing tables, no data migration. Additive and idempotent by construction; verified against live MySQL 8.0. Every column uses CHAR(36) COLLATE utf8mb4_unicode_ci to match employees/cost_centre_master collation and avoid errno 3780 on FK joins.
  ...
  "1241_minimum_wage_gate_columns.sql", // Adds min_wage_provisional/min_wage_check_note to salary_package_master and ats_offer_letters for the new minimum-wage validation gate (minimum-wage-gate.service.ts), which reads minimum_wage_master (028_statutory_compliance.sql) back for the first time â€" until now it had a working CRUD API/admin UI but nothing consumed it. Follows the same provisional-not-blocked shape as F&F's is_ff_provisional and TDS/gratuity's status/reason results; wired into payrollMasters.service.ts createPackage/updatePackage (checked by branch_name, the only location signal that table has) and ats/offer-letter.service.ts generateOfferLetter (checked by candidate.branch_id, falling back to branch_name). Both flag columns default 0 so all 295 existing packages and every historical offer letter read as "not yet evaluated", not "flagged" â€" nothing retroactive. information_schema-guarded PREPARE/EXECUTE, not ADD COLUMN IF NOT EXISTS which this MySQL 8 server rejects while recording the migration applied. Additive only; no DROP, no DELETE, no payroll/salary calculation logic touched â€" this is a validation gate, not an arithmetic change. Live coverage measured 2026-08-19: minimum_wage_master holds 24 active rows across only 6 states (DL/HR/KA/MH/TS/UP); of 1,356 active employees, 918 (68%) resolve to one of those 6 and get a real floor comparison, 430 (32%) resolve to a real state with no configured floor (provisional, not blocked), and 8 have no resolvable state at all.
  "migrations/430_rbac_matrix_applied_grants.sql", // RETROACTIVELY REGISTERED 2026-08-19 â€" shipped earlier the same day (eb8fafa8, the RBAC applier provenance fix) but never added here, an oversight caught by an independent QA re-audit reading the file tree directly. The number collides with the unrelated, already-applied sql/430_finance_grade_headcount_driver.sql (different directory, sql/ vs sql/migrations/ â€" the "migrations/" prefix here is load-bearing, same convention as 1300/1301 above). This did not weaken the RBAC safety fix itself: apply-rbac-page-matrix.mjs creates rbac_matrix_applied_grants inline via CREATE TABLE IF NOT EXISTS on every run, independent of this manifest. Registering it now so schema_migrations correctly shows it applied and the migration-manifest-guard test has a real entry to check against, not so anything starts working that wasn't already. Pure CREATE TABLE, no ALTER, no DROP, no DELETE; safe to run any time.
  "migrations/435_rbac_11_super_admin_only_pages.sql", // UAT finding D011 (uat-100pct-readiness branch, 2026-08-18): 11 live, reachable pages had role_page_access grants of super_admin only, making their nav entries invisible to every non-super_admin role even though the backend route guards accept a broader set. MATERNITY_LEAVE and ORG_MASTERS from the same sweep were already fixed by migrations 1231/1233. This closes the remaining 11: ATTENDANCE_BILLING_CONFIG, BENEFITS, CLIENT_MASTER, COMPLIANCE_AUDIT_REPORT, EXIT_COMMAND_CENTER, LEAVE_TYPES, MCNMEET, MOBILITY, PORTAL_DATA_MANAGER, PROCESS_CONFIG, SUPPORT_COMMAND_CENTER. Role grants are derived from each page's backend requireRole() calls (re-read 2026-08-23) and navConfig entries â€" write grants match backend write guards; view-only where only GET is confirmed. All INSERT â€¦ ON DUPLICATE KEY UPDATE; pure data migration, no DDL. Does NOT auto-apply until explicit user approval per CLAUDE.md migration-approval rule.
  "1242_employee_loans_approval_gate.sql", // Adds employee_loans.created_by/rejected_by/rejected_at/rejection_reason for the new loan approval gate (loans.routes.ts POST /:id/approve, /:id/reject). POST / previously self-stamped approved_by/approved_at with the creator's own id at INSERT time and inserted status='active' directly â€" zero approval gate, fake provenance. Now inserts status='pending_approval' and leaves approved_by/approved_at NULL until a real approver (finance_head/payroll_head/admin/super_admin, mirroring payroll.service.ts::updateRunStatus's head-role tier) calls /approve, which is blocked for the original creator (created_by === approver, matching PAYROLL_SELF_APPROVAL) and uses an `UPDATE ... WHERE status='pending_approval'` affected-rows check for the 409 race guard, mirroring leave.service.ts::reviewRequest. No ENUM/CHECK exists on `status` (plain VARCHAR(20)), so the new status values themselves need no DDL â€" only the provenance columns do. information_schema-guarded PREPARE/EXECUTE, matching 1241 immediately above. Additive only; touches none of the 67 existing rows (all status IN ('active','completed'), actively deducting real payroll) â€" no UPDATE, no DELETE, no payroll calculation logic changed. payrollCalculate.service.ts and running-salary.service.ts's loan-EMI reads already filter WHERE status='active', so 'pending_approval' rows are automatically excluded with no change to either file.
  "migrations/1303_client_billing_page_access.sql", // Client Billing frontend Task 1 (2026-08-19): page_catalog row + role_page_access grants for FINANCE_CLIENT_BILLING (/finance/client-billing), the workspace page over the client-billing replica backend (1300/1301/1302 above). Same cautionary reasoning as 1066/1129: without a grant here the page is invisible to every role except super_admin. Roles match client-billing.routes.ts's live ALLOWED_ROLES exactly (admin, finance, finance_head, accounts_head), plus an explicit super_admin row for audit-query consistency with the rest of this table. can_delete=0 for every role â€" no route in this module exposes a DELETE verb; a wrong document is rejected/superseded, not deleted, preserving client_invoice_audit_log. Registered but NOT applied to production as of this commit â€" see Task 1's own report for the live PREPARE-verification. Purely additive: two INSERT ... ON DUPLICATE KEY UPDATE statements, no ALTER, no DROP, no DELETE.
  "1243_backfill_branch_gst_state_code.sql", // Backfills branch_master.gst_state_code (added NULL-only by 1087_branch_master_gst_registration.sql, which explicitly deferred population) â€" this was blocking the client-billing module's createProforma/approveInvoice, which correctly 400 without it. gst_state_code was NULL on all 45 live branch_master rows as of 2026-08-19. The GST state code is a fixed, public 2-digit reference table (unchanged since 2019-2020) â€" not a per-branch judgment call â€" matched against branch_master's OWN existing `state` column (already populated on 27/45 rows) wherever present, and against `city`/`branch_name` (cross-checked against a sibling row for the same place that does have `state` populated) for the rest. cost_centre_master.vendor_gst_state was checked and rejected as an alternate source: it records the client/vendor's own GST state per billing relationship, not the branch's physical location (one branch_id alone carries 50+ contradictory vendor_gst_state values). 40 of 45 branches backfilled with high-to-very-high confidence via explicit id-scoped UPDATEs (not a derived/computed statement) â€" 5 deliberately left NULL (HEAD_OFFICE: genuinely ambiguous, org has two different head offices in this same table; SCAN_N_SMILE: no location signal; 3 TD-BR-* rows: synthetic smoke-test branches). Every WHERE clause also requires gst_state_code IS NULL, so a re-run is a no-op and this can never overwrite a value Finance enters by hand. Does not populate gstin (no real GSTIN exists anywhere in this schema â€" a genuine, separate, unaddressed gap), gst_type, or any tax-calculation/payroll logic. Explicitly approved by the owner after live review of the full 45-row confidence table.
  "1244_tds_slab_2025_26_new_regime_dedup.sql", // Deactivates 6 stale rows in payroll_tax_slab_master (financial_year='2025-26', regime='new') that were causing the exact TAX_SLABS_AMBIGUOUS condition getSlabs() (taxEngine.service.ts, fixed cd29c8d9 2026-08-17) is designed to refuse â€" 13 active rows where only 7 bands exist, live since 2026-07-20. Not exact duplicates: two genuinely different slab schemes both tagged '2025-26'/'new' â€" 7 rows from 2026-06-03 (0-4L 0%, 4-8L 5%, 8-12L 10%, 12-16L 15%, 16-20L 20%, 20-24L 25%, 24L+ 30%, the correct Budget 2025 structure, byte-identical to the FY2026-27 row set and matching statutory_config's independently-maintained tds_slab_* fallback rates exactly) vs 6 rows from 2026-07-20 (0-3L/3-7L/7-10L/10-12L/12-15L/15L+, the superseded Budget 2024 structure, mislabeled '2025-26'). Deactivates the 6 stale ids by explicit id list â€" not a MIN(id)/GROUP BY heuristic like 503_pt_slab_dedup.sql, since the two cohorts disagree on rate_pct/slab_to and a blind pick could keep the wrong one â€" then adds UNIQUE(financial_year, regime, slab_from, active_status), information_schema-guarded PREPARE/EXECUTE matching 503/1232. Harmless to any number computed so far: salary_prep_run.tds_mode is 100% 'manual' (66/66 runs), so the auto path that reads this table has never executed live. Explicitly approved by the owner after live review of both full cohorts.
  "1245_fix_head_office_cost_centre_branch_mixup.sql", // Undoes an accidental cost-centre reassignment caused by the "Head Office" branch-name collision, and clears one stale close_date. branch_master holds TWO distinct, real, active branches both displaying as a case variant of "Head Office" (HQ = Mumbai, CORP = Noida) â€" deliberately, per 1115_reactivate_operational_branches.sql, which warns "do not merge branches on name". org.service.ts listActive() dedupes same-named rows to one arbitrary survivor, so the Branch Budget picker silently resolved "Head Office" to Mumbai/HQ; six cost centres activated on 2026-08-19 intending the real Noida/CORP branch landed on HQ instead. Verified wrong against db_bill.cost_master (all six active there under Head Office) and against finance_cost_centre_monthly_driver rows already keyed to CORP from 2026-08-03. Second statement clears close_date=2023-12-30 on BSS/BO/CORP/302, which read active_status=1 but stayed invisible to branch-budget-allocation.service.ts::listActiveCostCentres (close_date IS NULL OR close_date > CURDATE()); the code fix preventing recurrence is in org.service.ts setStatus(). Both UPDATEs are guarded so a re-run matches nothing. Applied to production 2026-08-19 and verified; no DROP, no DELETE, no payroll/tax logic touched.
  "1246_add_computers_26_27_cost_sub_head.sql", // Adds the missing "Computers 26-27 Cost" sub-head under REPAIRS_MAINTENANCE. db_bill.tbl_bgt_expensesubheadingmaster SubHeadingId=119 ("Computers 26-27 COST", HeadingId=9 = Repairs & Maintenance, Status1=1, sub_close_status=1) is live in db_bill but was never mirrored into mas_hrms, so it could not be selected when raising a branch budget. db_bill re-creates this line per fiscal year (COMPUTERS-2020-21 COST ... Computers 25-26 COST, Computers 26-27 COST); mas_hrms never adopted that yearly-vintage pattern, which is why the FY row was absent. Distinct from CAPEX_COMPUTERS ("Computers - Cost") added by 1060_sync_expense_heads_from_db_bill.sql, which consolidates the equivalent rows from db_bill's SEPARATE HeadingId=27 (Repairs & Maintenance Capex) â€" placement on the plain head was confirmed with the owner, not assumed, since the two heads' yearly sub-head sets look identical at a glance. Defaults mirror its nearest sibling COMPUTER_PERIPHERALS (Device, 18% exclusive, fully recoverable, device_count, opex). INSERT IGNORE against UNIQUE(head_id, sub_head_code)/(head_id, sub_head_name), so a re-run is a no-op. Applied to production 2026-08-19 and verified; additive only.
  "migrations/1304_client_billing_cutover_schema.sql", // Client Billing Historical Cutover Task 1 (2026-08-19, docs/superpowers/plans/2026-08-19-client-billing-cutover.md / docs/superpowers/specs/2026-08-19-client-billing-cutover-design.md Â§3, Â§6): adds client_invoice.is_migrated (TINYINT(1) NOT NULL DEFAULT 0) + legacy_id (INT NULL, UNIQUE) and the identical pair on client_credit_note, plus a new throwaway client_invoice_migration_staging table (106 src_-prefixed columns mirroring db_bill.tbl_invoice verbatim, read live off db_bill's information_schema, plus target_id/target_gst_type/target_apply_gst/target_is_migrated and validation_error/validation_status for a later task's dry-run report). Writes zero historical business data â€" client_invoice/client_credit_note are both 0 rows as of this commit, and the staging table starts empty; the actual ~11,469-row load is explicitly out of scope for this migration and forbidden elsewhere in this plan without separate human sign-off. legacy_id's nullable-unique-index behavior (new rows always insert legacy_id=NULL, so a plain UNIQUE index never blocks ordinary invoice creation) was independently proven live against mas_hrms before this file was written: a throwaway x_nullable_unique_test_<timestamp> table took two legacy_id=NULL inserts successfully while a real duplicate non-NULL value correctly raised ER_DUP_ENTRY, table dropped after â€" see this task's own report for the exact command/output. information_schema-guarded PREPARE/EXECUTE idiom for both ALTER TABLEs, matching 431/1241/1242 (MySQL 8 does not support ADD COLUMN IF NOT EXISTS as a single clause the way this repo's history already found out the hard way); CREATE TABLE IF NOT EXISTS for the staging table. Every statement in this file was PREPARE-verified (compiled, never executed) against the real live mas_hrms connection before commit â€" confirmed afterward that neither client_invoice.is_migrated/legacy_id nor client_invoice_migration_staging existed, proving nothing was actually applied. Registered but NOT applied to production as of this commit. UPDATE (Task 2, same day): its two ALTERs DID apply at the next boot (2026-08-19 09:38, executor Work:25124, both columns confirmed live on both tables) but its CREATE TABLE failed for real with "Row size too large... 65535", recorded success=0 â€" see 1305 immediately below for the fix and root cause.
  "migrations/1305_client_billing_cutover_staging_fix.sql", // Client Billing Historical Cutover Task 2 prerequisite fix (2026-08-19). 1304's CREATE TABLE client_invoice_migration_staging failed live with InnoDB "Row size too large... 65535" (105 src_ VARCHAR columns + 7 target/validation columns sum to ~68,900 worst-case inline bytes, ~3,300 over the ceiling) and is permanently stuck retrying that same failure at every boot per runPendingMigrations.ts's own success=0-is-not-applied governance, since nothing in 1304's unchanged text differs between retries. Fix (independently verified live before commit: reproduced the identical error against a throwaway table carrying 1304's exact columns, confirmed ROW_FORMAT=DYNAMIC alone does NOT fix it, confirmed converting the four widest free-text columns â€" filepath/eptp_act_remarks/his_eptp_act_date/his_eptp_act_remarks, all originally VARCHAR(1000) and unindexed â€" to TEXT does, per the error's own "not counting BLOBs" carve-out): recreates client_invoice_migration_staging with those four columns as TEXT, identical otherwise to 1304's declared shape. Also creates client_credit_note_migration_staging (new, not in 1304 â€" Task 2's own implementer judgment call per the plan: a second table rather than a `kind`-discriminated shared one, since tbl_credit_note's 39 columns barely overlap tbl_invoice's 106 by name or meaning). Does not edit 1304's own file (this repo's established pattern for a migration whose DDL didn't take effect â€" matches 1116/1128/1211/1217/1218 exactly: a new numbered file re-runs the missing DDL, original left untouched) and does not touch client_invoice/client_credit_note's own schema (1304's ALTERs already succeeded). Both CREATE TABLE IF NOT EXISTS statements are naturally idempotent against 1304's own continued retries â€" once this file creates the table, 1304's unchanged CREATE TABLE IF NOT EXISTS no-ops and its own schema_migrations row flips to success=1. Applied directly against production 2026-08-19 by this task in addition to being registered here (shared-dev-server auto-apply timing, same reasoning as every migration in this manifest applied ahead of its own registration commit) â€" verified live: both tables exist, both 0 rows. No DROP, no DELETE, no data migration (that is extract.ts's job, a separate script, not this file).
  "migrations/1306_client_billing_cutover_addendum_columns.sql", // Client Billing Historical Cutover design addendum (2026-08-19, docs/superpowers/specs/2026-08-19-client-billing-cutover-addendum.md A3/A4): adds target_cost_centre_id (CHAR(36) NULL) to both *_migration_staging tables and target_invoice_id (CHAR(36) NULL) to client_credit_note_migration_staging only. A3: holds the resolved cost_centre_master.id for the addendum's own live re-confirmation of unresolved legacy cost_center codes (87 invoice rows unchanged from Task 3's report, 0 credit-note rows) so a later load task does not redo the lookup; stays NULL for the 87 genuinely-unmatched rows (excluded from this cutover pass per A3, never force-mapped). A4: holds the matched invoice's own target_id (its future client_invoice.id) for the 53 of 144 credit notes whose proforma_bill_no uniquely matches a staged invoice's bill_no with agreeing cost centres (re-verified live: 53 resolved / 86 ambiguous / 5 missing, 0 cost-centre disagreements, identical to the addendum's own figures); stays NULL for the other 91. Does not repurpose the existing target_id column (reserved for each staging row's own future primary key) â€" separate columns, unambiguous meaning. information_schema-guarded PREPARE/EXECUTE idiom, matching 431/1241/1242/1304 (MySQL 8 does not support ADD COLUMN IF NOT EXISTS as a single clause without a silent no-op recorded as applied). Three ALTER TABLEs adding one nullable CHAR(36) column apiece to throwaway staging tables only (client_invoice/client_credit_note/*_line untouched, still 0 rows as of this commit) â€" no DROP, no DELETE, no FK, no data migration (that is this addendum's own validate.ts UPDATE step, a separate script). Applied directly against production 2026-08-19 (same shared-dev-server auto-apply timing precedent as 1305) â€" verified live: all three columns exist on their respective tables.
  "migrations/1307_client_billing_cutover_missing_cost_centres.sql", // Client Billing Historical Cutover follow-up (2026-08-19): creates 8 cost_centre_master rows that were billed against in db_bill (tbl_invoice) but never actually created in cost_centre_master â€" discovered investigating the 87 unresolved cost_centre_id rows in the cutover's validation report. Only the "recent" cluster (18 of the 87 invoice rows, all finance_year 2026-27, sequential-numbered gaps sitting directly next to real active sibling cost centres for the same branch/process/company, e.g. BSS/IB/NOIDA-DD/1028-1033 already exist and are active but 1034-1040 do not); the other 67 (2015-16/2016-17, predating the current cost-centre scheme entirely) are NOT created â€" no reliable current equivalent exists. Each of the 8 codes references a genuinely distinct real client on its legacy invoice (RARE BASICS, TIC BEVERAGES, AYURVEDA HOUSE, SISHA GREEN TECH, Draco Brands, INTERNETWALE ONLINE SERVICES, Ride Zipo, ONROADS INDIA ASSISTANCE SERVICES â€" a strong signal these are 8 real billing relationships simply never onboarded, not duplicates or test data). vendor_gst_no left NULL for "Ride Zipo" whose legacy GSTIN is the literal placeholder "0000000000000", not a real number. company_name/branch_id/process_type/stream/cc_category/cc_type copied from each row's real active sibling in the same branch+process family (read live at authoring time); billing_client_name/vendor_gst_no/vendor_gst_state come from each code's own legacy invoice data, not the sibling. Idempotent via an explicit per-row existence check (cost_centre_code has no unique constraint in this schema). Applied directly against production 2026-08-19 with explicit user authorization ("create them") â€" verified live: all 8 rows exist with active_status=1; the 18 previously-blocked invoice staging rows were then re-resolved and flipped to validation_status='valid', and loaded into client_invoice for real via the same run-load.mjs runner used for the original cutover load.
  "migrations/1308_client_billing_tally_fields.sql", // Client Billing "Tally part" (2026-08-19): adds tally_head/client_tally_name (VARCHAR(255) NULL) to client_invoice and client_credit_note. Legacy db_bill.tbl_invoice froze cost_TallyHead/cost_client_tally_name per invoice at creation time â€" a real, actively-used accountant reference field (populated 5,483/10,797 and 7,240/10,797 of live legacy invoices), not automated Tally export (grepped every db_bill table for a "tally" column â€" none exists; this is purely manual-entry reference data, and stays that way here). client_invoice/client_credit_note had no equivalent column, and the 2026-08-19 historical cutover never carried the legacy value forward even though it was already captured verbatim in client_invoice_migration_staging.src_cost_tallyhead/src_cost_client_tally_name (the original 106-column extraction, migration 1304/1305) â€" this migration adds the columns; a separate one-off UPDATE (not itself a migration, since it targets already-loaded rows rather than schema) backfilled all 10,727 migrated invoices from that existing staging data (5,414/7,239 populated post-backfill, proportionally matching legacy). client_credit_note is NOT backfilled â€" tbl_credit_note never had a TallyHead column at all; new credit notes going forward resolve it live from cost_centre_master (client-billing-credit-note.service.ts). createProforma/createCreditNote (client-billing.service.ts/client-billing-credit-note.service.ts, same commit) now snapshot cost_centre_master.tally_head/billing_client_name at creation time for every new record, matching legacy's own frozen-snapshot behaviour rather than a live join. information_schema-guarded PREPARE/EXECUTE idiom, matching every prior migration in this module. Two nullable VARCHAR(255) columns on tables holding real data (client_invoice=10,727 rows, client_credit_note=139 rows) â€" zero backfill cost from the ALTER itself. Applied directly against production 2026-08-19 with explicit user authorization â€" verified live: all 4 columns exist, backfill counts confirmed exact.
  "migrations/1309_client_billing_unmapped_legacy_cost_centre.sql", // Client Billing Historical Cutover final resolution (2026-08-19): creates ONE explicitly-flagged placeholder cost_centre_master row (LEGACY-UNMAPPED-VODAFONE-2015-17) so the 67 remaining unresolvable invoices (2015-16/2016-17, predating the current cost-centre numbering scheme) can load and become visible instead of sitting invisible in staging indefinitely. NOT a guess at the specific historical cost centre â€" that was tried (client+branch+process-family matching, 7 of 10 legacy codes resolved to exactly one candidate) and explicitly rejected after a date-continuity check disproved it: the candidate for CM/FLD/KNL/036/037 turned out to have its own invoices dated the same week in Nov 2015 as the rows being matched to it, proving they were concurrently-active DIFFERENT cost centres for the same client/branch/process, not one renamed into the other. What IS known with certainty (all 67 rows are the same client, Vodafone Mobile Services Ltd., confirmed live) is stated honestly in the placeholder's own billing_client_name; which specific sub-team is stated as explicitly UNKNOWN rather than guessed. active_status=0/status='closed' â€" never a target for a new invoice going forward. Each invoice's own tally_head/client_tally_name (migration 1308) was already correctly backfilled independent of cost_centre_id. Applied directly against production 2026-08-19 with explicit user authorization ("handle it") â€" verified live: placeholder row exists, all 67 rows resolved and loaded (client_invoice now 10,794 rows). Separately (same session, not its own migration â€" targets already-loaded staging rows, not schema): the 5 remaining unresolvable credit notes were resolved for real using evidence found in their own free-text description field (which named the actual invoice for 3 of 5; the other 2 matched via cost-centre+amount+month agreement to the same invoice) â€" credit-note staging now 144/144 valid, client_credit_note fully loaded at 144 rows, 0 broken invoice_id FKs.
  ...
  // block from 6ea8ea5b) resolved 2026-08-20 while registering 1504 below â€"
  ...
  "1252_kpi_role_template_metrics.sql", // Loads 70 kpi_metric_master rows (definitions only â€" this file does not touch kpi_master_config or process_master) for a real HR/Ops-sourced role-based KPI target-setting spreadsheet (10 role templates: Admin, Process Manager, 3 Team Leader variants, Assistant Manager, Quality Analyst, Trainer, HR Recruiter, HR Executive/Sr Executive/Operations). family='custom'/category='custom' throughout â€" a standalone role-template set, not merged into the existing generic operational catalog. scoring_type is set explicitly (not left NULL) for every lower-is-better metric, since calculateMetricScore() treats a NULL scoring_type as a plain higher-is-better ratio regardless of the `direction` column â€" leaving it NULL on e.g. Attrition/Shrinkage would have silently inverted the score. scoring_type='boolean' used for every KPI whose only given target is a bare 1, confirmed live to have a working boolean-scoring branch (actual=1 -> 100, actual=0 -> 0) before relying on it. 7 metrics are shared/reused across multiple role configs (SELF_ATTENDANCE_BOOLEAN, ACPT_AUDITS, ZTP_AUDITS, GPI_SHRINKAGE, GPI_ATTRITION_LT10, GPI_CLIENT_SATISFACTION_ESCALATION, COST_CONTROL_ALL_ASPECTS) â€" one metric row, several kpi_master_config assignments, the schema's own designed-for pattern. Applied directly against production 2026-08-19 with explicit owner authorization after live review of the exact designation mapping, target/weightage conversion and every open item (2 name-inferred designations, the 100%-default convention for unquantified compliance-style KPIs, and 2 templates accepted at partial 75% weight pending real targets for the remaining unquantifiable items) â€" verified live via a rolled-back dry-run transaction before the real apply, then re-verified post-apply: kpi_metric_master at 93 rows (23 pre-existing + 70 new). The accompanying kpi_master_config rows (59, weightage-sum-validated per role/scope) and the Finfort process_master reactivation were applied in the same session as pure data, not part of this migration file. "Team Leader (Paytm)" and "Account Manager"/"HR Payroll" (no matching live process/designation) deliberately excluded; "HR Executive/Sr Executive/Operations" groups loaded as metric definitions only (no numeric target anywhere in the source for any of their 7 groups, so no config row could be assigned).
  "1504_leave_encashment_rate_config.sql", // F&F Phase 1 compute engine (ff-compute.service.ts): INSERT IGNORE seeds the leave_encashment_day_divisor statutory_config key INACTIVE (is_active=0) â€" a placeholder value only, so the key is discoverable but leave-encashment stays pending_configuration until a payroll owner reviews and activates a real approved divisor. No guessed rate is live from this migration alone.
  "1505_leave_encashment_tax_exemption_config.sql", // F&F Phase 2 TDS true-up (ff-compute.service.ts's resolveTdsTrueUp): INSERT IGNORE seeds leave_encashment_tax_exemption_limit statutory_config key INACTIVE (is_active=0) â€" placeholder only. Until reviewed/activated by the payroll/tax owner, the true-up treats leave encashment as fully taxable (conservative default), never guessing a s.10(10AA) exemption figure.
  "1506_exit_clearance_task_missing_migration.sql", // Reconstructs exit_clearance_task's missing CREATE TABLE â€" the table has been live in production and read/written by 20+ files (exit-intelligence.service.ts, exit.routes.ts, ff-approval-guard.compat.routes.ts, reporting adapters) with no CREATE TABLE anywhere in sql/, only its column list surviving in schema-snapshot.json. CREATE TABLE IF NOT EXISTS, structure copied verbatim from SHOW CREATE TABLE against production 2026-08-20 (same columns/enums/indexes, no FKs â€" matching the live table, which has none either).
  "1501_apr_multi_server_config.sql", // Seeds 3 GPI VICIdial server configs (apr_server_gpi01/gpi02/gpi5) into integration_config for apr-vicidial-sync.worker.ts, INACTIVE (active_status=0) with no credentials â€" those go through the encrypted external-DB credential screen separately. Plain INSERT ... ON DUPLICATE KEY UPDATE, no ALTER, no syntax issue. Applied out of band; verified live 2026-08-20: all 3 rows exist.
  "1502_apr_source_column.sql", // Adds source ENUM('sync','manual')/uploaded_by/upload_batch_id to apr, plus idx_apr_source â€" lets a manual upload be protected from being silently overwritten by the sync workers. Already correctly information_schema-guarded. Applied out of band; verified live 2026-08-20: all 3 columns + index exist.
  "1507_roster_daily_assignment_updated_at.sql", // Adds roster_daily_assignment.updated_at (DATETIME, DEFAULT/ON UPDATE CURRENT_TIMESTAMP, matching created_at's own type). roster.governance.service.ts's shift-reassignment path (Task 11 amendment workflow, commit 1ab6cbae) writes `updated_at = NOW()` against a column that has never existed â€" confirmed live before this migration: 0 such columns, every reassignment call throwing ER_BAD_FIELD_ERROR. Applied live 2026-08-20 with explicit approval; verified: column exists with the correct type/default.
  ...
  "1548_unlinked_grn_review_page_access.sql", // Registers FINANCE_UNLINKED_GRN_REVIEW (/finance/unlinked-grn-review) in page_catalog + role_page_access (super_admin/admin/finance_head/accounts_head, view+export only). Added alongside the frontend route/nav wiring in the same commit this time â€" see the annual-budget-summary lesson (migration 1537 existed on disk for a day before anyone added it to this manifest, leaving the page unreachable). Purely additive.
  "1549_manager_wfm_quality_dashboard_access.sql", // Grants manager role view access to WFM_DASHBOARD and QUALITY_DASHBOARD in role_page_access. Matches dashboardAccessRegistry.ts allowedRoleKeys â€" both gates must agree. Reactivates an existing QUALITY_DASHBOARD row (active_status=0 from 2026-07-25 RBAC cleanup) and adds the missing WFM_DASHBOARD grant. Purely additive (INSERT ... ON DUPLICATE KEY UPDATE, no schema changes).
  ...
  "1554_workforce_mandate_alert_threshold.sql", // Adds alert_threshold_pct DECIMAL(5,2) DEFAULT 80.00 to workforce_mandate. Required by hc-gap-alert.cron.ts daily job â€" fires when coverage_pct drops below this threshold. ADD COLUMN IF NOT EXISTS, backward-compatible; existing rows default to 80%.
  ...
  "migrations/1558_helpdesk_ticket_raised_by.sql", // Adds helpdesk_ticket.raised_by_user_id + resolved_by_user_id (both CHAR(36) NULL, no FK, matching assigned_to on the same table) and idx_helpdesk_ticket_raised_by. The maker/checker pair for the separation-of-duties guard added to POST /tickets/:id/resolve in the same commit: helpdesk had no occurrence of "maker" or "checker" anywhere, so one holder of one HELPDESK_ADMIN_ROLES role could raise a ticket on behalf of an employee, self-assign it via /take and resolve it, with the acting user surviving only in sensitive_action_log's change_summary JSON â€" telemetry that writeSensitiveActionLog is explicitly allowed to drop. No backfill: helpdesk_ticket holds 4 rows, all INSERTed in the same second on 2026-06-01 (seed data), and module_key='HELPDESK' in sensitive_action_log has only TICKET_ASSIGNED (3) and TICKET_ESCALATED (1) â€" TICKET_CREATED/TICKET_RESOLVED/TICKET_TAKEN have never been written, so no ticket has ever gone through this API. Those 4 rows keep raised_by_user_id NULL and the guard treats NULL as "raiser unknown" and lets the resolve through rather than inventing a failure on rows that predate the column. information_schema-guarded PREPARE/EXECUTE, not ADD COLUMN IF NOT EXISTS (unsupported on MySQL 8; this repo has recorded migrations applied while their DDL did nothing â€" see 1304/1305). Collation stated explicitly: helpdesk_ticket and auth_user are both utf8mb4_unicode_ci, verified live 2026-08-24. Additive and idempotent; registered but NOT applied by hand â€" applies at the next backend restart like every other manifest entry.
  ...
  "1633_exit_pass_qr_token.sql", // Phase 4 of Asset & Material Exit Pass: the QR that 1538/1539/1540 each explicitly deferred ("live QR token validation"). Those phases left the module HALF-WIRED â€" 1539 added exit_verification_method ENUM('qr','manual') and exit-pass.routes.ts has rejected anything but those two values since Phase 2, yet nothing could produce 'qr': the print layout drew no QR and NativeExitPassVerify.tsx hardcoded method:"manual", so the enum's 'qr' branch was unreachable in production. Adds ONE nullable CHAR(64) + a unique index. Stores a sha256 HASH, never the token â€" the convention 409_visitor_management_foundation.sql set with tracking_token_hash (visitor.security.test.ts asserts the raw token is never columned), so a leaked backup yields no working gate credential. The raw token is stored NOWHERE and re-derived from HMAC(secret,'exitpass:v1:'||id) in exit-pass.qr.ts, which is what makes a REPRINT work: the visitor flow can be hash-only because its token is emailed once, but a gate pass gets reprinted and a one-way token would leave every reprint with a dead QR. The token proves the PHYSICAL PASS WAS PRESENTED and is NOT an authorization credential â€" authz stays requireAuth + security role + status='approved', so a scanned token alone can never verify an exit. Single-use is free from the existing state machine (exit verification moves status off 'approved', so a re-scan reads 'already_used'). Pre-existing approved passes get NULL and keep working via manual entry; the app backfills a hash on first print. Purely additive on a table this project created in 1538. No DROP, no DELETE.
  ...
  "1641_create_client_portal_users.sql", // Seeds one client_user row per client that has mapped processes, using placeholder emails (portal-{code}@mcnhrms.teammas.in). Idempotent INSERT â€¦ SELECT WHERE NOT EXISTS. Numbered 1641 â€" deliberate collision with 1641_deactivate_test_slabs; manifest tracks by full filename so both run independently.
  "1641_deactivate_test_slabs_and_deduplicate_packages.sql", // Data-quality: deactivates the three test/demo salary slabs (TD-SLAB-182710, TD-SLAB-113934, TD-SLAB-182842, all seq_order=999) that were visible in the production Payroll Masters UI but are not valid business slabs. Also removes exact-duplicate salary_package rows (same band_id + cost_centre_code + ctc_monthly) that have no salary_structure references â€" e.g. Band F / BSS / OB / Noida / 592 appearing twice at â‚¹15,000 in the NOIDA packages list. Only orphaned duplicates are deleted; the canonical (older) row is preserved. No schema changes.
  ...
  "1663_deduction_types_tds_upload.sql", // Seeds four missing deduction codes into payroll_deduction_type: SHORT_COLL (Short Collection), INSURANCE, PROF_TAX (Professional Tax), LEAVE_DED (Leave Deduction). ON DUPLICATE KEY UPDATE â€" safe to re-run. Applied to production 2026-09-03.
  "1664_social_feed_page_access.sql", // The page_catalog row and view grants for SOCIAL_FEED (/social-feed, "MAS Connect"). The feed shipped 2026-08-01 with its route, backend module and sync cron but with NO catalog row and no grants anywhere in backend/sql/ â€" verified live 2026-09-03, page_catalog holds zero SOCIAL% rows â€" so <Gate pageCode="SOCIAL_FEED"> denied the entire organisation and the page has been unreachable since the day it shipped. Same failure 1066, 1129 and 1632 each describe: access.service.ts builds its permission map from ACTIVE page_catalog rows, so a page_code with no row can be held by nobody. Grants are DERIVED from MY_PROFILE's live grant set rather than listed, because the route is `<ProtectedRoute><Gate>` with no role array (any authenticated user) and the feature's own commit says "for all employees" â€" whoever can open their own profile can open the company feed, on production and on a rebuilt database alike, with no list here to fall behind the role set. View-only: the editable half (/social-feed/admin) is guarded by its own role array in the route and carries no page code, so nothing here grants create/edit/delete/export. Purely additive and idempotent â€" two INSERT ... ON DUPLICATE KEY UPDATE statements, no ALTER, no DROP, no DELETE, no FOREIGN KEY, and no other page's grants are touched. NOT YET APPLIED to production: it runs on the next deploy, which is also what makes the page appear for the first time.
  "1665_incentive_types_canonical_list.sql", // Syncs incentive_master to the owner-approved canonical list (16 types). Adds 10 missing types, renames Overtime Allowance â†' Overtime Incentive. ON DUPLICATE KEY UPDATE â€" safe to re-run.
  "1666_seed_workforce_mandate_from_billing.sql", // Fills workforce_mandate, which has held ZERO rows since it was created â€" the reason portal.kpi-engine.service.ts, 1647_portal_kpi_config.sql, /api/manpower-risk/cost-center, process-pnl.service.ts's getWorkforceMap() and hc-gap-alert.cron.ts all read nothing today. Source is cost_center_billing_config (db_bill-synced): 258 rows, 38 with current_mandate > 0 totalling 896 seats, buffer_pct 10.00 throughout â€" the owner's chosen source of truth, and the same rows now drive both the HR Headcount & Shortage board and the process P&L workforce map. Mandate is per COST CENTRE and consumers key on process+branch, so process resolves through a ladder measured against production: cost_centre_master.process_id (672 seats), else the dominant process of that cost centre's active staff (2), else another cost centre of the same BILLING client in the same branch (133 â€" Godfrey Philips BSS/OB/AHMH-JD/474, flagged inactive with no process_id while 148 people work on the process), else an exact process_name match (10). The remaining 17 cost centres / 89 seats are deliberately NOT seeded â€" no process can be honestly derived, so the API surfaces them as unmapped mandate for HR to fix. effective_from is the first of the CURRENT FINANCIAL YEAR, not today, because P&L reads this table with effective_from <= period_end; it is fixed rather than CURDATE()-derived because the unique key includes it and a moving value would insert a second active row per process on every run. ON DUPLICATE KEY UPDATE refreshes mandated_hc ONLY and never buffer_pct, which HR edits per process. Verified: EXPLAIN plans clean on 8.0.42, the SELECT yields 20 process rows / 807 of 896 seats. Purely additive.
  ...
  "1672_attendance_rule_config_cost_centre.sql", // Owner requirement 2026-09-05: build an attendance exception by Branch -> the branch's own cost centres ("call centres") -> Designation. attendance_rule_config (044) already scopes by designation_id/process_id/branch_id but has no cost_centre_id, even though cost centre â€" not process â€" is the unit employees are actually tagged to: 1,102 of 1,106 active employees (99.6%) carry cost_centre_id, all 1,106 carry branch_id, and cost_centre_master.branch_id is populated on 934 of 937 rows (99.7%), verified live 2026-09-05, versus only 24 of 937 cost centres linking to any process at all. processEmployee() in attendance-engine.service.ts already reads e.cost_centre_id and discarded it before this change. Adds one nullable cost_centre_id column (AFTER process_id), one index, and widens scope_type with four new values appended after 'global' (cost_centre, cost_centre_designation, branch_cost_centre, branch_cost_centre_designation) â€" existing enum positions and all 30 live rows (cost_centre_id NULL, resolves as "any cost centre") are untouched. Purely additive, information_schema-guarded. Applied to production 2026-09-05: cost_centre_id column, index and widened enum verified live; all 30 existing rows still have cost_centre_id NULL, unaffected.
  "1673_upload_batch_job_heartbeat.sql", // A running import/approval is tracked in an in-process Map (jobMap in bulk-approval-async.ts, jobs in batch-job.ts) while the batch is marked 'importing'/'approving' in the DATABASE â€" so a deploy, pm2 restart or OOM erases the tracker and orphans the batch with nothing left to finish it. Measured live 2026-09-05: BATCH-1788604867017 sat 'importing' for 2.5 hours with 1,246 of 3,765 rows unprocessed; nothing retried it, nothing failed it, nobody was told. Adds two nullable columns (job_heartbeat_at, job_owner) so a running job can PROVE it is alive, letting a dead job be told from a slow one in minutes rather than the 30 the reaper must otherwise wait on updated_at alone (which moves only as rows complete). Plus one index on (batch_status, job_heartbeat_at) so the reaper sweep is not a full scan. Purely additive and information_schema-guarded: existing rows keep job_heartbeat_at NULL, which the reaper reads as "no heartbeat, fall back to updated_at" â€" exactly its current behaviour. Deliberately not used to auto-resume: an import that died for an unknown reason must not restart in a loop unattended, and approval batches move pay.
  "1674_communication_send_block.sql", // Re-registered 2026-09-07, together with restoring the file itself. It shipped in 8be3d102 and adds send_blocked/block_reason/blocked_at/blocked_by to communication_provider_config â€" the genuine per-channel kill switch that providerFactory consults before every SMS, email and WhatsApp send, distinct from is_enabled, which does not stop sending. An unrelated payroll commit (c75eb26d) deleted the .sql from the tree; a later commit then unregistered it here on the reasonable-looking grounds that the file was missing, which treated the symptom. The four columns exist in this database, so nothing is broken today â€" but they existed only because the migration ran before the file was lost, and a rebuilt database would have had none of them while send-block.ts went on querying send_blocked. The restored file is byte-identical to the committed version and its ALTER is information_schema-guarded PREPARE/EXECUTE, so replaying it anywhere it already ran is a no-op.
  "1675_onfido_process_dashboard_page.sql", // Onfido process KPI/Quality/Operations dashboard (new page, reads onfido_db â€" a separate physical database the Bulk Upload Hub's 7+ Onfido report templates populate). This migration only touches mas_hrms's access-control tables: one page_catalog row (ONFIDO_PROCESS_DASHBOARD, path /onfido-process/dashboard) and role_page_access grants for super_admin/admin/ceo/coo/manager/process_manager/team_leader/branch_head/qa/quality_analyst/wfm. Purely additive, INSERT IGNORE on both tables.
  "1676_process_kpi_dashboard_page.sql", // Registered 2026-09-07. Written earlier but never added to this manifest, so it never ran: PROCESS_KPI_DASHBOARD had no page_catalog row in production, which made /performance/process-kpi-dashboard invisible in /settings/access-control and therefore impossible to grant to anyone â€" no catalog row means no row to tick, and canViewPage() only lets super_admin past on its isSuperAdmin short-circuit. INSERT IGNORE on page_catalog and role_page_access, so registering it now is safe whether or not it was applied by hand.
  "1677_process_metric_actual.sql", // Registered 2026-09-07 alongside 1676/1678 â€" same KPI set, same omission from this manifest. Fully information_schema-guarded and idempotent, so it is a no-op if the table was already created by hand (see the 1644-1646 precedent, applied manually 2026-09-03).
  ...
  "1707_bank_ledger_report_page_access.sql", // Registered 2026-09-09. Payment Voucher System Phase 2. Registers FINANCE_BANK_LEDGER (/finance/bank-ledger), the Credit/Debit report page (PRD Â§6.1/Â§6.2), read/export only. Data-only: no schema, no column, no row deleted.
  ...
  "1756_bank_exception_auto_assign_event.sql", // Registered 2026-09-11. Seeds the bank_exception_invalid_assigned notification event (enabled/live), per the 1022 registry convention, so bank-exception-auto-assign.service.ts's auto-assign-to-branch-payroll-HR pass has an event to call when an INVALID/MISSING/CONFLICT bank exception gets its first owner. Used 1756 rather than 1755 because a concurrent session had already claimed 1755 (wfm_notification_rule) for an unrelated feature at the same time.
  "1757_roster_command_center_console_page_codes.sql", // Registered 2026-09-16 â€" found unregistered by a live audit, 4 days after being written and after the console it gates was reportedly "deployed and live-tested" on 2026-09-12. ANOTHER migration, 1757_bellavita_masmis_uploaders_real_headers.sql, silently claimed the 1757 slot (a concurrent-session number collision, same failure mode as the 1755/1756 note above, just not caught at the time) and is the one recorded in schema_migrations for filename "1757..."; neither 1757 file was ever in this array, so this one never ran. Live-confirmed impact: page_catalog had ZERO of the 8 WFM_ROSTER_* console page codes and role_page_access had ZERO rows for any of them â€" every role except super_admin (which bypasses page_catalog checks entirely via useUserRole.ts's `isSuperAdmin ||` shortcut) has been unable to see 7 of the Roster Command Center's 8 tabs (only Live Monitoring, migration 1766, existed) since the console shipped. This migration is unchanged from how it was written; only the registration was missing. Purely additive (ON DUPLICATE KEY UPDATE throughout, matches each tab's own backend requireRole() exactly, invents no new role).
  "1758_grn_accounts_head_approval_stage.sql", // Registered 2026-09-12. Adds 'accounts_head_approved' to grn_request.status and the accounts_head_reviewed_by/_at/_note columns grn.service.ts and grn-smart.service.ts's review() now read and write â€" the real Accounts Head approval gate inserted between Branch Head and Finance Head (owner ruling, 2026-09-12; GRN was a 2-stage chain before this). Additive only: every existing status value and column is untouched.
  "1760_exit_absconding_since.sql", // Registered 2026-09-12. Adds exit_request.absconding_since (nullable DATE) + index. The New Exit form has always demanded "Absconding Since" as a mandatory field and discarded it â€" nothing sent it, the create schema would have rejected it, and no column existed to hold it; its only effect was auto-filling the proposed last working day as that date PLUS SEVEN DAYS, labelled "grace period". Owner ruling 2026-09-12: the 7 days is how long the company waits before deciding, not time the employee is paid for, so the leaver's paid-through date was landing a week after they stopped working â€" and that date feeds payroll's employment-end-date resolver, which prorates the final month. Kept separate from last_working_day_confirmed on purpose: this is the attendance fact (last day present, never changes), that is the HR decision (can be revised). Additive and guarded; existing rows keep NULL, which reads correctly as "not an absconding, or predates this column".
  "1762_fnf_transfer_batch.sql", // Registered 2026-09-12. Creates fnf_transfer_batch + fnf_transfer_batch_item + fnf_transfer_import â€" the Full & Final settlement's OWN bank-transfer batch (owner ruling 2026-09-12: separate from monthly salary, same bank-file machinery and approvals). Mirrors salary_transfer_batch/_item's shape and its open_flag generated-column dedup trick, keyed on full_final_calculation_id instead of run_id because F&F has no payroll run to key against. Additive only; no existing table touched.
  "1764_exit_clearance_task_attachment.sql", // Registered 2026-09-12. Adds attachment_url (nullable VARCHAR 700) to exit_clearance_task, supporting the WFM "Client ID deactivation" clearance step where WFM attaches a screenshot/email confirming the agent's client-system ID was deactivated. All existing tasks and the other seven default clearance tasks are unaffected (column is nullable with no default constraint).
  "1764_noc_signatory_not_mandatory.sql", // Registered 2026-09-12. Adds is_mandatory to noc_signatory_template and noc_signatory (defaulting all stages to 1/mandatory so nothing changes silently), then sets Accounts and Finance stages to 0 (non-mandatory). Business owner ruling 2026-09-12: those two stages are trackable but must not block NOC completion the way TL/PM/HR/Branch Manager/IT/Admin still do. Both tables default 1 so future-seeded stages are mandatory unless explicitly overridden.
  "1700_noc_case_opened_event.sql", // Registered 2026-09-14. Seeds the noc_case_opened notification event so the NOC case creation flow has an event to trigger on open.
  "1759_roster_conflict_resolution_evidence.sql", // Registered 2026-09-14. Adds resolution evidence columns to wfm_roster_conflict_log (resolution_action, resolution_remarks, resolved_at, resolved_by, resolution_status).
  "1761_esic_registration_automation.sql", // Registered 2026-09-14. Creates esic_registration_case and esic_registration_event for ESIC registration workflow automation.
  ...
  "1765_portal_sessions.sql", // Registered 2026-09-14. Creates portal_sessions table used by the HR Admin portal-impersonation endpoint (INSERT was failing at runtime â€" table did not exist).
  "1766_roster_live_monitoring_branch_head_wfm_only.sql", // Registered 2026-09-14. Owner ruling: Roster Console's Live Monitoring tab (WFM_ROSTER_LIVE_MONITORING) is branch_head + wfm only. Discovered live-checking this: none of 1757's 8 page codes had actually been applied to production (missing from page_catalog entirely, not just role_page_access) despite 1757's own frontend prerequisite already being deployed â€" so this migration registers the page_catalog row itself (idempotent) rather than assuming 1757 ran, then grants branch_head+wfm and soft-revokes admin/hr if present (active_status=0, same pattern as 1661). Matching backend change: roster-intelligence.routes.ts splits a new LIVE_MONITORING_ROLES constant out of the pre-existing ADMIN_ROLES (which stays untouched â€" it also gates the unrelated /manager-digest self-service check) and applies it to the 5 endpoints this page code gates, in the same commit â€" role_page_access alone would leave the tab hidden while the endpoints still answered admin/hr directly. The other 7 tabs' page codes are still unregistered on production; flagged to the owner, not fixed here (out of scope for this ask).
  // 1767_onfido_external_audit_add_total_columns.sql â€" REMOVED FROM MANIFEST (moved to knownUnlisted).
  ...
  "1771_exit_clearance_dedicated_pages.sql", // Registered 2026-09-15. Registers PROVISIONING_MANAGER_HANDOVER / PROVISIONING_HR_EXIT / PROVISIONING_PAYROLL_EXIT / PROVISIONING_TRAINER_EXIT in page_catalog + role_page_access, so manager/hr/payroll/trainer (who own 5 of the 9 exit_clearance_task areas but had no dedicated page â€" only a read-only drawer) can reach their new Exit Clearance queue pages. admin/wfm's own clearance tasks instead gained a new section on their existing PROVISIONING_ADMIN/PROVISIONING_WFM_ALIGNMENT pages, needing no new page_catalog rows. Purely additive (ON DUPLICATE KEY UPDATE throughout, can_create=0 everywhere â€" task generation stays admin/hr-only).
  "1772_exit_clearance_it_role_retarget.sql", // Registered 2026-09-15. Owner ruling: "IT access closure" clearance task moves from owner_role='admin' to 'it' â€" it's IT's own work and was never reachable by a pure IT-only account. Backfills the 8 currently-open rows only; cleared/waived history untouched. Matching code change in exit-intelligence.service.ts (new tasks already create with owner_role='it').
  "1773_branch_wfm_role_and_page_access.sql", // Registered 2026-09-15. Adds branch_wfm to workforce_role_catalog (so it appears in Access Control UI) and grants wfm + branch_wfm can_view=1 on OPERATIONS_DASHBOARD, PROCESS_KPI_DASHBOARD, PROCESS_DATA_SOURCE in role_page_access.
  "1774_trainer_clearance_removed.sql", // Registered 2026-09-15. Owner ruling: trainer clearance ("LMS and certification closure") dropped from the exit process entirely. Waives any still-open trainer/lms clearance task (cleared/waived history untouched) and removes the PROVISIONING_TRAINER_EXIT page + its role_page_access grants from migration 1771. Matching code change in exit-intelligence.service.ts (task no longer created for new exits).
  "1775_pnl_seat_billing_line.sql", // Registered 2026-09-15, owner-approved ("approved for mas_hrms"). CREATE TABLE IF NOT EXISTS pnl_seat_billing_line â€" per-cost-centre/LOB seat billing lines that the Live P&L seat-rate revenue estimate reads (pnl-seat-billing.service.ts), written only by the P&L Configuration > Seat billing tab and audited. Purely additive; every string column has an explicit utf8mb4_unicode_ci. Until this runs the Seat billing tab is read-only (writes answer 503) and the estimate uses last-invoice lines only.
  "1776_wfm_process_operations_access.sql", // Registered 2026-09-15. Grants wfm + branch_wfm can_view/can_export on PROCESS_OPERATIONS (role_page_access), because the Onfido dashboard moved inside Process Operations and wfm held only ONFIDO_PROCESS_DASHBOARD. Rows stay per-viewer scoped by readableProcessIds (user_assignment_scope). Purely additive, replay-safe (uq_role_page + ON DUPLICATE KEY UPDATE). 1775 is parked, not skipped by accident.
  "1777_branch_head_wfm_page_access.sql", // Registered 2026-09-16, owner-requested ("Branch Head should be able to see all [WFM pages]"). Grants branch_head view access to WFM_DASHBOARD, WFM_ATTENDANCE_DASHBOARD, WFM_ROSTER, WFM_PLANNING_RULES, WFM_SLOT_REQUIREMENTS, WFM_WEEKOFF_DAY_RULES, WFM_WEEKOFF_FAIRNESS, ATTENDANCE_DISPUTES, ATTENDANCE_REGULARIZATION, ROSTER_MASTER; reactivates a previously-disabled WFM_AUTO_ROSTER grant. Matching code changes: dashboardAccessRegistry.ts allowedRoleKeys, workforce.routes.tsx ProtectedRoute roles=, wfm.routes.ts GET /weekoff/fairness-scores requireRole, rbacPageMatrix.ts LIVE_IMPORTED_PAGE_CODES. Deliberately excludes ATTENDANCE_LOOKUP (surfaces payslip/payroll data â€" separate decision), TEAM_ATTENDANCE (deactivated org-wide) and WFM_ROSTER_MANAGER_QUEUE (no mounted route). Purely additive, replay-safe (WHERE NOT EXISTS on inserts, idempotent UPDATE).
  "1778_attendance_lookup_branch_head_payroll_hr.sql", // Registered 2026-09-16, owner-specified access list ("Attendance Lookup - branch WFM, Branch head, Branch Payroll HR, Payroll head (All branch) and super admin (All branch)"). branch_wfm/payroll_head/super_admin already held the grant; adds branch_head and payroll_hr. Row scoping (own branch vs all-branch) comes from each user's existing user_assignment_scope row via buildScopeWhereClause, not from this migration. Matching code changes: employee.routes.ts hr-hub* routes' requireRole + buildScopeWhereClause allowedRoles, AdminAttendanceView.tsx ALLOWED_ROLES, rbacPageMatrix.ts. Purely additive, replay-safe (WHERE NOT EXISTS).
  "1779_live_monitoring_process_branch_wfm_access.sql", // Registered 2026-09-16, owner-specified access list ("branch WFM, Branch head, Process manager (respective process), Operations manager (respective process) and super admin (All branch)"). branch_head already held WFM_ROSTER_LIVE_MONITORING; adds branch_wfm, process_manager, operations_manager (super_admin needs no row â€" auto-granted every active page). Matching code change: roster-intelligence.routes.ts/service.ts now resolve each caller's real user_assignment_scope and filter detectUnplannedAbsences/generateManagerDailyDigests by branchIds/processIds â€" previously these ran completely unscoped even for the roles that already held the page (a pre-existing gap this fix also closes for branch_head), except wfm/super_admin which keep the unrestricted access the 2026-09-14 ruling (migration 1766) gave them. Purely additive, replay-safe (WHERE NOT EXISTS).
  "1780_employee_master_snapshot_salary_start_joining_month.sql", // Registered 2026-09-16, owner-requested (MAS63459/RAVIKAR MISHRA CTC investigation: "add - Salary start date column and Joining Month column also"). ALTER TABLE employee_master_snapshot ADD COLUMN salary_effective_date TEXT NULL, joining_month TEXT NULL â€" the two new Employee Master Export report columns (report-catalog.ts, SNAPSHOT_COLUMNS in employee-master-snapshot.service.ts) need matching columns on the cached snapshot table before the next refresh can insert into them. Same change also fixed Gross/CTCOffered/NetInHand on this report to read salary_component_assignments/employee_salary_assignment (what Payroll Head's final-salary review actually writes to) instead of the stale employee_salary_snapshot mirror â€" no schema change needed for that half. Purely additive, idempotent via individual INFORMATION_SCHEMA.COLUMNS guards. Registration line restored here (a concurrent session's own SQL file and manifest comment â€" see its own header â€" content untouched) after the pre-push migration-manifest-guard test flagged it as unregistered; left out of this commit's own diff intent, re-added only to keep the shared manifest array internally consistent so the guard can pass.
  "1781_roster_live_monitoring_admin_hr_regression_fix.sql", // Registered 2026-09-16. Corrective fix for a race this same session caused minutes earlier: registering 1757_roster_command_center_console_page_codes.sql ran slightly before that file's own stale Live Monitoring block (granting admin/hr) was edited out, so a backend picked up the not-yet-fixed content and re-inserted admin/hr into WFM_ROSTER_LIVE_MONITORING with active_status=1 â€" directly contradicting the 2026-09-14 owner ruling (migration 1766, "branch_head + wfm only") that 1757's own text had already been superseded by. 1757 is now tracked applied and won't re-run its corrected content, so this re-revokes admin/hr directly. Confirmed via role_page_access.created_at (2026-09-16T07:53:58Z) exactly matching 1757's applied_at.
  "1782_notification_template_employee_identity.sql", // Registered 2026-09-16, owner directive (every email naming a specific employee must show Employee Code, Process/LOB and Reporting Manager name). Updates the stored Handlebars templates for regularization_decision/roster_published/shift_changed/payslip_ready in communication_template to render process_name/reporting_manager_name the notification producers already started passing into the data payload; shift_changed had no employee-identifying row at all. Each UPDATE is a REPLACE() on an exact, verified-unique substring, naturally idempotent. Another concurrent session's own SQL file and manifest comment â€" content untouched â€" registered here only to keep the shared manifest array internally consistent for the pre-push guard, same as the 1780 entry above.
  "1783_enable_provisioning_overdue_reminder.sql", // Registered 2026-09-16, owner directive (every pending/actionable item must generate a reminder if unresolved). Enables the provisioning_overdue notification type in communication_notification_types and inserts a backfill_floor row so the existing notifyOverdueProvisioning() cron producer activates. Another concurrent session's own SQL file â€" content untouched â€" registered here only to satisfy the shared migration-manifest guard.
  "1784_leave_approval_overdue_reminder.sql", // Registered 2026-09-16, owner directive (leave approvals pending >N hours must send a reminder to the approver). Adds reminder_count INT NOT NULL DEFAULT 0 and last_reminder_at DATETIME NULL to leave_request (both idempotent via INFORMATION_SCHEMA guard); inserts leave_approval_overdue notification type; inserts backfill_floor row. Another concurrent session's own SQL file â€" content untouched â€" registered here to keep the shared MIGRATION_MANIFEST consistent and allow schema-snapshot to be updated with the two new leave_request columns so schema-column-refs guard passes.
  "1785_leave_exit_cc_wfm_payroll_not_hr.sql", // Registered 2026-09-16, owner directive (leave-approval and exit "employee has left" emails must not copy generic branch HR â€" copy the branch's WFM and Payroll HR instead). Pure recipient_spec UPDATE, no schema change: resignation_submitted/revoked and exit_lwd_approaching swap Cc from branch_hr to wfm_chain+payroll_hr; leave_pending_branch_head drops branch_hr from To (was a joint approver alongside branch_head, confirmed with owner before touching it) and adds wfm_chain+payroll_hr to Cc.
  "1786_cost_centre_process_mapping_backfill.sql", // Registered 2026-09-16, owner directive ("you have cost center id and each cost center id has process id mapping then do it"). Backfills cost_centre_master.process_id (never overwriting an existing value) from two sources already on file: Pass 1 matches the legacy process_name_bill against process_master.process_name exactly, branch-scoped (resolves 2 rows live). Pass 2 uses employees.process_id where every employee on a cost centre agrees on one process (resolves 54 rows live). Cost centres with mixed-process employees (138) or none at all (430) are deliberately left NULL â€" no confident single answer to assign. Companion code change: grn.service.ts's createUnbudgetedDraft()/createDraft() now fall back to cost_centre_master.process_id when the budget line's own process_id is empty (which is nearly always, 4 of 790 lines), so newly-raised GRNs against a now-mapped cost centre get a Process instead of blank.
  "1787_pnl_employee_cost_centre_override.sql", // Registered 2026-09-16, owner-confirmed (cost centre BSS/BO/NOIDA-2/577 is a back-office pool that works entirely for BSS/BO/NOIDA-2/576/Onfido, understating 576's margin and overstating 577's cost). Renumbered from 1785 to 1787 â€" 1785 and 1786 were taken by another concurrent session's already-merged work (leave/exit CC notification swap, cost-centre/process backfill) by the time this was ready to push. CREATE TABLE IF NOT EXISTS pnl_employee_cost_centre_override â€" one row per employee, mapping their pay to a different cost centre for P&L attribution only; employees.cost_centre_id (roster/attendance/leave/payroll) is never touched. Read via LEFT JOIN + COALESCE in pnl-reconciliation.service.ts (readPayroll/readUnallocatedPayroll/exceptions) and ceo-overview.service.ts (peopleByBranch), so Live P&L, Insights, Trend and CEO Overview all pick it up the same way. Purely additive.
  // 1788 deliberately skipped â€" see knownUnlisted (db_masmis privilege gap).
  "1789_journal_entry.sql", // Registered 2026-09-17, on review branch only (owner decision: review via PR before merging). Drafted by another session against a read-only clone as a payment-voucher/GRN double-entry general ledger (journal_entry + journal_entry_line, journalService.post()/reverse()) â€" see docs delivered alongside: 00-architecture-plan/payment-voucher-double-entry-plan.md, per-task briefs/reports. This session applied it to a real checkout for the first time, found and fixed a real bug in journal.service.ts's paise-rounding (two amounts ~1 paisa apart could falsely "balance" â€" its own test now catches this), found and fixed a real regression the diff caused in grn-budget-audit-controls.contract.test.ts's mock sequencing, ran the FULL existing finance test suite (936 tests â€" not just the new ones) to confirm no other regressions, and ran the read-only verify-head-subhead-ledger-coverage.ts pre-flight check against production data: 4 active budget lines (~â‚¹6.95L) have no matching Ledger Head and would hard-refuse a Finance Head approval the moment GRN journal-posting goes live â€" must be fixed (add the Ledger Head, or correct the budget line's head/sub_head text) before merging this to main. Purely additive (CREATE TABLE IF NOT EXISTS); posts nothing until the accompanying grn.service.ts/payment-voucher.service.ts/bank-reconciliation-match.service.ts/tally-export.service.ts wiring in this same commit is live.
  "1790_grn_budget_line_ledger_head_alignment.sql", // Registered 2026-09-17, on review branch only, owner directive ("do fix it check db_bill you can get many answers from there") to close the 4 pre-flight gaps 1789's own comment names. Corrects finance_budget_line.head/sub_head text on 8 active budget lines (text only â€" no amounts/approvals/GRNs touched): a stray double space in "Generator  Hire" (Hiring Charges), a stale FY2025-26 Computer-Software-Cost vintage on a current-FY2026-27 budget line, and Furniture/Electrical "- COST" lines raised under the opex "Repairs & Maintenance" head that db_bill's own tbl_bgt_expensesubheadingmaster confirms were already recategorised to "Repairs & Maintenance Capex" there (HeadType A active vs the opex variant's HeadType B Status1=0/disabled) â€" see the migration's own header for the exact db_bill evidence per line. Re-verified 0 gaps remain via verify-head-subhead-ledger-coverage.ts after applying.
  "1791_historical_grn_ledger_head_alignment.sql", // Registered 2026-09-17, owner directive ("please complete all") ahead of the Phase 6 journal backfill. A dry run of backfill-journal-entries.ts --grn against all 82,486 candidate GRNs found 1,010 vendor GRNs (26 distinct head/sub_head combos) that would fail EXPENSE_LEDGER_NOT_FOUND â€" 1790 only covered active budget lines' text, not the free-text copy on grn_request itself. Corrects the 13 combos (889 of 1,010 rows) with exactly one matching active ledger row once paired with the right head â€" same wrong-head/Capex pattern as 1790, plus a few head/sub_head pairs swapped between sibling heads (Tour Expenses vs Tours Travelling & Conveyance, Fee & Subscription vs Finance Expenses/Freight & Cargo Charges, etc). The remaining 13 combos (121 rows, genuinely ambiguous self-referential text like head=sub_head="Communication & Connectivity") and 167 vendor GRNs with no vendor_id or vendor_name recorded are deliberately left for the backfill to report by name rather than guessed. Text only, scoped to grn_type='vendor' AND status IN the same consumed-status set the backfill script selects on.
  "1792_finance_ledger_reports_page_access.sql", // Registered 2026-09-17, owner directive ("please complete all"). Registers FINANCE_LEDGER_REPORTS in page_catalog/role_page_access for the new /finance/ledger-reports page (LedgerReportsPage.tsx â€" Trial Balance, Vendor Ledger, Head/Subhead Spend, all read directly off journal_entry_line). Roles match BANK_ACCOUNT_READ_ROLES exactly, same as every endpoint on ledger-reports.routes.ts already does. Entirely read-only: can_create/can_edit/can_delete/can_export all 0, no write action or export button exists on this page.
  "1793_grn_approval_email_notifications.sql", // Registered 2026-09-17, owner directive (GRN raise/approve/reminder emails with Branch/Amount/Head-Subhead/Date/Remarks). Inserts 3 notification_event_config rows (grn_submitted, grn_accounts_head_pending, grn_approval_overdue) + 3 branded communication_template rows; adds grn_request.reminder_count/last_reminder_at (INFORMATION_SCHEMA-guarded, not MariaDB's ADD COLUMN IF NOT EXISTS); seeds worker_config for grn-approval-reminder. See grn.notifications.ts and grn-approval-reminder.worker.ts (the latter's own ROLLOUT_AT constant excludes the pre-existing 162-GRN backlog from the reminder sweep).
  "1794_historical_imprest_grn_ledger_head_alignment.sql", // Registered 2026-09-17, owner directive ("keep fixing and building"), continuation of 1791. After the Phase 6 backfill finished (42,782 journal entries posted), a fresh live query of the remaining 604 non-salary un-journaled rows across 66 head/sub_head/grn_type combos found 12 combos (271 rows) that are the grn_type='imprest' counterpart of the exact wrong-head/Capex-reclassification pattern 1791 already fixed for grn_type='vendor' only. Same method: sub_head text matched case/whitespace-insensitively against active finance_expense_sub_head_master rows, applied only where exactly one candidate existed. The other ~333 rows are NOT a ledger-head problem at all â€" live-diagnosed as two separate data gaps (167 vendor GRNs with vendor_id NULL; 86 GRNs with amount_with_tax AND amount both NULL/zero) â€" and ~20 combos with no matching active ledger row, same "genuinely ambiguous, left untouched" treatment as 1791. Text only, scoped to grn_type='imprest' AND status IN the same consumed-status set the backfill script selects on.
  "1795_performance_indexes.sql", // Registered 2026-09-17. Composite indexes for the most-common query patterns from the 2026-09-17 perf audit. All CREATE INDEX â€¦ IF NOT EXISTS â€" safe to re-run. Targets: attendance_daily_record (employee_id+record_date), payroll_run_line (employee_id+month_year), leave_request (employee_id+status), salary_assignment (employee_id+effective_date), journal_entry_line (je_id+account_code).
  "1796_reginald_live_sales_shopify_columns.sql", // Registered 2026-09-17, owner report ("this uploader is missing for reginald men dashboard in process operation"). The REGINALD_ABANDONED_CART_SALES upload template's required_columns (Order id/Amount/LOB) matched only the original Google Form export, not the real file the owner has ("Reginald men - Live sales.xlsx", a direct Shopify order export with different headers) â€" every upload of the real file failed frontend validation outright (every column "Unknown", all 3 required columns "missing"). Updates required_columns to the new format (Shopify Order Name/Grand Total/Date), optional_columns to the union of both formats, and sample_row to a real-shaped new-format example. Matching code change in reginald-abandoned-cart-sales-bulk.service.ts accepts either header set (old tried first), so a historical-format re-upload still works. Purely additive to one row's column lists.
  "1797_reginald_live_sales_optional_columns_fix.sql", // Registered 2026-09-17. Corrective fix for 1796, found by live-uploading the real file through BulkUploadHub immediately after: csvHealthHasBlockingError() treats every required+optional column as a header that MUST be present (blank is fine, absent is not) â€" 1796's "union of both formats" optional_columns meant the new file (which naturally has none of the old format's 9 columns) was blocked with "CSV structure issue found". Narrows optional_columns to just the new format's own remaining columns; the old format stays supported at the service level (tried first) but is no longer declared as template columns, since it is not actively produced any more (last row via that path: 2026-09-09).
  "1798_journal_entry_branch_cost_centre_process.sql", // Registered 2026-09-17, owner directive ("Branch name, cost center, process names are also missing understand it should have depth information"). journal_entry (the double-entry ledger built earlier today) carried no branch/cost-centre/process dimension at all â€" Ledger Reports couldn't be sliced or drilled down by any of the three, even though grn_request.branch_id/cost_centre_id/process_id are always populated on the source record. Adds three nullable columns + indexes to journal_entry, backfills all 43,053 already-posted GRN-sourced entries from their own source GRN. Wired into journal.service.ts's post()/reverse() and grn-journal-posting.service.ts (grn.service.ts's SELECT * already carried these fields through). ledger-reports.service.ts/.routes.ts and LedgerReportsPage.tsx updated to filter Trial Balance/Head-Subhead Spend by branch/cost-centre/process and show them in the drill-down drawer. Payment Voucher's own journal posting (release(), wired earlier today, never yet exercised â€" payment_voucher has 0 released rows) deliberately left NOT wired to this dimension: a voucher can span multiple GRNs/cost-centres and payment_voucher itself carries no branch/cost-centre/process column, so guessing one would misattribute real spend â€" flagged as a follow-up needing a business rule, not guessed.
  "1799_payment_voucher_withdraw_status.sql", // Registered 2026-09-17, owner directive (CEO/CA compliance review, "no issues at all fully compliant"). A voucher had no self-service cancellation path: the raiser couldn't withdraw their own mistake before the CEO acted on it, and an approved-but-unreleased voucher had no stop mechanism at all. Adds a new terminal ENUM value 'withdrawn' (distinct from 'rejected' â€" different party, different meaning) plus withdrawn_by/withdrawn_at/withdrawal_reason columns. Backend withdraw() method (payment-voucher.service.ts): raiser can withdraw their own voucher while status='raised'; anyone who could release it (finance_head/super_admin) or the CEO who approved it can recall it while status='ceo_approved', before money moves. Not added to ACTIVE_VOUCHER_STATUSES anywhere (payment-voucher.service.ts, paymentVoucherStatus.ts, vendor-payment.service.ts) â€" a withdrawn voucher is dead, same as rejected.
  "1800_gs1_daily_actual_missing_columns.sql", // Registered 2026-09-17, owner report ("HRMS Issue and observation Reginald & GS1" â€" GS1 dashboards show no data). gs1_email_daily_actual/gs1_datakart_daily_actual were missing data_source/source_reference columns that migration 1769's own CREATE TABLE text declares and that the (rewritten, see gs1-email-daily-bulk.service.ts/gs1-datakart-daily-bulk.service.ts) bulk-upload services insert into on every row -- every upload attempt failed outright with an SQL "Unknown column" error, independent of the format-mismatch root cause those same rewrites fix. Also adds the missing uq_gs1_approval_audit (process_id, audit_date, auditee_name, auditor_name) UNIQUE KEY to gs1_approval_audit_raw, which the service's own long-standing doc-comment claimed existed and relies on for ON DUPLICATE KEY UPDATE, but SHOW INDEX confirmed only PRIMARY KEY(id) live. All three tables confirmed empty (0 rows) live -- this pipeline never successfully imported a row -- so this is a pure additive/idempotent schema fix with no backfill.
  "1801_payment_voucher_attachment.sql", // Registered 2026-09-17, owner directive (CEO/CA compliance review, "no issues at all fully compliant"). A payment voucher had no way to attach supporting documentation (invoice, bank advice, approval memo) at any stage. Adds attachment_path/attachment_original_name/attachment_mime/attachment_uploaded_by/attachment_uploaded_at, same shape grn_request's own attachment columns already use. Backend saveAttachment() (payment-voucher.service.ts) + multer upload/download route pair (payment-voucher.routes.ts), mirroring grn.routes.ts's existing pattern exactly (30MB limit, pdf/jpg/jpeg/png/webp only). Locked once status='released' â€" money has moved, the attachment becomes part of the historical record (same principle as a GRN's own attachment lock at status='draft', just a different lock point since payment_voucher has no draft stage in practice).
  "1802_gs1_template_real_format.sql", // Registered 2026-09-17, same owner report as 1800. Live upload_template_master rows for all 3 GS1 upload types held EMPTY required_columns/optional_columns (1769's INSERT ... ON DUPLICATE KEY UPDATE never actually rewrites those two columns on a re-run), so BulkUploadHub's header-presence validation was a silent no-op for GS1 uploads regardless of format. Rewrites all 3 rows via direct UPDATE to the real "GS1.xlsx" export's own header row (verified live from the source file, sheet by sheet: "Email " 19 cols, "Data Kart" 20 cols, "Approval" 89 cols, 86 unique after dedup). Declares the CURRENT real format only (same 1796->1797 lesson: a union with the old imagined format means every column absent from whichever format was actually uploaded shows as missing/unknown and blocks the file); the old field names stay supported as a service-level fallback only.
  "1803_payroll_ledger_voucher.sql", // Registered 2026-09-17, owner-confirmed follow-up to the CEO/CA compliance review (39,099 historical grn_type='salary' GRNs, ~Rs 39.3 Cr, ruled closed history and never journal-posted â€" see 6260310a). Creates payroll_ledger_voucher (one row per (run x branch) salary voucher, giving each a real UUID to use as journal_entry.source_id='payroll', UNIQUE(run_id,branch_id) as the one-time-posting idempotency guard) and two new sub-heads â€" "Gross Salary" and "Employer Statutory Contribution" â€" under the existing active "Salary & Workman Compensation" expense head, which had zero sub-heads until now. Feeds payroll-journal-posting.service.ts's postSalaryVoucherToLedger(), called from a new POST /api/finance/salary-voucher/runs/:runId/post-to-ledger route (finance_head/super_admin only â€" a stronger gate than the read-only voucher-view roles). Purely additive.
  "1804_gs1_approval_audit_missing_columns.sql", // Registered 2026-09-18. Corrective fix for 1800, found by live-running the real GS1 Approval import immediately after applying 1800/1802 (owner-approved end-to-end verification): every row failed with ER_BAD_FIELD_ERROR "Unknown column 'data_source'" on gs1_approval_audit_raw. 1800 only backfilled data_source/source_reference onto the two "_daily_actual" tables and only added the missing UNIQUE KEY to gs1_approval_audit_raw, without re-checking that table's own columns for the same gap â€" same root cause (table created by an earlier run of 1769 before these columns were added to its text). Table confirmed still empty live at the time of this fix. Purely additive.
  "1805_gs1_approval_audit_per_sku_key.sql", // Registered 2026-09-18, same owner-approved end-to-end verification pass as 1804. All 7 real rows in the "Approval" sheet share the same auditee/auditor/date (it's a per-SKU/per-GTIN QC log, 7 distinct GTINs/companies, same two people, same day) â€" the inherited UNIQUE KEY (process_id, audit_date, auditee_name, auditor_name) silently collapsed all 7 into 1 row via ON DUPLICATE KEY UPDATE, live-confirmed by importing the real file and finding only 1 row where 7 were expected. Adds a gtin column and rekeys to (process_id, audit_date, auditee_name, auditor_name, gtin); deletes the 1 pre-existing collapsed row (not a real standalone record) so the immediately-following re-import lands all 7 correctly.
  "1806_email_ticket_analyst_daily_actual.sql", // Registered 2026-09-18, owner report ("HRMS Issue and observation Reginald & GS1" â€" Reginald sheet: Molecular/Reginald EmailAPR Tickets tabs have no per-analyst view). email_ticket_daily_actual only ever stored day-wise totals across every analyst. Confirmed live that the upstream ticketing DB (db_email/molecular_db_email, already live-synced by molecular-email-sync.service.ts) has tickets.assigned_to and ticket_messages.created_by as real user ids joining to users.id/name â€" a per-analyst breakdown is directly derivable from the same source, no new upload format needed. One row per (dashboard, analyst, day); opening_pending deliberately left off (a report-wide balance across all analysts together in the source Apps Script, no clean per-analyst decomposition). Purely additive.
  "1807_esign_signer_identity_check.sql", // Registered 2026-09-18, owner-directed fraud-alert feature, demonstrated live as a deliberate test: a joining kit addressed to MAS63544/RISHABH PANDEY was completed using a DIFFERENT person's Aadhaar (owner's own â€" "Shivam Shiv Giri" â€" confirmed by parsing the embedded PKCS#7 signature's X.509 certificate, see backend/src/shared/esignCertificateIdentity.ts). Neither the Luckpay status API nor the document's own visible text carries independently-verified signer identity â€" both just echo what our own system supplied; the certificate is the one place real, CA-issued identity exists. New table (not a reuse of candidate_fraud_alert, whose candidate_id is NOT NULL and models a different concern â€" duplicate identity across candidates at hiring-gate time, not signer-vs-document-owner at eSign-completion time, often well after candidate_id is populated). Alert-only: a row is written for every completed eSign, match or mismatch, nothing blocks or reverses.
  "migrations/0060_portal_admin_impersonation_log.sql", // Registered 2026-09-18 â€" found missing from this manifest while wiring a real super-admin impersonation flow. The file has sat in backend/sql/migrations/ since it was written but was never added here, so portal_admin_impersonation_log has never existed on this database: every impersonation attempt's audit-log INSERT has always failed with ER_NO_SUCH_TABLE (confirmed live). CREATE TABLE IF NOT EXISTS, idempotent. Must run before 1808 below, which ALTERs this same table's column types.
  "1808_portal_admin_impersonation_log_uuid_fix.sql", // Registered 2026-09-18. portal_admin_impersonation_log.admin_user_id/portal_user_id were declared INT UNSIGNED in 0060_portal_admin_impersonation_log.sql, but auth_user.id and client_user.id are both CHAR(36) UUIDs â€" every impersonation attempt has always failed the audit-log INSERT with a SQL error, so the feature has never actually worked. Widens both columns to CHAR(36) and adds a jti column for per-session revocation. No existing rows to preserve (the INSERT always errored).

  // META Campaign Automation (Lead Gen Ads -> screened ATS candidate). Registered 2026-09-18.
  "1809_meta_campaign_tables.sql", // Data foundation: meta_campaign (one row per requisition's Lead Gen campaign; meta_form_id is UNIQUE because it is the webhook's only routing key back to a requisition â€" META's payload carries form_id and never our IDs, so two requisitions sharing a form ID would attach candidates to the wrong one non-deterministically), meta_lead_raw (one row per form fill; meta_lead_id UNIQUE because META redelivers any leadgen_id it did not get a 2xx for, and re-processing must not mint duplicate candidates; raw_payload kept verbatim so leads can be re-parsed after a field-mapping fix, which matters because the Graph API only serves lead data for 90 days), meta_webhook_log (raw body written before parsing, MEDIUMTEXT not JSON so a malformed payload is still storable for diagnosis). No FOREIGN KEYs, deliberately: the plan's ON DELETE CASCADE to job_requisition would destroy spend history on requisition deletion, and CHAR(36) FKs across utf8mb4_unicode_ci hit the same collation trap documented on 1500/1536. Purely additive, three new tables.
  "1810_job_requisition_meta_fields.sql", // Adds bmi_assessment_url + meta_target_age_min/max + meta_target_locations (JSON) + meta_target_radius_km to job_requisition. Read by the campaign brief email (notifyMarketingTeam) and by lead-screener.service.ts, which screens an incoming lead's age against the requisition's own band rather than a global constant. Each column guarded via INFORMATION_SCHEMA prepared statements (the 1804 pattern) rather than a bare multi-column ADD COLUMN, which throws ER_DUP_FIELDNAME on re-run; `ADD COLUMN IF NOT EXISTS` as used by 538 is MariaDB-only syntax and a parse error on MySQL 8. Purely additive.
  "1811_marketing_team_email_setting.sql", // Seeds org_settings.marketing_team_emails = '[]'. The plan specified a `system_config` table, which does not exist in this schema (nor does `branch_config`); org_settings is the real key/value table and already stores JSON in its TEXT column (domain_whitelist = '[]'). Seeded empty on purpose so the feature ships inert â€" notifyMarketingTeam() returns early on an empty list, so no environment starts emailing until an admin fills it in. INSERT IGNORE, idempotent.
  "1812_meta_campaign_page_access.sql", // Registers page_catalog ATS_META_CAMPAIGNS (/ats/meta-campaigns) and grants role_page_access to the 11 roles in meta-campaign.routes.ts's read list, with can_edit only for the four in its write list. Required, not cosmetic: the route is wrapped in WorkforcePageGate and ProtectedRoute also resolves a page code from the URL, so without a role_page_access row every non-super_admin sees "Access not available" and the sidebar entry stays hidden. page_path matches the router exactly â€" a drifting path is the documented 404 cause in 1022_page_catalog_path_reconciliation.sql. Both upserts re-runnable.
  "1813_employees_aadhaar_number_candidate_id.sql", // Adds employees.aadhaar_number (CHAR(12), full Aadhaar for ESIC/PF filing) and employees.candidate_id (CHAR(36), FK back to ats_candidate). Both columns are written by employee-creation-orchestrator.service.ts in its employees INSERT (added owner decision 2026-09-02 and comment "Never written before" respectively) but no migration ever created them â€" causing ER_BAD_FIELD_ERROR on every offer approval since the code shipped, producing the generic 500 "quote reference â€¦" error. Idempotent: each column guarded via INFORMATION_SCHEMA prepared statement.
  "1814_client_portal_password_login.sql", // Registered 2026-09-18, owner-requested feature: password-based client login alongside the existing email-OTP flow. Adds client_user.login_id/password_hash/must_change_password and process_master.slug (persisted, not derived live, so a later process rename never silently changes an already-shared /:slug_clientportal URL). All additive, information_schema-guarded.
  "1815_meta_integration_config.sql", // DB-backed META credentials so a super_admin can rotate them in the UI instead of editing .env and restarting. Not a convenience: the Graph API long-lived token expires every ~60 days, so an env-only design guarantees a recurring outage only a deploy can fix. Shape follows 071_communication_provider_config.sql (non-secrets in JSON, ALL secrets in one AES-256-GCM secret_enc blob so adding a credential later needs no migration, last_test_* because "saved" and "works" are different facts), and reuses that file's COMM_SECRET key so no new key material is needed. Single row enforced by UNIQUE on a constant config_key, which also lets the service UPSERT without a prior SELECT. Also registers page META_INTEGRATION_SETTINGS (/admin/meta-integration) granted to super_admin ONLY - this screen exposes the App Secret and the ads access token, which together read lead PII and ad spend for the whole business account. Numbered 1815 because 1813/1814 were taken by concurrent work.
  "1816_marketing_brief_recipients_update.sql", // Sets org_settings.marketing_team_emails (the brief's To line) to the owner's named marketing contact, brijesh.kumar@teammas.co.in, and adds a NEW key marketing_team_cc_emails for the fixed Cc list (rajesh.ramachandran@teammas.in, shivam.giri@teammas.in). The requisition's branch head is deliberately NOT stored here — it is resolved per-send from branch_head_assignments/user_assignment_scope in notifyMarketingTeam(), because it varies per requisition and a stored address would go stale the moment a branch head changes. This IS a behaviour change versus 1811: the brief stops being inert (1811 seeded '[]') and starts actually sending, on every environment this runs on, because a real recipient now exists. ON DUPLICATE KEY UPDATE overwrites 1811's empty value on purpose.
  "1817_leave_approval_overdue_cc_employee.sql", // leave_approval_overdue was Cc-ing branch_hr on every pending-approval reminder. Owner directive 2026-09-18: only the reporting manager (To) and the employee (Cc). Removes branch HR from the noise; they are not decision-makers for a subordinate's leave approval.
  "1818_payroll_export_pages_page_access.sql", // Registers SALARY_TREND_EXPORT and ATTENDANCE_REGISTER_EXPORT (Gate-wrapped pages added 2026-09-18) in page_catalog/role_page_access; roles copied from each route's existing ProtectedRoute/navConfig, read+export only.
  "1819_payment_voucher_expense_head.sql", // payment_voucher records the vendor-mapped expense Head/Sub-head a vendor payment is against (4 nullable columns).

  // Quality-Learning Governance (QA-triggered training auto-assignment) foundation.
  "1820_quality_learning_governance.sql", // Creates skill_category, qa_trigger_rule, skill_content_mapping, training_assignment (additive, all new tables). Registers a new task_type='quality_coaching_required' into the EXISTING tat_matrix_master/escalation_matrix_master (INSERT IGNORE against their existing unique keys uq_task_branch/uq_escalation_task_level) so the existing tat-escalation.worker.ts drives TAT/escalation for this task type with zero new notification code. training_assignment does not duplicate deadline/status — it references task_tat_instance via tat_instance_id.
  "1821_training_dialer_hold.sql", // Creates training_dialer_hold: a manual-enforcement audit table for critical training TAT breaches. Vicidial/dialer is deliberately read-only in this codebase (db/dialerDb.ts triple-enforces it — SQL-verb allowlist, MySQL session READ ONLY, DB grants), so this does NOT write to Vicidial. It records that a hold should happen, and a Work Inbox item routes the actual manual pause to WFM/Ops who already have Vicidial admin access.
  "1822_training_assignment_lms_provisioning.sql", // Adds lms_learner_id/lms_provisioning_status/lms_provisioning_note/lms_content_confirmed_by/lms_content_confirmed_at to training_assignment. mcn_lms has no per-trainee content-assignment table (content visibility is a pure classroom_id -> module_master -> content_master cascade) and this codebase's own UAT governance checklist (CS-04) blocks changes to LMS-owned domains, so this does NOT write course assignments into the LMS. It tracks: (1) whether the employee's LMS learner identity exists (reusing the existing best-effort provisionLmsIdentityForEmployee write to trainee_master), and (2) whether a training coordinator has manually added the mapped content to the employee's classroom curriculum, since that step has no automatable path today.
  "1823_training_reminder_nudge_event.sql", // Seeds notification_event_config event_code='training_reminder_nudge' (enabled=1, dispatch_mode='live', cooldown_minutes=30 — deliberately short since the gateway's cooldown keys on (event_code, entity_type, entity_id) alone, not on dedupeKey, so a long cooldown would silently swallow a manager's second manual nudge attempt). Backs the manager dashboard's bulk "Send Reminder" action — distinct from the automatic task_sla_breach_l1/l2/l3 escalation ladder, which is the system escalating on a schedule rather than a manager acting on demand.
  "1824_fix_employee_page_grants.sql", // Soft-deletes five admin/payroll page grants incorrectly held by role_key='employee': PAYROLL_RUNNING_BREAKDOWN (live payroll console), ENGAGEMENT_COMMAND_CENTER, PEOPLE_EXPERIENCE_COMMAND_CENTER, EMPLOYEE_LIFECYCLE (HR admin), and SALARY_REVISION (payroll_hr/head console). All five API endpoints already 403 for employees at the route level; this removes the nav grants so the sidebar stops offering the links.
  "1825_mcn_content_builder_request.sql", // Registered 2026-09-19, owner directive ("MCN Content Builder" — Option A integration of OpenMAIC/THU-MAIC as a standalone external authoring tool, not a codebase merge). CREATE TABLE IF NOT EXISTS mcn_content_builder_request — a tooling-link worklist (requested -> in_progress -> built -> mapped, or cancelled) tracking that a coordinator asked for LMS content to be authored via OpenMAIC, pasted back an informational builder_url/export_reference once built, and eventually linked the real skill_content_mapping row created after manually uploading the export into the LMS. No write path into mcn_lms, no LLM keys in this backend, no LMS schema change — same CS-04 boundary as 1822_training_assignment_lms_provisioning.sql. builder_url is never fetched server-side.
  "1826_meta_lead_walkin_reply_columns.sql", // Adds walkin_reply_sent_at + walkin_reply_text to meta_leads. Previously numbered 1810 (conflict with job_requisition_meta_fields); renumbered to 1826 to resolve duplicate.
  ...
  "1836_journal_voucher.sql", // Registered 2026-09-22. Manual Journal Voucher (source_type='manual' of the double-entry ledger, previously with no writer): new tables journal_voucher + journal_voucher_line (draft lines; posted lines live in journal_entry_line via journalService.post at approval) and page registration FINANCE_JOURNAL_VOUCHERS. Additive: CREATE TABLE IF NOT EXISTS + page_catalog/role_page_access upserts; no existing table altered. Number 1836 collides with 1836_user_assignment_scope_cost_centre.sql above (both registered the same day by different sessions) — the manifest tracks full filenames, not numbers, so both coexist; not renumbered to avoid rewriting a file already reviewed.
  "1841_ats_onboarding_request_created_at_index.sql", // Registered 2026-09-22, owner approved. Adds idx_onb_req_created_at (created_at) to ats_onboarding_request so listOnboardingRequests()'s "ORDER BY r.created_at DESC LIMIT 500" (added same day) can walk the table in order instead of sorting the whole joined history first — /ats/onboarding-requests measured 13.5s live, the LIMIT alone only got it to 8.8s. Additive, ONLINE DDL, idempotent.
  ...
  "migrations/1867_ats_candidate_rewalkin.sql", // Registered 2026-09-25. Creates ats_candidate_rewalkin (one row per repeat walk-in registration). CREATE TABLE IF NOT EXISTS, additive. Already applied to production by hand on 2026-09-25; registered here so a fresh database gets it.
  "migrations/1870_job_requisition_deadline_automation.sql", // Registered 2026-09-25. Creates job_requisition_alert_log and job_requisition_expiry_decision for deadline alerts and HR expiry decisions. CREATE TABLE IF NOT EXISTS, additive.
  ...
  "1871_process_metric_actual_covering_indexes.sql", // Registered 2026-09-25. Adds idx_pma_feed_cover and idx_pma_source_date on process_metric_actual for the Process Operations /feeds and /processes aggregates. Online (INPLACE, LOCK=NONE), guarded and idempotent; no data change. 1870 is reserved for gnc_lob_target (pending on another branch).
  ...
  "1883_salary_start_date_governance.sql", // Registered 2026-09-25. Salary start date governance: adds employees.salary_start_pre_joining_approved TINYINT(1) NOT NULL DEFAULT 0 (information_schema-guarded; set only by salary-start-date.service.ts when Payroll Head backdates before joining with a recorded reason), creates employee_salary_start_date_audit (one row per date change: old/new date, source, authority, reason, actor), and seeds payroll_config_flags(salary_start_date_gate_enforced=false) so the payroll readiness mismatch check is a WARNING until the existing mismatches are repaired. All additive and idempotent; no existing row changes value.
  "1884_employees_salary_start_check_v2.sql", // Registered 2026-09-25. Drops employees CHECK chk_ssd_not_before_doj (1857), which refused Payroll Head's intentional backdating on employees.salary_start_date - the column payroll reads - while the old sync helper swallowed the error (70 of 213 HRMS-onboarded employees affected). DROP CHECK is metadata-only. The rule is now enforced on every write path by salary-start-date.service.ts; a replacement CHECK honouring employees.salary_start_pre_joining_approved (1883) needs a ~15 min rebuild of employees and is left for a maintenance window. Guarded, replay-safe.
  "1884_employees_salary_start_check_v2.sql", // Registered 2026-09-25. Replaces employees CHECK chk_ssd_not_before_doj (1857) with chk_ssd_not_before_doj_v2: salary_start_date >= date_of_joining OR salary_start_pre_joining_approved = 1. 1857 made Payroll Head's intentional backdating impossible at the database, and syncSalaryStartDateEverywhere swallowed the error, so employees.salary_start_date (the column payroll reads) stayed on the joining date for 70 of 213 HRMS-onboarded employees. One ALTER (drop + add together, so no window without a constraint); guarded on the v2 name, replay-safe. Requires 1883 (the flag column). Rebuilds employees (~55k rows) once - writes wait a few seconds.
...
function isIdempotentMigrationError(error: unknown): boolean {
  ...
  // dodging the message-based fallbacks below â€" those match substrings like "already exists",
 ...
 * mysql2/promise does not understand DELIMITER â€" it is a CLI-only command.
 */
  ...
  while (i < len) {
    ...
    if (!prevIsWord && /[A-Za-z_]/.test(ch)) {
      ...
      if (word === "END") {
        ...
        // control-flow terminators â€" they do NOT close a BEGIN...END block
        const isControlEnd =
          followWord === "IF" ||
          followWord === "LOOP" ||
          followWord === "WHILE" ||
...
/**
 * â"€â"€â"€ Gate 3: the schema a migration declares must actually exist afterwards â"€â"€â"€
 *
 ...
 * the reachable ones â€" payslip_emailed breaks two mounted endpoints and incentives_applied_at
 ...
 * So this asserts only what it can read unambiguously and stays silent otherwise â€" silence costs
 * a missed assertion, a wrong assertion costs an outage. Specifically it skips:
 *
 *   Â· any file containing DROP / RENAME / CHANGE COLUMN â€" the file's net intent is not
 ...
 *   Â· TEMPORARY tables, which are session-scoped and gone by the time we look
 *   Â· anything schema-qualified (`db_masmis.foo`) â€" another database this account may not see;
...
function stripNoise(sql: string): string {
  ...
  while (i < noBlock.length) {
    if (noBlock[i] === "'") {
      ...
      while (i < noBlock.length) {
        if (noBlock[i] === "\\") {
          i += 2; // backslash escape â€" skip both chars
        } else if (noBlock[i] === "'") {
          i++;
          if (noBlock[i] === "'") {
            i++; // '' doubled apostrophe = escaped quote â€" stay inside the literal
          } else {
      ...
      // Line comment â€" only reached when outside a string literal
      while (i < noBlock.length && noBlock[i] !== "\n") i++;
    } else if (noBlock[i] === "#") {
      // Hash comment â€" only reached when outside a string literal
      while (i < noBlock.length && noBlock[i] !== "\n") i++;
    } else {
      result += noBlock[i];
      i++;
 ...
 *
 * A column on a table that is itself missing is reported as the table only â€" one cause, one line,
...
export async function runPendingMigrations(
  ...
  try {
    ...
    if (!lockAcquired) {
      // Lock held by another instance (e.g. another PM2 worker already running migrations).
      // Mark health as ok-assumed and return — the caller in server.ts will treat this as
      // a lock-contention signal and start the server without crashing.
      migrationHealth = {
        status: "ok",
        applied: [],
        skipped: [],
    ...
    for (const file of files) {
      ...
      try {
        ...
        // GATE 3: the file ran without throwing â€" now prove it actually left the schema it
    ...
    // Lock-contention errors are re-thrown so server.ts can treat them as
    // "schema already current" rather than "schema broken" -- crashing the process on a
...
export function buildSchemaMigrationsInsertStatement(
  ...
  // branch below documents â€" and this half was missed when that one was fixed, which turned the
  ...
  //   2. On the next boot its SQL ran again and SUCCEEDED â€" it is idempotent, and every column
  ...
  //   4. The runner caught that and reported it as THE MIGRATION failing â€" so the error attached
  ...
  // renaming the file was the only escape â€" which re-runs an applied migration everywhere else.
  //
  ...
  // optional column, so on a table without them the clause was omitted entirely â€" the first failure
...
export async function verifySchemaVersion(): Promise<SchemaVerificationState> {
  ...
  try {
    ...
    // This intentionally does NOT reuse the app's shared pool (db/mysql.ts) â€" migrations
    ...
    // (connect + queries) stalled 79+ seconds on a single /api/health/version hit â€" every
    ...
    const work = (async (): Promise<SchemaVerificationState> => {
      ...
      try {
        ...
        if (!tables.length) {
          // Table doesn't exist â€" all migrations are pending
