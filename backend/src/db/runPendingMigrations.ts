...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1700_noc_case_opened_event.sql", // Registered 2026-09-09. Adds the 7th NOC event, noc_case_opened, for databases where 1697 was already applied before that event was written into it. A separate file rather than an edit to 1697 because 1697 is recorded success=1 on this estate and the runner will never re-execute it — editing it alone would fix a fresh install and silently leave every existing database with 6 of 7 events. Verified live 2026-09-09 against mas_hrms: 6 registered and noc_case_opened absent, so notifyCaseOpened() would have answered {outcome:'disabled', reason:"event is not registered"} and the spec's initiator-escalation email (notify the levels ABOVE whoever opened the case, never below) would have gone nowhere with no error at the call site — loadEventConfig returns null for an unregistered event and notify() reports 'disabled' without throwing. 1697 still carries the event too, so a fresh database gets all 7 from it and this file's INSERT IGNORE is then a no-op; the two paths converge rather than one superseding the other. Numbered 1700 because another session already holds 1699. Data-only, idempotent.
  ...
  // 1753/1754 restored 2026-09-11: registered earlier this session (Salary Transfer &
  // Reconciliation build), already applied to production, but a concurrent session's own
  // broad edit to this array dropped both lines -- confirmed the files still exist on disk
  // and schema_migrations already records them as applied; re-listing here only restores
  // manifest/reality parity, it does not re-run anything.
  ...
  "1759_roster_conflict_resolution_evidence.sql", // Registered 2026-09-12. Adds resolution_action/resolution_remarks/resolved_by/resolved_at to wfm_roster_conflict_log. The Roster Requests conflict modal has always collected a Resolution Action and POSTed it, but rosterConflictService.resolve() ran a bare `UPDATE ... SET resolved = 1` and the route never read req.body — the manager's decision was discarded on arrival and the table had no column to hold it (RR12, WFM Roster deep audit 2026-09-12). Additive and guarded; rows already at resolved = 1 keep NULL evidence, which honestly reflects that it was never captured.
  ...
  "1761_esic_registration_automation.sql", // Registered 2026-09-12. Creates esic_registration_case + esic_registration_event — additive tracking tables for the supervised ESIC registration bot (Google Sheets queue + Playwright workstation, backend/automation/esic-registration-bot/worker.py). Tracks only non-sensitive case status/lifecycle metadata; Aadhaar, bank and OTP values stay in the authorised Google workspace and are never written here. CAPTCHA, OTP and final portal submission remain human-only checkpoints by design.
  ...
  "1763_noc_cases_page_access.sql", // Registered 2026-09-12. Makes /payroll/noc-cases (the NOC Certificate clearance workspace) reachable by the people who actually sign a NOC. payroll.routes.tsx guards that route with <Gate pageCode="PAYROLL_NOC_CASES"> and no such row existed in page_catalog or role_page_access, so Gate's super_admin bypass was the only way in and it 403'd for the other 19 roles the route's own ProtectedRoute admits — the route file flags this in its own comment rather than hiding it. The eight signatories are Team Leader, Process Manager, HR, Branch Manager, IT, Admin, Accounts and Finance, NONE of which is a payroll role, which is exactly why the existing PAYROLL_NOC grants (the older single-document upload flow, payroll-only) do not cover this page. Role list mirrors READ_ROLES in noc-case.routes.ts exactly; if the two drift a user is admitted to a page where every action 403s, the "admitted then refused" failure this codebase has hit before. can_delete is 0 for everyone because nothing in the API deletes a NOC — granting it would advertise a capability that does not exist. This grant only decides who may OPEN the screen; which stage a user may sign, and whether they are scoped to that branch, is enforced per request against noc_signatory.role_key and cannot be expressed here. Additive, INSERT IGNORE on both tables.
  "1764_exit_clearance_task_attachment.sql", // Registered 2026-09-14. Adds nullable attachment_url VARCHAR(700) to exit_clearance_task. Supports the new WFM "Client ID deactivation" clearance task where WFM attaches the Operations confirmation email/screenshot (subject: "Update regarding analyst status in software"). Non-mandatory: all existing tasks and the other seven default clearance tasks are unaffected. Single ALTER TABLE … ADD COLUMN IF NOT EXISTS, safe as no-op on any environment where the column already exists.
  ];
