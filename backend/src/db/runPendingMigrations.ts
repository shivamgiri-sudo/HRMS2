...
const MIGRATION_MANIFEST: string[] = [
 ...
 (cost centre BSS/BO/NOIDA-2/577 is a back-office pool that works entirely for BSS/BO/NOIDA-2/576/Onfido, understating 576's margin and overstating 577's cost). Renumbered from 1785 to 1787 — 1785 and 1786 were taken by another concurrent session's already-merged work (leave/exit CC notification swap, cost-centre/process backfill) by the time this was ready to push. CREATE TABLE IF NOT EXISTS pnl_employee_cost_centre_override — one row per employee, mapping their pay to a different cost centre for P&L attribution only; employees.cost_centre_id (roster/attendance/leave/payroll) is never touched. Read via LEFT JOIN + COALESCE in pnl-reconciliation.service.ts (readPayroll/readUnallocatedPayroll/exceptions) and ceo-overview.service.ts (peopleByBranch), so Live P&L, Insights, Trend and CEO Overview all pick it up the same way. Purely additive.
  ];
