...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1708_housing_owner_sale_raw.sql", // Registered 2026-09-09, numbered 1708 because 1707 was claimed by another session the same day. Housing Owner's "Sale Raw" and "Call Logs" sheets (per its own SOP: Sale Raw pastes sale data "up to the Discount % column"; Call Logs is a cleaned Tata Dialer Agent Performance export -- no DB backing exists for either). Real sample read via Chrome (the user's own authenticated Drive session): "Housing Owner Sep'26.xlsx", sheets "Sale Raw" and "Call Logs". This source is a plain .xlsx (not .xlsb) whose Date columns already resolve to native dates. Creates housing_owner_sale_raw (per-order) and housing_owner_call_logs (per-agent per-day, durations in seconds). Additive: two new tables only.
  ...
  "1713_lp_cdr_and_cr_report_raw.sql", // Registered 2026-09-09. LP's "10. CR Reports"/"11. CDR" (per its own SOP: "Open BPO Panel... Select Mascallnet NRGN Call History... Paste into CR Reports sheet" / "Select BPO CR Reports... Paste into CDR sheet" -- no DB backing exists for either). Real samples read from the same two LP workbooks already downloaded this session for LP Leads/APR, sheets "CDR" and "CR Report". The CDR sheet's own "Connected Time" column is corrupted at the source (time-only cell format, but the underlying value decodes to nonsense years like 3163/3311) -- deliberately not read; "Disconnected Time" (plain readable text) is the only timestamp kept. Row identity for lp_cdr_raw verified live against all 1,872 real Regional rows: (Ticket_Ref, Unique, Date) is unique with zero collisions -- (Ticket_Ref, Attempt) alone collides 450 times, since "Attempt" is a fixed per-lead total, not a per-call sequence (that's what "Unique" is). lp_cr_report_raw's (Name, Mobile, CreatedOn) verified unique across all 219 real Regional rows. Additive: two new tables only.
  ...
  "1716_housing_premium_cdr_raw.sql", // Registered 2026-09-09. Found while auditing every sheet of the same real Housing Premium workbook downloaded this session for Sale Raw (sql/1706) -- a full outbound call log, 194,546 real rows in the sample file, not named in Housing Premium's own SOP text but real data with no DB backing anywhere. Scoped down to the columns a KPI actually needs (Routing Numbers/Routing Status/Start Time/Time/Count/Unique Count/"V+W"/Talk Time left out deliberately, same reasoning as clovia_crm_disposition sql/1705). Row identity verified live across all 194,546 real rows with zero collisions: (Date, Date Row Count) -- CALLER+MEMBER+End_Time alone collides (a re-attempt can log an identical phone/agent/timestamp with a different DURATION). End_Time is a FRACTIONAL Excel serial (date+time), floored per the same convention as clovia_crm_disposition; DURATION/Talk_Duration/Ringing_Duration are plain seconds already, unlike this session's other duration columns. Additive: new table only.
  ...
  "1718_clovia_apr_daily_actual.sql", // Registered 2026-09-09. Found while auditing every sheet of the same Clovia workbook family used this session for Chat Performance/CRM Disposition/Team Alignment -- "APR-Utilization Raw" holds per-agent per-day productivity across all three Clovia LOBs (Inbound, Outbound, Email), not named in Clovia Steps.docx's own SOP text but real data with no DB backing anywhere. Scoped down from the source's full 35 columns to the ones a KPI actually needs, same reasoning as clovia_crm_disposition (sql/1705). Row identity: the sheet's own "Unique ID" concatenates MAS ID+Date -- used here as its logical parts (MAS ID, Date), verified live as unique across all 74 real non-blank rows. Duration columns are day-fraction decimals, same convention as DU APR (sql/1710). Additive: new table only.
  "1719_clovia_ib_cdr_raw.sql", // Registered 2026-09-09. Found while auditing every sheet of the same Clovia workbook family used this session for Chat Performance/CRM Disposition/Team Alignment/APR Utilization -- "IB CDR Raw" is a full inbound call log, 1,627 real rows in the sample file, not named in Clovia Steps.docx's own SOP text but real data with no DB backing anywhere. Scoped down from the source's full 29 columns to the ones a KPI actually needs, same reasoning as clovia_crm_disposition (sql/1705). Row identity verified live across all 1,627 real rows with zero collisions: (Phone Number, CallTime, Agent Id, Count) -- the raw export contains genuine duplicate rows for the same physical call (one tagged Unique, one Repeat), and even Phone+CallTime+Agent Id alone still collides on those. NOTE: this commit also fixes a real bug caught while verifying this file -- parseSecondsFlexible's day-fraction cutoff was <= 3 (should be < 1, since a same-day fraction can never reach 1.0), which misread a plain 2-3 second duration as "2-3 days"; fixed here and in the two already-pushed files that copied the same heuristic (du_apr_daily_actual sql/1710, clovia_apr_daily_actual sql/1718). Additive: new table only.
  "1720_clovia_outbound_cdr_raw.sql", // Registered 2026-09-09. Found while auditing every sheet of the same Clovia workbook family used this session for Chat Performance/CRM Disposition/Team Alignment/APR Utilization/IB CDR Raw -- "Outbound Report" is a full outbound call log, 1,317 real rows in the sample file, not named in Clovia Steps.docx's own SOP text but real data with no DB backing anywhere. Row identity verified live across all 1,317 real rows with zero collisions: (UAN, Count) -- the sheet's own UAN column (a phone+date concatenation) alone collides 48 times, same genuine-duplicate-raw-export-row pattern already found in IB CDR Raw (sql/1719). Length (Sec) arrives already in plain seconds; Start Time/End Time are FRACTIONAL Excel serials, floored per the clovia_crm_disposition convention. Additive: new table only.
  ...
  "1724_bella_repeat_apr_daily_actual.sql", // Registered 2026-09-09. Found while auditing every sheet of "Bella Vita Repeat LOB Mis Dashboard July26.xlsb", not named in BELLAVITA Dashboard.docx's own SOP text, but real data with no DB backing anywhere: per-agent per-day call/chat productivity for the Repeat Customer LOB specifically, 615 real rows in the sample file. Verified live: all 4 real MAS codes sampled resolve to real employees.employee_code rows with matching names. This dashboard's own "Sale Raw" sheet was checked and confirmed to duplicate the already-live db_masmis.bb_sale table (an exact sample row matched live) -- skipped as already covered; "Cancelled Orders" was checked and confirmed genuinely empty (header only) -- skipped; "Team Details" was checked and found corrupted (literal "0x2a" Excel error codes in its Name column) -- skipped per this session's standing rule against guessing at broken data. Row identity: the sheet's own "Unique ID" (Date+NOIID concatenation) is unique across all 614 real non-blank rows. Additive: new table only.
  ...
  "1730_dalmia_ib_cdr_raw.sql", // Registered 2026-09-10. Dalmia Cement's own real MIS workbook ("Dalmia July'26.xlsx", found via Drive search, a real dashboard with Snap/Performance/Business Dashboard sheets) has an "IB CDR Raw" sheet: 966 real per-call inbound CDR rows, 2026-07-01 to 2026-07-14, real MAS agent codes (e.g. MAS62620=BAGA LAXMAN RAO, matching employees exactly). Checked across every schema on both hosts (mas_hrms, dialer_db, db_masmis, db_external, Shivamgiri, db_audit, onfido_db, bella_db) -- zero "dalmia"-named tables found anywhere, confirming this is a genuine gap. Row identity: (Agent Id, CallTime, Phone Number) -- one exact byte-identical duplicate row pair found live, handled via ON DUPLICATE KEY UPDATE. Live-verified: scripts/run-dalmia-imports.ts run for real, 965/966 imported (1 merged into its duplicate), KPI Studio wired (DALMIA_IB_CDR_TOTAL_CALLS/CONNECTED_PCT/ACHT) and checked against real data (2026-07-01: 70 calls, 69 connected). Additive: new table only.
  ...
  "1733_dalmia_apr_utilization_raw.sql", // Registered 2026-09-10. The same Dalmia Cement workbook's "APR-Utilization Raw" sheet: per-agent per-day productivity, 109 real rows, 2026-07-01 to 2026-07-14, real MAS codes confirmed live (e.g. MAS62624=J ANANTHALAKSHMI). Dropped as broken VLOOKUP "#N/A" formula errors: Team Leader/FHD/Tenure/Sub Lob. The sheet has two columns both literally named "Attendance" (a genuine duplicate header) which collapse to one JSON key -- only the surviving text code ("P"/"HD") is kept. Live-verified: 109/109 imported via scripts/run-dalmia-imports.ts, KPI Studio wired (DALMIA_APR_TOTAL_CALLS/ACHT/UTILIZATION_PCT) and checked against a real row (MAS62624 2026-07-01: utilization=41.329%, ACHT=134s). Additive: new table only.
