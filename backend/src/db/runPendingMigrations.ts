...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1757_roster_command_center_console_page_codes.sql", // Registered 2026-09-16 — found unregistered by a live audit, 4 days after being written and after the console it gates was reportedly "deployed and live-tested" on 2026-09-12. ANOTHER migration, 1757_bellavita_masmis_uploaders_real_headers.sql, silently claimed the 1757 slot (a concurrent-session number collision, same failure mode as the 1755/1756 note above, just not caught at the time) and is the one recorded in schema_migrations for filename "1757..."; neither 1757 file was ever in this array, so this one never ran. Live-confirmed impact: page_catalog had ZERO of the 8 WFM_ROSTER_* console page codes and role_page_access had ZERO rows for any of them — every role except super_admin (which bypasses page_catalog checks entirely via useUserRole.ts's `isSuperAdmin ||` shortcut) has been unable to see 7 of the Roster Command Center's 8 tabs (only Live Monitoring, migration 1766, existed) since the console shipped. This migration is unchanged from how it was written; only the registration was missing. Purely additive (ON DUPLICATE KEY UPDATE throughout, matches each tab's own backend requireRole() exactly, invents no new role).
  ...
  "1760_exit_absconding_since.sql", // Registered 2026-09-12. Adds exit_request.absconding_since (nullable DATE) + index. The New Exit form has always demanded "Absconding Since" as a mandatory field and discarded it — nothing sent it, the create schema would have rejected it, and no column existed to hold it; its only effect was auto-filling the proposed last working day as that date PLUS SEVEN DAYS, labelled "grace period". Owner ruling 2026-09-12: the 7 days is how long the company waits before deciding, not time the employee is paid for, so the leaver's paid-through date was landing a week after they stopped working — and that date feeds payroll's employment-end-date resolver, which prorates the final month. Kept separate from last_working_day_confirmed on purpose: this is the attendance fact (last day present, never changes), that is the HR decision (can be revised). Additive and guarded; existing rows keep NULL, which reads correctly as "not an absconding, or predates this column".
  "1762_fnf_transfer_batch.sql", // Registered 2026-09-12. Creates fnf_transfer_batch + fnf_transfer_batch_item + fnf_transfer_import — the Full & Final settlement's OWN bank-transfer batch (owner ruling 2026-09-12: separate from monthly salary, same bank-file machinery and approvals). Mirrors salary_transfer_batch/_item's shape and its open_flag generated-column dedup trick, keyed on full_final_calculation_id instead of run_id because F&F has no payroll run to key against. Additive only; no existing table touched.
  ...
  "1771_exit_clearance_dedicated_pages.sql", // Registered 2026-09-15. Registers PROVISIONING_MANAGER_HANDOVER / PROVISIONING_HR_EXIT / PROVISIONING_PAYROLL_EXIT / PROVISIONING_TRAINER_EXIT in page_catalog + role_page_access, so manager/hr/payroll/trainer (who own 5 of the 9 exit_clearance_task areas but had no dedicated page — only a read-only drawer) can reach their new Exit Clearance queue pages. admin/wfm's own clearance tasks instead gained a new section on their existing PROVISIONING_ADMIN/PROVISIONING_WFM_ALIGNMENT pages, needing no new page_catalog rows. Purely additive (ON DUPLICATE KEY UPDATE throughout, can_create=0 everywhere — task generation stays admin/hr-only).
  "1772_exit_clearance_it_role_retarget.sql", // Registered 2026-09-15. Owner ruling: "IT access closure" clearance task moves from owner_role='admin' to 'it' — it's IT's own work and was never reachable by a pure IT-only account. Backfills the 8 currently-open rows only; cleared/waived history untouched. Matching code change in exit-intelligence.service.ts (new tasks already create with owner_role='it').
