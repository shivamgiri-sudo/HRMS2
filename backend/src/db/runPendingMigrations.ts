...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1623_team_roster_page_access_for_managers.sql", // Grants TEAM_ROSTER page access to manager-shaped roles (manager, assistant_manager, team_leader, process_manager, branch_head) so the page-code gate and the role_page_access gate describe the same audience. 78 active employees have direct reports; 64 hold only the employee role — a separate gap addressed by manager-attribution. Additive role_page_access inserts only, idempotent via INSERT IGNORE.
  "1624_employee_manager_history.sql", // Creates employee_manager_history for effective-dated manager attribution — one row per (employee, manager, period) so "who managed this person on date X" is a lookup rather than a guess against today's pointer. Seeds the current state as provenance='seed'. Additive: one new table plus seed rows. No existing column, row or index is altered.
