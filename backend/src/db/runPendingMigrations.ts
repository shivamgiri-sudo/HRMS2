...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1727_finnable_quality_audit_raw.sql", // Registered 2026-09-10. Finnable's own "Finnable Audit" Google Sheet (Audit_Data tab) -- found via Drive search after "Finnable_Dashboard_v4" (a downloaded Apps Script project) turned out to be a generic, undeployed template with literal "YOUR_HOST"/"YOUR_DATABASE" placeholders, nothing real to verify against. The real sheet is an AI call-quality audit pipeline output, 423 real rows in the sample, no DB backing anywhere. Verified live: all 3 real MAS codes sampled resolve to real employees.employee_code rows. Scoped down from the source's full 78 columns to the ones clearly structured across the real sample -- the dozens of audit-taxonomy fields are 'None' for the overwhelming majority of real rows and their scoring semantics are not verifiable without the QA team, so left out rather than guessed, same discipline as clovia_quality_audit_raw (sql/1723). The sheet's own "entrydate" column does not hold dates in the real sample (e.g. "29:37.8") -- a genuine source defect, not coerced. "MobileNo" is NOT a phone number in the real data (values like "FN1286254440") -- stored as reference_number, not assumed to need masking. Row identity: the sheet's own "id" column, verified live unique across all 423 real rows. No KPI Studio wiring, same reasoning as clovia_quality_audit_raw. Additive: new table only.
  ];
