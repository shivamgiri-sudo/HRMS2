...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1884_employees_salary_start_check_v2.sql", // Registered 2026-09-25. Drops employees CHECK chk_ssd_not_before_doj (1857), which refused Payroll Head's intentional backdating on employees.salary_start_date - the column payroll reads - while the old sync helper swallowed the error (70 of 213 HRMS-onboarded employees affected). DROP CHECK is metadata-only. The rule is now enforced on every write path by salary-start-date.service.ts; a replacement database CHECK needs a ~15 min rebuild of employees and is left for a maintenance window. Guarded, replay-safe.
];
