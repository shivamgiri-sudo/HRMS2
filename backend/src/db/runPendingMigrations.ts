...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1213_wfm_shift_master_immutability_trigger.sql", // creates trg_wfm_shift_master_protect_locked, which enforces that a locked shift version cannot be edited in place. Depends on 1200 having added is_locked, so it must follow it and does. Idempotent: DROP TRIGGER IF EXISTS immediately before CREATE TRIGGER, which is supported syntax here
  "460_budget_line_tax_amendment.sql", // creates finance_budget_line_tax_amendment, read and written by process-pnl/branch-budget.service.ts for the governed two-step tax amendment. Table exists on production out of band. Listed last rather than in numeric position because its FK references finance_budget_header, so it must run after the finance phase files below it in a fresh build. CREATE TABLE IF NOT EXISTS; its only DROP match is the ON DELETE CASCADE inside that FK definition, not a statement // seeds page_catalog + role_page_access for PAYROLL_BANK_READINESS. Ships WITH the page rather than after it: WorkforcePageGate resolves access from role_page_access, so a route and component with no catalog row is invisible to everyone but super_admin (who is elevated to all active page codes unconditionally) — the FINANCE_COST_CENTRES state that 1066 and 1104 both cited as a warning and neither fixed. can_view mirrors READ_ROLES in bank-payment-readiness.routes.ts exactly and can_edit mirrors MANAGE_ROLES, so no role reaches the page whose API calls would then be refused. can_export mirrors PAYROLL_EXPORT_ROLES but does NOT by itself release account numbers: GET /payment-file additionally requires hasOrgWideScope(), an explicit user_assignment_scope row with scope_type=all, which no page grant can bypass. can_delete is 0 for everyone — an exception is resolved or waived, never erased, because why someone could not be paid is audit history. Additive and idempotent, two INSERT ... ON DUPLICATE KEY UPDATE against reference data.
