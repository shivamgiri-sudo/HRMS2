...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1674_communication_send_block.sql", // Owner asked 2026-09-06 for a real "Pause SMS" switch. The existing UI toggle (Settings > Communication Config, is_enabled) does not do it: provider.factory.ts falls back to env-var credentials whenever is_enabled=0 (true for all 3 channels in production today, verified 2026-09-05), so flipping it off changes nothing — deliberately so, per provider.interface.ts's isConfigured() doc comment and unconfigured-channel-skip.contract.test.ts, since treating is_enabled as an on/off switch would have silenced email, the only channel that has ever delivered (907/4 vs 0/901 SMS, 0/903 WhatsApp, measured 2026-08-08). Adds 4 columns to communication_provider_config (send_blocked, block_reason, blocked_by, blocked_at) — a genuinely separate flag, guarded at providerFactory itself (both getProviderAsync and getProvider) rather than folded into is_enabled, so every caller (dispatch.service.ts, ats.otp.service.ts, ats.onboarding.service.ts) is covered without touching the protected fallback behaviour. Purely additive: all existing rows default send_blocked=0, unaffected until an admin explicitly pauses a channel through the new UI control. Idempotent, information_schema-guarded. Applied to production 2026-09-06: columns verified live, all 3 channels still send_blocked=0, unaffected.
  ];
