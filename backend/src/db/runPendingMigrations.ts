...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1644_onfido_utilization_upload.sql", // Creates onfido_utilization_upload (queue-wise daily HC snapshot: Approved/Required/Active HC, Buffer %, Shortfall) plus the ONFIDO_UTILIZATION_BULK upload_template_master row for the Onfido Process Dashboard's "Onfido Utilization" bulk upload feature. UNIQUE KEY (upload_date, queue_name) is the override-not-duplicate contract at the database level: the paired route's ON DUPLICATE KEY UPDATE relies on it so a re-upload for the same date/week replaces the existing row instead of creating a second one. SCAFFOLD: column names are modelled on the Queue Wise panel already shown in the Onfido dashboard mock (Required HC = Approved HC x 120%) because the real Onfido Utilization sheet template was never attached to the request; adjust column names here (and in onfido-utilization.routes.ts, which reads them by name) if the real sheet differs. Purely additive, no DROP, no DELETE, no FOREIGN KEY.
