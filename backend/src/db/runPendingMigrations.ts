...
const MIGRATION_MANIFEST: string[] = [
  ...
  "1688_payroll_subtab_grant_preservation.sql", // Backfills each payroll sub-tab page code from the page code its merged parent is gated on, so per-tab RBAC (src/components/security/TabGate.tsx) changes nobody's access on the day it ships. Five payroll screens were merged into tabbed pages and N page codes collapsed into 1 -- one PAYROLL_BANK_READINESS grant, held by 11 roles, reached Payment Center's disbursal pipeline, payment-file export, CSV upload and manual payment entry. The folded screens' codes still exist with their pre-merge grants, so the tabs are gated on those rather than on anything new; but read literally those grants are narrower than the page grant and nobody maintained them after the merge, so gating alone would revoke access in daily use: 8 roles would lose the Disbursal tab and payroll_branch/payroll_head would lose both PF tabs and land on an empty page. This INSERTs the child grant from the parent row (flags copied, WHERE NOT EXISTS, never an UPDATE or DELETE), reading role_page_access rather than a hardcoded role list so it preserves what the LIVE table holds -- including grants added through the Access Control UI that never appear in backend/sql. Narrowing who may disburse then stays a separate, visible decision. Idempotent and safe to re-run; ends with two verification SELECTs, one of which reports any child page_catalog row left inactive, since getAccessMe() filters on it and an inactive row would hide the tab regardless of the grant.
  ];
