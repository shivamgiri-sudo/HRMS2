...
describe("Migration Governance", () => {
  ...
  describe("Migration Manifest Validation", () => {
    it("should have no duplicate entries in manifest", async () => {
      const { MIGRATION_MANIFEST } = await import("../runPendingMigrations.js") as any;
      // Note: The manifest is not exported directly, but we can test manifest structure
      // through the verifySchemaVersion function's behavior
