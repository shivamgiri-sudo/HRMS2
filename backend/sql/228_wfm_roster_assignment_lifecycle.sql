...
--     DROP COLUMN is_week_off,
--     DROP COLUMN final_roster_status,
--     DROP COLUMN employee_ack_status,
--     DROP COLUMN employee_ack_at,
--     DROP COLUMN employee_rejection_reason,
--     DROP COLUMN manager_action_status,
--     DROP COLUMN manager_action_by,
--     DROP COLUMN manager_action_at,
--     DROP COLUMN manager_action_reason,
--     DROP COLUMN system_decision_reason,
--     DROP COLUMN published_to_rta_at;
--   DROP INDEX idx_wra_cycle ON wfm_roster_assignment;
--   DROP INDEX idx_wra_final_status ON wfm_roster_assignment;
--   DROP INDEX idx_wra_ack_status ON wfm_roster_assignment;
-- ============================================================
...
ALTER TABLE wfm_roster_assignment
  ...
  ADD COLUMN cycle_id          VARCHAR(36)  NULL COMMENT 'FK weekly_roster_cycle.id — set when assignment originates from a cycle'
    AFTER plan_id,

  -- Shift template reference (wfm_shift_template) in addition to legacy shift_id (wfm_shift_master)
  ADD COLUMN shift_template_id VARCHAR(36)  NULL COMMENT 'FK wfm_shift_template.id — preferred over shift_id for new assignments'
    AFTER cycle_id,

  -- Week-off flag
  ADD COLUMN is_week_off       TINYINT(1)   NOT NULL DEFAULT 0
    AFTER shift_template_id,

  -- ── Final lifecycle status (source of truth for RTA) ──────────────────────
  ADD COLUMN final_roster_status ENUM(
    'generated',
  ...
  ADD COLUMN employee_ack_status       ENUM('pending','acknowledged','rejected')
                                                     NOT NULL DEFAULT 'pending',
  ADD COLUMN employee_ack_at           DATETIME     NULL,
  ADD COLUMN employee_rejection_reason VARCHAR(500) NULL,

  -- ── Manager action ─────────────────────────────────────────────────────────
  ADD COLUMN manager_action_status     ENUM(
    'pending','realigned','force_approved','escalated','rejected_request'
  ) NULL,
  ADD COLUMN manager_action_by         VARCHAR(36)  NULL COMMENT 'auth_user.id who acted',
  ADD COLUMN manager_action_at         DATETIME     NULL,
  ADD COLUMN manager_action_reason     VARCHAR(500) NULL,

  -- ── System / engine metadata ───────────────────────────────────────────────
  ADD COLUMN system_decision_reason    VARCHAR(500) NULL COMMENT 'Why the engine made this assignment',
  ADD COLUMN published_to_rta_at       DATETIME     NULL;

-- Indexes for RTA sync and manager review queue
ALTER TABLE wfm_roster_assignment
  ADD INDEX idx_wra_cycle        (cycle_id),
  ADD INDEX idx_wra_final_status (final_roster_status),
  ADD INDEX idx_wra_ack_status   (employee_ack_status);
