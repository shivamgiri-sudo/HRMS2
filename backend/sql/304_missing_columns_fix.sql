...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='salary_prep_line' AND COLUMN_NAME='overtime_pay'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='ats_candidate' AND COLUMN_NAME='employee_code'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='jclr_entries'
  ) THEN
