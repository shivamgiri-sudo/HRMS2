...
ALTER TABLE ats_employment_offer
  ADD COLUMN IF NOT EXISTS pf_opt_out TINYINT(1) NOT NULL DEFAULT 0
    COMMENT '1 = employee voluntarily opted out of PF under EPF Act §17(1)',
  ADD COLUMN IF NOT EXISTS pf_opt_out_consent_at DATETIME NULL
...
ALTER TABLE candidate_onboarding_profile
  ADD COLUMN IF NOT EXISTS pf_opt_out_elected TINYINT(1) NOT NULL DEFAULT 0
    COMMENT 'Candidate elected to opt out of PF on Form 11 online step (1 = yes)',
  ADD COLUMN IF NOT EXISTS pf_opt_out_consent_text TEXT NULL
    COMMENT 'Full Form 11 declaration text shown to candidate at time of consent',
  ADD COLUMN IF NOT EXISTS pf_opt_out_consented_at DATETIME NULL
