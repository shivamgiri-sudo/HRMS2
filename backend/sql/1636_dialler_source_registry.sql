...
-- second normalized-row shape for the same idea. This table carries no FOREIGN KEY, unlike
-- migration 1500's — migration 1500's FK to `process_master` is the one currently blocking every
...
CREATE TABLE IF NOT EXISTS dialler_source_column_mapping (
  ...
  KEY idx_dscm_source_active (dialler_source_id, active_status)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci
