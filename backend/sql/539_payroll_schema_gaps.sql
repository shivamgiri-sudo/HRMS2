...
ALTER TABLE statutory_config
  ADD COLUMN IF NOT EXISTS is_active TINYINT(1) NOT NULL DEFAULT 1 AFTER config_value;
...
ALTER TABLE salary_prep_run
  ADD COLUMN IF NOT EXISTS financial_year VARCHAR(9) NULL AFTER updated_at,
  ADD COLUMN IF NOT EXISTS payroll_model_version VARCHAR(50) NULL AFTER financial_year;
...
ALTER TABLE salary_prep_line
  ADD COLUMN IF NOT EXISTS gross_before_lwp DECIMAL(12,2) NOT NULL DEFAULT 0 AFTER gross_salary,
  ADD COLUMN IF NOT EXISTS employer_statutory_cost DECIMAL(12,2) NOT NULL DEFAULT 0 AFTER pf_employer,
  ADD COLUMN IF NOT EXISTS manual_adjustment_total DECIMAL(12,2) NOT NULL DEFAULT 0 AFTER other_deductions,
  ADD COLUMN IF NOT EXISTS calculation_status VARCHAR(30) NOT NULL DEFAULT 'pending' AFTER status,
  ADD COLUMN IF NOT EXISTS calculation_version VARCHAR(50) NULL AFTER calculation_status,
  ADD COLUMN IF NOT EXISTS calculation_notes TEXT NULL AFTER calculation_version;
