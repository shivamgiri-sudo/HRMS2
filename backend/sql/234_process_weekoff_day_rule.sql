...
-- Ensure nullable columns so INSERT IGNORE works across schema versions
-- (trigger_event and audience are NOT NULL in migration 132; give them defaults here
--  for this seed-only batch — production already has these rows via IGNORE)
ALTER TABLE notification_template
  MODIFY COLUMN trigger_event VARCHAR(100) NOT NULL DEFAULT 'wfm_notification',
  MODIFY COLUMN audience      VARCHAR(100) NOT NULL DEFAULT 'employee';

INSERT IGNORE INTO notification_template
  (id, template_code, template_name, subject, body_template, channel, trigger_event, active_status)
VALUES
  (UUID(), 'WEEKOFF_DEMAND_CONFLICT',
   ...
   'in_app', 'weekoff_demand_conflict', 1),
  ...
  (UUID(), 'WEEKOFF_ALTERNATE_ASSIGNED',
   ...
   'in_app', 'weekoff_alternate_assigned', 1),
  ...
  (UUID(), 'WEEKOFF_NO_PREFERENCE_AUTO_ASSIGNED',
   ...
   'in_app', 'weekoff_auto_assigned', 1),
  ...
  (UUID(), 'MANAGER_REALIGNMENT_DONE',
   ...
   'in_app', 'roster_realignment_done', 1),
  ...
  (UUID(), 'MANAGER_FORCE_APPROVED',
   ...
   'in_app', 'roster_force_approved', 1),
  ...
  (UUID(), 'ROSTER_ESCALATED_TO_HR',
   ...
   'in_app', 'roster_escalated_to_hr', 1),
  ...
  (UUID(), 'COVERAGE_SHORTAGE_ALERT',
   ...
   'in_app', 'coverage_shortage_alert', 1),
  ...
  (UUID(), 'FINAL_ROSTER_PUBLISHED',
   ...
   'in_app', 'final_roster_published', 1);
