...
```bash
# Run migration
mysql -h <mas_hrms DB host — see backend/.env> -u shivam_user -p<set DB_PASSWORD in backend/.env> mas_hrms < backend/sql/101_client_master_enhancement.sql

# Verify tables
mysql -h <mas_hrms DB host — see backend/.env> -u shivam_user -p<set DB_PASSWORD in backend/.env> mas_hrms -e "SHOW TABLES LIKE '%client%';"
